  ...
  return (
    <div className="max-w-4xl mx-auto p-3 md:p-10 space-y-6 md:space-y-10 min-h-screen text-slate-200 font-sans pb-10 bg-[#050507] text-left border-4 md:border-8 border-emerald-500/50 rounded-[2rem] md:rounded-[4rem] my-2 md:my-4 shadow-[0_0_100px_rgba(16,185,129,0.2)]">
      <div className="text-center space-y-1 md:space-y-3">
        <Badge className="bg-blue-600 text-white font-black italic px-3 py-0.5 text-[8px] md:text-[10px] uppercase rounded-full">Contact Automation Engine</Badge>
        <h1 className="text-4xl md:text-6xl font-black text-white uppercase italic tracking-tighter leading-none drop-shadow-2xl">Contact Sync</h1>
      ...
      <div className="bg-[#13141f] border-2 border-white/5 rounded-[2.5rem] p-6 md:p-10 shadow-2xl space-y-8 relative overflow-hidden">
        ...
        <div className="bg-[#0a0b14] border border-white/5 rounded-3xl p-6 md:p-10 flex items-start gap-6 shadow-inner text-left">
          ...
          <div className="space-y-3">
            <p className="text-[12px] font-black text-blue-500 uppercase tracking-[0.3em] italic mb-2">運用プロトコル / CONTACT SYNC</p>
            <div className="space-y-3 text-sm md:text-xl font-black text-slate-200">
              <p className="flex items-center gap-4 leading-snug"><span className="text-blue-500 italic text-2xl">#1</span> 名刺の写真をフォルダへ保存、または撮影</p>
              <p className="flex items-center gap-4 leading-snug"><span className="text-blue-500 italic text-2xl">#2</span> 名前・会社名・連絡先をAIが自動で読み取り</p>
              <p className="flex items-center gap-4 leading-snug"><span className="text-blue-600 italic text-2xl">#3</span> 各人を「本物の連絡先」へ1行ずつ自動追加</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <div className="space-y-6">
            <button 
              ...
              className={`w-full h-32 ${isProcessing ? 'bg-slate-800' : 'bg-blue-600 hover:bg-blue-500'} text-white font-black rounded-[2rem] shadow-2xl flex flex-col items-center justify-center gap-2 transition-all active:scale-95 border-b-8 border-blue-900 active:border-b-0`}
            >
              {isProcessing ? (
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="bg-black/40 p-6 rounded-[2rem] border border-white/5 text-center shadow-inner">
                  <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1 italic text-left pl-2">登録済み人数</p>
               ...
               <div className="bg-black/40 p-6 rounded-[2rem] border border-white/5 text-center shadow-inner text-left">
                  <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1 italic pl-2">APIステータス</p>
                  <p className="text-xl font-black text-emerald-500 italic uppercase">Contacts Active</p>
               </div>
            </div>
          </div>
          ...
          <div className="bg-black rounded-[3rem] p-8 border border-white/5 shadow-inner flex flex-col relative overflow-hidden h-[300px] text-left">
             <div className="flex items-center gap-3 text-blue-500 font-black italic uppercase text-xs mb-4"><Zap size={16} /> Data Processing Output</div>
      ...
      <div className="bg-blue-600/5 border-2 border-blue-500/20 rounded-[2.5rem] p-8 space-y-4 italic shadow-inner text-left">
         <div className="flex items-center gap-3 text-blue-500">
            <Database size={20} />
            <p className="text-xs font-black uppercase tracking-widest">Business Intelligence Protocol</p>
         </div>
      ...
      <div className="text-center opacity-10 mt-10 font-black uppercase tracking-[0.5em] italic text-[10px]">CRM Automation Hub • NextraLabs 2026</div>
    </div>
  )
}
...
const NoSSRWrapper = dynamic(() => Promise.resolve(MasterEngine), {
  ssr: false,
  loading: () => <div className="min-h-screen bg-[#050507] flex items-center justify-center font-black italic text-emerald-500 animate-pulse uppercase tracking-[0.5em]">Initializing Contact Node...</div>
})
