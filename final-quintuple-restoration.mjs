...
const fixList = [
  { name: 'ai-recipe-scope', label: 'AIレシピ・スコープ', icon: '🍳', what: '食材からのレシピ生成', how: '冷蔵庫にある食材を入力。AIが最高の料理を提案します。' },
  { name: 'disaster-agent', label: 'AI災害エージェント', icon: '🚨', what: '災害情報のリアルタイム解析', how: '状況を入力。AIが最適な避難行動をアドバイスします。' },
  { name: 'sales-automation', label: 'AIセールス・オートメーション', icon: '📈', what: '営業プロセスの自動化', how: '顧客情報を入力。AIが最適なアプローチ案を作成します。' }
];
...
export default function ${t.name.split('-').map(s => s.charAt(0).toUpperCase() + s.slice(1)).join('')}() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-10 space-y-16 min-h-screen text-slate-200 font-sans pb-40">
      ...
      <DebugPanel data={null} toolId="${t.name}" />
    </div>
  )
}
`;
...
console.log('✅ QUINTUPLE CHECK COMPLETE: REMAINING TOOLS FULLY RESTORED.');