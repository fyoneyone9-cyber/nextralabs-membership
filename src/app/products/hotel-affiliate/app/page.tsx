...
import { 
  ...
  Network, Globe, ShoppingBag, Store, LayoutList
} from 'lucide-react'
...
export default function HotelAffiliateApp() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult("指定されたホテルの特徴と周辺イベントを解析しました。今週末に付近で大型フェスが開催されるため、『直前予約・穴場宿』として楽天アフィリエイトリンクを含む紹介文を生成しました。想定成約率は4.2%です。");
      ...
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-emerald-500/20 pb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl md:text-5xl font-black italic uppercase tracking-tighter text-white">Hotel Affiliate AI</h1>
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">
            紹介したいホテル名やエリアを入力してください。AIが楽天トラベルの最新空室データとトレンドを解析。SNSやブログで「思わず予約したくなる」魅力的な紹介文と、収益化のためのアフィリエイトリンクを自動生成します。
          </p>
        </div>

        <Card className="bg-[#13141f] border border-white/5 rounded-2xl overflow-hidden shadow-xl p-8 space-y-6">
          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic ml-1">Hotel / Area to Promote</label>
          <input className="w-full h-14 bg-black border-2 border-white/10 rounded-xl px-6 font-bold text-white outline-none focus:border-emerald-500 transition-all" placeholder="例：箱根の露天風呂付き客室がある高級旅館" />
          <Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full h-24 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-3xl rounded-[2rem] shadow-xl uppercase italic">
            {isAnalyzing ? <Loader2 className="animate-spin h-10 w-10" /> : '収益化紹介文を生成する 🚀'}
          </Button>
        </Card>
        ...
        {result && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <Card className="bg-emerald-500/5 border-2 border-emerald-500/30 rounded-[3.5rem] p-12 shadow-inner">
              <h3 className="text-2xl font-black text-white italic uppercase mb-8 flex items-center gap-3"><Zap className="text-emerald-400" /> AI Revenue Insight</h3>
            ...
            <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {[
                  { step: '01', title: '訴求点抽出', desc: '独自のAIスコアリングにより、そのホテルの「最も成約率が高い魅力」を特定。', icon: Search },
                  { step: '02', title: 'ターゲット配信', desc: '最適なハッシュタグと、読者の感情を揺さぶる「予約ボタン」の配置を指南。', icon: TrendingUp },
                  { step: '03', title: '報酬最大化', desc: '季節のキャンペーンやポイントアップ情報を加味し、クリック率を2倍にします。', icon: ShoppingBag },
                ].map((s, i) => (
            ...
            <a href="https://www.amazon.co.jp/s?k=ホテルアフィリエイト+副業+SNS活用&tag=nextralabs-22" target="_blank" className="block group">
              <div className="bg-gradient-to-r from-blue-600 to-indigo-800 p-10 rounded-[3rem] flex items-center justify-between shadow-2xl transition-all hover:scale-[1.01]">
                <div className="space-y-2">
                  <p className="text-[10px] font-black text-white/50 uppercase tracking-widest italic">Affiliate Mastery</p>
                  <h3 className="text-2xl font-black text-white italic leading-tight">「旅」を仕事に変える。最強のホテルアフィリエイト戦略。</h3>
                </div>
