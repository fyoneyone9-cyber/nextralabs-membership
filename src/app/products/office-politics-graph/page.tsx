      ...
      <section className="relative overflow-hidden py-16 md:py-24 border-b border-emerald-500/20">
        ...
        <div className="container mx-auto px-4 relative text-center max-w-5xl">
          ...
          <div className="space-y-8">
            <Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 px-3 py-1 font-bold tracking-tight uppercase">💎 MASTERMODEL</Badge>
            <h1 className="text-5xl md:text-8xl font-bold text-white tracking-tighter uppercase leading-none">OFFICE POLITICS AI</h1>
            <p className="text-xl md:text-2xl text-emerald-400 font-bold ">組織の歪みを可視化 × 不敗の戦略</p>
            ...
            <div className="flex justify-center gap-6 pt-6">
              <ToolLaunchButton productId="office-politics-graph" className="h-24 px-16 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-bold text-3xl rounded-[2rem] shadow-[0_0_50px_rgba(16,185,129,0.4)] transition-all uppercase active:scale-95" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 bg-black/40 border-b border-white/5">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="flex items-center justify-center gap-3 mb-10">
            ...
            <h2 className="text-2xl font-bold text-white uppercase">Nextra利用規約に基づく制限</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {[{ p: '無料', c: '会員登録必須', d: '体験利用' }, { p: 'ライト', c: '1回/日', d: '全ツール合計' }, { p: 'スタンダード', c: '2回/日', d: '全ツール合計' }, { p: 'プレミアム', c: '3回/日', d: '全ツール合計' }].map((item, i) => (
              <div key={i} className="bg-white/5 border border-white/10 p-6 rounded-2xl text-center">
                <p className="text-[10px] font-bold text-slate-500 uppercase mb-1">{item.p} Plan</p>
                <p className="text-xl font-bold text-white ">{item.c}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {features.map((f, i) => (
            <div key={i} className="bg-[#13141f] border border-white/5 p-10 rounded-[3rem] space-y-6 hover:border-emerald-500/50 transition-all shadow-xl">
              ...
              <p className="text-slate-400 font-bold text-sm leading-relaxed ">{f.description}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 py-20 text-center">
        <div className="bg-gradient-to-br from-emerald-600 to-teal-900 rounded-[4rem] p-12 md:p-24 shadow-2xl relative overflow-hidden">
          <div className="relative z-10 space-y-8 text-center flex flex-col items-center">
            <h2 className="text-4xl md:text-7xl font-bold text-white uppercase leading-none tracking-tighter">不敗の立ち回りを、<br/>今すぐAIと。</h2>
            <ToolLaunchButton productId="office-politics-graph" className="h-20 px-12 bg-white text-emerald-900 font-bold text-2xl rounded-2xl shadow-xl hover:scale-105 transition-all" />
          </div>
        </div>
      </section>
    </div>
