...
import { Zap, Loader2, CheckCircle2, TrendingUp, Search, Info, ShoppingCart, Repeat, Download, Video, ImageIcon, FileText, Lock, Copy, Scale, LineChart, Scissors, Sparkles, Trash2 } from 'lucide-react'
...
export default function ClosetCoachApp() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult("写真を解析しました。着用形跡のないトップスを処分候補としてリストアップしました。");
    ...
    <div className="min-h-screen bg-[#050507] text-slate-100 p-4 md:p-12 font-sans selection:bg-emerald-500/30 text-left">
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">クローゼットの写真やリストを入力してください。AIが断捨離プランを策定します。</p>
        </div>
        ...
        <Card className="bg-[#13141f] border border-white/10 rounded-2xl p-8 text-center space-y-6 shadow-xl relative">
          <div className="w-full h-48 bg-black/40 border-2 border-dashed border-white/10 rounded-2xl flex items-center justify-center cursor-pointer hover:border-emerald-500/50 transition-all">
             ...
             <p className="text-slate-500 font-bold uppercase italic">Upload Photo or Drop here</p>
          </div>
          <Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full h-20 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-2xl rounded-[2rem] shadow-xl uppercase italic active:scale-95">断捨離診断を実行 🚀</Button>
        </Card>
          ...
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {[{ title: '重複検知', desc: '似た服の過剰を可視化。', icon: Search }, { title: '一軍選定', desc: '今のあなたに似合う服を厳選。', icon: Sparkles }, { title: 'スペース解放', desc: '処分ステップを提示。', icon: Trash2 }].map((s, i) => (
            ...
            <a href="https://www.amazon.co.jp/s?k=断捨離+収納&tag=nextralabs-22" target="_blank" className="block group">
              <div className="bg-gradient-to-r from-blue-600 to-indigo-800 p-10 rounded-[3rem] flex items-center justify-between shadow-2xl transition-all">
                <h3 className="text-2xl font-black text-white italic">不敗の整理術：脳を軽くする究極の収納 ➔</h3>
