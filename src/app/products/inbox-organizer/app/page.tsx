      ...
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12 text-white font-black">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-emerald-500/20 pb-10">
          ...
          <div className="flex flex-col items-end gap-2">
            ...
            <ApiLinkIndicator model="Gmail API v1 / OAuth2" />
          </div>
        </div>
        ...
        <div className="space-y-8">
          <Button onClick={fetchEmails} disabled={loading} className="w-full h-24 bg-blue-600 hover:bg-blue-50 text-white font-black text-3xl rounded-[2rem] shadow-xl uppercase italic transition-all">
            {loading ? <Loader2 className="animate-spin h-10 w-10 mx-auto" /> : '最新メールを解析する 🚀'}
          </Button>

          <div className="grid grid-cols-4 gap-4">
            {quadrants.map(q => (
              <div key={q.id} className="bg-black/40 border border-white/5 p-4 rounded-2xl text-center shadow-inner">
          ...
          <div className="space-y-4 text-left text-white font-black">
            {emails.map((email) => (
              <Card key={email.id} className="bg-[#13141f] border border-white/5 rounded-2xl p-6 flex justify-between items-center group hover:border-emerald-500/50 transition-all shadow-xl">
                <div>
                  <p className="text-[10px] font-black text-blue-400 uppercase italic tracking-widest">{email.from}</p>
