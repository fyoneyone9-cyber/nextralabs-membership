...
export default function BuzzWriterApp() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult("文章を解析しました。共感・反論・有益のバランスを最適化し、読了率を最大化する構成へ添削しました。");
    ...
    <div className="min-h-screen bg-[#050507] text-slate-100 p-4 md:p-12 font-sans selection:bg-emerald-500/30 text-left text-white font-black">
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">投稿予定の文章を入力してください。AIが心理トリガーを分析し、数千人の反応を引き出す文章へ添削します。</p>
        </div>
        ...
        <Card className="bg-[#13141f] border border-white/5 rounded-2xl p-8 space-y-6 shadow-xl">
          ...
          <Button onClick={handleAnalyze} disabled={isAnalyzing || !text} className="w-full h-24 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-3xl rounded-[2rem] shadow-xl uppercase italic">バズり文章へ変換 🚀</Button>
        </Card>
        ...
        {result && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <Card className="bg-emerald-500/5 border-2 border-emerald-500/30 rounded-[3.5rem] p-12 shadow-inner text-left">
              ...
              <Button onClick={() => alert('コピー完了')} className="h-16 px-10 bg-white text-slate-950 font-black text-lg rounded-xl shadow-xl hover:bg-slate-100 transition-all italic">結果をコピー</Button>
            </Card>
            ...
            <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {[{ title: 'フック強化', desc: '最初の一行を再構築。', icon: Search }, { title: 'リズム調整', desc: '改行位置を最適化。', icon: Sparkles }, { title: '拡散戦術', desc: '最適な投稿時間を提示。', icon: TrendingUp }].map((s, i) => (
