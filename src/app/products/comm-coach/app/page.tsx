...
import { 
  ...
  MessageCircle, MessageSquare, Heart, ShieldAlert, Sparkles, Languages
} from 'lucide-react'
...
export default function CommCoachApp() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult("会話ログを解析しました。相手の『沈黙』に対して焦りを感じ、質問を畳み掛けている傾向があります。3秒待ってから共感の相槌を打つだけで、相手の自己開示が大幅に増える可能性があります。");
      ...
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-emerald-500/20 pb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl md:text-5xl font-black italic uppercase tracking-tighter text-white">Comm Coach AI</h1>
              <p className="text-emerald-400 font-bold uppercase tracking-[0.2em] text-[10px] italic">Strategic Interpersonal Intelligence</p>
            </div>
          </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">
            苦手な相手とのやり取り、デートの振り返り、または商談のログを入力してください。AIが相手の言葉の裏にある「隠れた意図」を解析し、信頼を勝ち取るための最適な返信や相槌、立ち回り方法をアドバイスします。
          </p>
        </div>

        <Card className="bg-[#13141f] border border-white/5 rounded-2xl overflow-hidden shadow-xl p-8 space-y-6">
          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic ml-1">Conversation Log / Target Response</label>
          <textarea className="w-full h-40 bg-black border-2 border-white/10 rounded-xl p-6 font-bold text-white outline-none focus:border-emerald-500 transition-all" placeholder="例：上司から『最近どう？』と聞かれたが、本当は何を期待されているのか知りたい。" />
          <Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full h-24 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-3xl rounded-[2rem] shadow-xl uppercase italic">
            {isAnalyzing ? <Loader2 className="animate-spin h-10 w-10" /> : 'コミュニケーションを解析する 🚀'}
          </Button>
        </Card>
        ...
        {result && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <Card className="bg-emerald-500/5 border-2 border-emerald-500/30 rounded-[3.5rem] p-12 shadow-inner">
              <h3 className="text-2xl font-black text-white italic uppercase mb-8 flex items-center gap-3"><Zap className="text-emerald-400" /> AI Interpersonal Insight</h3>
              <div className="text-xl text-white font-bold italic leading-loose whitespace-pre-wrap">{result}</div>
            </Card>

            <div className="space-y-6">
              <h3 className="text-xl font-black text-white italic uppercase tracking-widest border-l-4 border-emerald-500 pl-4">Connection Roadmap</h3>
              <div className="grid md:grid-cols-3 gap-6">
                {[
                  { step: '01', title: '深層心理抽出', desc: '相手の言葉のトーンと頻出語から、真の心理状態と欲求をAIが特定。', icon: Search },
                  { step: '02', title: '最適レスポンス', desc: '相手が最も心地よいと感じる返信の「速さ・長さ・熱量」を策定。', icon: Sparkles },
                  { step: '03', title: '関係掌握', desc: '長期的に有利なポジションを築くための、心理的リード方法を指南。', icon: TrendingUp },
                ].map((s, i) => (
            ...
            <a href="https://www.amazon.co.jp/s?k=話し方+聞き方+心理学&tag=nextralabs-22" target="_blank" className="block group">
              <div className="bg-gradient-to-r from-blue-600 to-indigo-800 p-10 rounded-[3rem] flex items-center justify-between shadow-2xl transition-all hover:scale-[1.01]">
                <div className="space-y-2">
                  <p className="text-[10px] font-black text-white/50 uppercase tracking-widest italic">Social Intelligence</p>
                  <h3 className="text-2xl font-black text-white italic leading-tight">「言葉」で人を惹きつける。不敗のコミュニケーション術。</h3>
                </div>
