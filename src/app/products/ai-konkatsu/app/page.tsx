...
import { 
  ...
  Table, ListChecks, FileText, Database, ShieldCheck, Heart, UserCircle, Briefcase, FileSearch
} from 'lucide-react'
...
export default function AiKonkatsuApp() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult("お相手とのLINE履歴を解析しました。相手は現在『安心感』を求めており、少し返信を遅らせて大人の余裕を醸し出すのが最適です。次回のデートで話すべき3つの話題を提案しました。");
      ...
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-emerald-500/20 pb-10">
          <div className="flex items-center gap-4">
            <div className="p-4 bg-emerald-500/10 rounded-2xl border border-emerald-500/20"><Heart className="h-10 w-10 text-emerald-400" /></div>
            <div>
              <h1 className="text-3xl md:text-5xl font-black italic uppercase tracking-tighter text-white">AI Konkatsu Coach</h1>
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">
            お相手のプロフィールや、最近のLINEのやり取りを入力してください。上級心理カウンセラーの知見を学習したAIが、相手の「深層心理」と「あなたが選ばれるための戦略」を策定。成婚までの最短ルートをナビゲートします。
          </p>
        </div>

        <Card className="bg-[#13141f] border border-white/5 rounded-2xl overflow-hidden shadow-xl p-8 space-y-6">
          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic ml-1">Target Profile / Chat Log</label>
          <textarea className="w-full h-40 bg-black border-2 border-white/10 rounded-xl p-6 font-bold text-white outline-none focus:border-emerald-500 transition-all" placeholder="例：30代女性、公務員、趣味は読書。最近LINEの返信が少し遅くなってきた気がする。" />
          <Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full h-24 bg-emerald-600 hover:bg-emerald-50 text-slate-950 font-black text-3xl rounded-[2rem] shadow-xl uppercase italic">
            {isAnalyzing ? <Loader2 className="animate-spin h-10 w-10" /> : 'AIコーチングを開始する 🚀'}
          </Button>
        </Card>
          ...
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {[
                  { step: '01', title: '深層心理抽出', desc: '相手の言葉の裏にある「本音」と「欲求」をAIが特定します。', icon: Search },
                  { step: '02', title: '最適レスポンス', desc: '好感度を最大化するための返信速度、内容、トーンをプランニング。', icon: Heart },
                  { step: '03', title: '成婚クロージング', desc: '次のステップ（真剣交際等）へ繋げるための具体的アクション。', icon: TrendingUp },
                ].map((s, i) => (
            ...
            <a href="https://www.marriage-road.jp/" target="_blank" className="block group">
              <div className="bg-gradient-to-r from-rose-600 to-pink-800 p-10 rounded-[3rem] flex items-center justify-between shadow-2xl transition-all hover:scale-[1.01]">
                <div className="space-y-2">
                  <p className="text-[10px] font-black text-white/50 uppercase tracking-widest italic">Professional Support</p>
                  <h3 className="text-2xl font-black text-white italic leading-tight">AIを超えた「本物」の成婚サポート。マリッジロードジャパン ➔</h3>
                </div>
