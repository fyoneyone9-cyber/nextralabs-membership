...
import { 
  ...
  BarChart2, LineChart, Cpu, Rocket
} from 'lucide-react'
...
export default function PriceTrackerApp() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult("対象商品の価格推移をAIが解析しました。現在、過去3ヶ月で最安値に近い水準にあります。来週以降、在庫減少に伴い15%程度の値上がりが予想されるため、今が『買い』のタイミングです。");
      ...
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-emerald-500/20 pb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl md:text-5xl font-black italic uppercase tracking-tighter text-white">Price Tracker AI</h1>
              <p className="text-emerald-400 font-bold uppercase tracking-[0.2em] text-[10px] italic">Market Analysis & Bottom Price Prediction</p>
            </div>
          </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">
            監視したい商品のURLまたは型番を入力してください。AIが主要ECサイトの価格変動履歴を巡回・解析し、「今が底値か」「待つべきか」をデータに基づき冷徹にジャッジ。最も損をしない購入タイミングを提示します。
          </p>
        </div>

        <Card className="bg-[#13141f] border border-white/5 rounded-2xl overflow-hidden shadow-xl p-8 space-y-6">
          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic ml-1">Product URL / Model Number</label>
          <input className="w-full h-14 bg-black border-2 border-white/10 rounded-xl px-6 font-bold text-white outline-none focus:border-emerald-500 transition-all" placeholder="例：iPhone 15 Pro 256GB ブラック" />
          <Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full h-24 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-3xl rounded-[2rem] shadow-xl uppercase italic">
            {isAnalyzing ? <Loader2 className="animate-spin h-10 w-10" /> : '底値予測を実行 🚀'}
          </Button>
        </Card>
        ...
        {result && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <Card className="bg-emerald-500/5 border-2 border-emerald-500/30 rounded-[3.5rem] p-12 shadow-inner">
              <h3 className="text-2xl font-black text-white italic uppercase mb-8 flex items-center gap-3"><Zap className="text-emerald-400" /> AI Pricing Analysis</h3>
              <div className="text-xl text-white font-bold italic leading-loose whitespace-pre-wrap">{result}</div>
            </Card>

            <div className="space-y-6">
              <h3 className="text-xl font-black text-white italic uppercase tracking-widest border-l-4 border-emerald-500 pl-4">Buy/Wait Roadmap</h3>
              <div className="grid md:grid-cols-3 gap-6">
                {[
                  { step: '01', title: '履歴解析', desc: '過去1年の価格変動パターンから、セールの周期性を特定します。', icon: Search },
                  { step: '02', title: '未来予測', desc: '新モデル発表時期や在庫動向を加味し、2週間後の価格を推定。', icon: LineChart },
                  { step: '03', title: '実行指令', desc: '「今すぐ購入」または「待機」の最終判断と、予約注文のアドバイス。', icon: TrendingUp },
                ].map((s, i) => (
            ...
            <a href="https://www.amazon.co.jp/s?k=お得な買い物+節約術&tag=nextralabs-22" target="_blank" className="block group">
              <div className="bg-gradient-to-r from-blue-600 to-indigo-800 p-10 rounded-[3rem] flex items-center justify-between shadow-2xl transition-all hover:scale-[1.01]">
                <div className="space-y-2">
                  <p className="text-[10px] font-black text-white/50 uppercase tracking-widest italic">Smart Shopping</p>
                  <h3 className="text-2xl font-black text-white italic leading-tight">賢く買い、資産を残す。Nextra Labs推奨の買い物術。</h3>
                </div>
