...
import { Zap, Loader2, CheckCircle2, TrendingUp, Search, Info, ShoppingCart, Repeat, Brain, ListChecks, FileText, FileDown } from 'lucide-react'

const PRESETS = [
  { id: 'itpass', label: 'ITパスポート', content: 'ITパスポート試験形式で問題を生成してください。' },
  { id: 'fe', label: '基本情報', content: '基本情報技術者試験形式で問題を生成してください。' }
];
...
export default function AiExamGeneratorApp() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult("予想問題を30問生成しました。Googleドキュメントへ出力可能です。");
    ...
    <div className="min-h-screen bg-[#050507] text-slate-100 p-4 md:p-12 font-sans selection:bg-emerald-500/30 text-left">
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">学習したい内容を入力、または雛形を選択してください。AIが問題を生成し、Googleドキュメントへ書き出し可能です。</p>
        </div>

        <div className="grid grid-cols-2 gap-4">
          {PRESETS.map(p => (
            <Button key={p.id} onClick={() => setInputData(p.content)} variant="outline" className="h-16 border-2 border-white/5 bg-[#13141f] text-white font-black italic hover:border-emerald-500">{p.label}形式</Button>
          ))}
        </div>
        ...
        <Card className="bg-[#13141f] border border-white/5 rounded-2xl p-8 space-y-6">
          ...
          <Button onClick={handleAnalyze} disabled={isAnalyzing || !inputData} className="w-full h-20 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-2xl rounded-2xl shadow-xl uppercase italic">
        ...
        {result && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <Card className="bg-emerald-500/5 border-2 border-emerald-500/30 rounded-[3.5rem] p-12 shadow-inner text-left">
              ...
              {docUrl && docUrl !== '#' && <Button onClick={() => window.open(docUrl)} className="ml-4 h-16 px-10 bg-blue-600 text-white font-black text-lg rounded-xl shadow-xl hover:bg-blue-500 transition-all italic"><FileDown className="mr-2" /> 開く</Button>}
            </Card>
            ...
            <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {[{ title: '弱点特定', desc: '合格を妨げている「穴」を特定。', icon: Search }, { title: '特訓問題', desc: '予想問題を自動生成。', icon: ListChecks }, { title: '克服・合格', desc: '正答率100%を目指します。', icon: TrendingUp }].map((s, i) => (
            ...
            <a href="https://www.amazon.co.jp/s?k=資格試験+参考書&tag=nextralabs-22" target="_blank" className="block group text-left">
