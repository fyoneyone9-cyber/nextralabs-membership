...
import { 
  ...
  Repeat, Brain, ListChecks, ShieldAlert, FileText, FileDown, 
  ClipboardPaste, RefreshCw, Smartphone
} from 'lucide-react'

// 試験別・実物過去問データに基づくプリセット（完全復旧）
const PRESETS = [
...
export default function AiExamGeneratorApp() {
  ...
  const handleAnalyze = async () => {
    if (!inputData) return;
    setIsAnalyzing(true);
    // 憲法遵守：gsk-google-docs連携を含む実務ロジックを再接続
  ...
  const copyPrompt = () => {
    ...
    alert('最強の学習指示（プロンプト）をコピーしました。外部AIへ貼り付けて使用してください。');
  };
    ...
    <div className="min-h-screen bg-[#050507] text-slate-100 p-4 md:p-12 font-sans selection:bg-emerald-500/30 text-left">
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-emerald-500/20 pb-10">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-emerald-400 font-bold uppercase tracking-[0.3em] text-[10px] italic mt-2">Nextra Learning Intelligence Engine</p>
            </div>
          </div>
          <Badge className="bg-emerald-500 text-slate-950 font-black italic px-8 py-2 text-sm rounded-full shadow-lg">MASTER PLAN</Badge>
        </div>
        ...
        <div className="space-y-4">
          <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic ml-4">Exam Target Presets</p>
        ...
        {result && (
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700 text-left">
            <Card className="bg-emerald-500/5 border-2 border-emerald-500/30 rounded-[3.5rem] p-16 shadow-inner relative overflow-hidden text-left text-white">
              ...
              <div className="border-t border-emerald-500/20 pt-10 text-center">
                {!docUrl ? (
                  <Button 
                    ...
                    className="h-24 px-16 bg-white text-slate-950 font-black text-2xl rounded-[2rem] shadow-xl hover:bg-slate-100 transition-all uppercase italic flex items-center gap-4 mx-auto"
                  >
                  ...
                  <Button 
                    ...
                    className="h-24 px-16 bg-blue-600 text-white font-black text-2xl rounded-[2rem] shadow-xl hover:bg-blue-500 transition-all uppercase italic flex items-center gap-4 mx-auto"
                  >
            ...
            <div className="space-y-8">
              ...
              <div className="grid md:grid-cols-3 gap-8">
                {[{ title: '弱点特定', desc: '過去問実データから「知識の穴」をAIが可視化。', icon: Search }, { title: '特訓問題', desc: '弱点に特化した実物同様の問題を自動生成。', icon: ListChecks }, { title: '合格圏内', desc: 'AIの解説で正答率100%を維持し本番へ。', icon: TrendingUp }].map((s, i) => (
            ...
            {/* Amazon収益化 */}
            <a href="https://www.amazon.co.jp/s?k=ITパスポート+過去問+参考書&tag=nextralabs-22" target="_blank" className="block group">
              <div className="bg-gradient-to-r from-emerald-600 to-teal-900 p-16 rounded-[4rem] flex items-center justify-between shadow-2xl transition-all hover:scale-[1.01] relative overflow-hidden">
                ...
                <div className="space-y-4 text-left relative z-10">
                  ...
                  <h3 className="text-4xl md:text-6xl font-black text-white italic leading-tight text-left">AI厳選：最短合格を叶える最強参考書。 ➔</h3>
                </div>
