...
import { 
  ...
  Ticket, Calendar, Bell, Globe, Rocket
} from 'lucide-react'
...
export default function TicketScoutApp() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult("対象の公演情報を解析しました。公式先着の他に、提携クレカ枠とファンクラブ2次先行が来週火曜に開始されます。AIが推奨する『最速予約スケジュール』を作成しました。");
      ...
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-emerald-500/20 pb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl md:text-5xl font-black italic uppercase tracking-tighter text-white">AI Ticket Scout</h1>
              <p className="text-emerald-400 font-bold uppercase tracking-[0.2em] text-[10px] italic">Strategic Ticket Acquisition Engine</p>
            </div>
          </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">
            入手したい公演名やアーティスト名、URLを入力してください。AIが全世界の販売サイトや先行予約枠を巡回し、最も当選確率が高い、または早く入手できる「攻略ルート」を特定。あなたのカレンダーへ自動同期するための情報を生成します。
          </p>
        </div>

        <Card className="bg-[#13141f] border border-white/5 rounded-2xl overflow-hidden shadow-xl p-8 space-y-6">
          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic ml-1">Event / Artist / Venue</label>
          <input className="w-full h-14 bg-black border-2 border-white/10 rounded-xl px-6 font-bold text-white outline-none focus:border-emerald-500 transition-all" placeholder="例：日本武道館 公演名、10月開催" />
          <Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full h-24 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-3xl rounded-[2rem] shadow-xl uppercase italic">
            {isAnalyzing ? <Loader2 className="animate-spin h-10 w-10" /> : 'チケットをスカウトする 🚀'}
          </Button>
        </Card>
        ...
        {result && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <Card className="bg-emerald-500/5 border-2 border-emerald-500/30 rounded-[3.5rem] p-12 shadow-inner">
              <h3 className="text-2xl font-black text-white italic uppercase mb-8 flex items-center gap-3"><Zap className="text-emerald-400" /> AI Scouting Insight</h3>
              <div className="text-xl text-white font-bold italic leading-loose whitespace-pre-wrap">{result}</div>
            </Card>

            <div className="space-y-6">
              <h3 className="text-xl font-black text-white italic uppercase tracking-widest border-l-4 border-emerald-500 pl-4">Acquisition Roadmap</h3>
              <div className="grid md:grid-cols-3 gap-6">
                {[
                  { step: '01', title: '全枠特定', desc: 'FC先行、クレカ枠、プレイガイド先行など、全ルートを網羅。', icon: Search },
                  { step: '02', title: '確率最大化', desc: '過去の倍率データから、最も当選しやすい日時と席種をリコメンド。', icon: TrendingUp },
                  { step: '03', title: '自動リマインド', desc: '申込み開始15分前にAIが通知。争奪戦への準備を完璧に整えます。', icon: Bell },
                ].map((s, i) => (
            ...
            <a href="https://www.amazon.co.jp/s?k=ライブ+双眼鏡+便利グッズ&tag=nextralabs-22" target="_blank" className="block group">
              <div className="bg-gradient-to-r from-emerald-600 to-teal-800 p-10 rounded-[3rem] flex items-center justify-between shadow-2xl transition-all hover:scale-[1.01]">
                <div className="space-y-2">
                  <p className="text-[10px] font-black text-white/50 uppercase tracking-widest italic">Event Mastery</p>
                  <h3 className="text-2xl font-black text-white italic leading-tight">「最高の一瞬」を逃さない。Scout推奨のライブ参戦グッズ。</h3>
                </div>
