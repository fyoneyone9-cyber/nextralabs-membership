...
import { 
  ...
  Clapperboard, Play, Youtube, FileText, Sparkles
} from 'lucide-react'
...
export default function YoutubeProducerApp() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult("入力されたテーマから、視聴維持率を最大化する『起承転結』の台本を生成しました。10分動画用の構成案と、バズるサムネイルの指示書も付属しています。");
      ...
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-emerald-500/20 pb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl md:text-5xl font-black italic uppercase tracking-tighter text-white">YouTube Producer</h1>
              <p className="text-emerald-400 font-bold uppercase tracking-[0.2em] text-[10px] italic">Strategic Video Content Synthesis</p>
            </div>
          </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">
            制作したい動画のテーマ、ターゲット、または元となる記事を入力してください。AIがYouTubeのアルゴリズムに愛される「台本・構成・タイトル・タグ」をセットで自動生成。企画から台本完成までの時間を大幅に短縮します。
          </p>
        </div>
        ...
        <Card className="bg-[#13141f] border border-white/5 rounded-2xl overflow-hidden shadow-xl p-8 space-y-6">
          ...
          <textarea className="w-full h-48 bg-black border-2 border-white/10 rounded-xl p-6 font-bold text-white outline-none focus:border-emerald-500 transition-all" placeholder="例：最新のAIツールのレビュー動画。20代のクリエイター向けに。5分程度の尺。" />
          <Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full h-24 bg-emerald-600 hover:bg-emerald-50 text-slate-950 font-black text-3xl rounded-[2rem] shadow-[0_0_50px_rgba(16,185,129,0.4)] transition-all uppercase italic">
            {isAnalyzing ? <Loader2 className="animate-spin h-10 w-10" /> : '動画台本を自動生成する 🚀'}
          </Button>
        </Card>
          ...
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {[
                  { step: '01', title: '市場調査', desc: '同ジャンルで最も再生されている動画を解析し、勝てる角度を特定。', icon: Search },
                  { step: '02', title: '台本構成', desc: '離脱ポイントを排除し、平均視聴維持率50%以上を狙う構成を策定。', icon: FileText },
                  { step: '03', title: 'バズ施策', desc: 'クリックを誘発するサムネイル指示書と、検索に強いタグを出力。', icon: Sparkles },
                ].map((s, i) => (
            ...
            <a href="https://www.amazon.co.jp/s?k=YouTube+動画編集+機材&tag=nextralabs-22" target="_blank" className="block group">
              <div className="bg-gradient-to-r from-red-600 to-amber-800 p-10 rounded-[3rem] flex items-center justify-between shadow-2xl transition-all hover:scale-[1.01]">
                <div className="space-y-2">
                  <p className="text-[10px] font-black text-white/50 uppercase tracking-widest italic">Creator Essentials</p>
                  <h3 className="text-2xl font-black text-white italic leading-tight">「銀の盾」への最短距離。プロ推奨の動画制作機材。</h3>
                </div>
