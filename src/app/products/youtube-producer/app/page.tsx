      ...
      <div className="max-w-6xl mx-auto space-y-12 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-16 relative overflow-hidden bg-black/40 backdrop-blur-xl">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-12">
          ...
          <TabsContent value="script" className="space-y-8 animate-in fade-in">
            {script && (<div className="space-y-8 text-center"><div className="flex flex-col md:flex-row md:items-center justify-between gap-8 border-b border-white/10 pb-10"><h3 className="text-4xl font-black text-white italic uppercase flex items-center gap-4 tracking-tighter"><Sparkles className="text-emerald-400 h-10 w-10" /> 生成された黄金台本</h3><div className="flex gap-6"><div className="bg-emerald-500/10 border-2 border-emerald-500/30 px-10 py-6 rounded-[2rem] text-center shadow-lg"><p className="text-xs font-black text-emerald-500 uppercase tracking-[0.2em] italic mb-2">バズ予測スコア</p><p className="text-6xl font-black text-white italic tracking-tighter">{script.viralScore}%</p></div><div className="bg-white/5 border-2 border-white/10 px-10 py-6 rounded-[2rem] text-center flex flex-col justify-center shadow-lg"><p className="text-xs font-black text-white/30 uppercase tracking-[0.2em] italic mb-2">再生時間</p><p className="text-4xl font-black text-white italic tracking-tighter">{script.estimatedMinutes}分</p></div></div></div><div className="grid gap-10 text-left">{[{ t: '導入 (オープニング)', c: script.opening, col: 'from-emerald-500/20' }, { t: '本編 (ボディー)', c: script.body, col: 'from-blue-500/10' }, { t: '結末 (エンディング)', c: script.closing, col: 'from-purple-500/10' }].map((s, i) => (<Card key={i} className={`bg-gradient-to-br ${s.col} to-transparent border-2 border-white/10 rounded-[3rem] p-12 shadow-2xl`}><h4 className="text-xl font-black text-emerald-400 uppercase tracking-[0.3em] italic mb-8 border-l-8 border-emerald-500 pl-6">{s.t}</h4><p className="text-2xl text-white font-bold italic leading-[2] whitespace-pre-wrap">{s.c}</p></Card>))}</div><div className="flex justify-center pt-10"><Button onClick={generateExtras} disabled={isProcessing['characters']} className="h-32 px-24 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-4xl rounded-[3rem] shadow-[0_20px_50px_rgba(16,185,129,0.4)] uppercase italic group scale-105 active:scale-95 transition-all"><span className="flex items-center gap-4">ステップ 4: 全データを一括出力 <ChevronRight className="h-10 w-10 group-hover:translate-x-2 transition-transform" /></span></Button></div></div>)}
          </TabsContent>
          ...
          <TabsContent value="music" className="space-y-12 animate-in fade-in">
            <div className="bg-indigo-500/5 border-4 border-indigo-500/20 rounded-[3rem] p-12 space-y-10 text-center shadow-2xl">
