      ...
      <div className="max-w-6xl mx-auto space-y-12 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-16 relative overflow-hidden bg-black/40 backdrop-blur-xl">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-12">
          ...
          <TabsContent value="genre" className="space-y-8 animate-in fade-in">
             <div className="space-y-12">
               ...
               <div className="border-t-4 border-white/5 pt-12 space-y-6">
                 ...
                 <div className="grid md:grid-cols-5 gap-4">{IMAGE_STYLES.map((s) => (<button key={s.id} onClick={() => setImageStyle(s.id)} className={`p-4 rounded-2xl font-black italic text-center border-4 transition-all ${imageStyle === s.id ? 'bg-emerald-500 border-emerald-400 text-slate-950 scale-105 shadow-[0_0_20px_rgba(52,211,153,0.3)]' : 'bg-white/5 border-white/5 text-slate-400 hover:border-white/20'}`}><span className="text-sm">{s.label}</span></button>))}</div>
               </div>
               <div className="border-t-4 border-white/5 pt-12 space-y-6">
          ...
          <TabsContent value="script" className="space-y-8 animate-in fade-in">
            {script && (<div className="space-y-8 text-center"><div className="flex flex-col md:flex-row md:items-center justify-between gap-8 border-b border-white/10 pb-10"><h3 className="text-4xl font-black text-white italic uppercase flex items-center gap-4 tracking-tighter"><Sparkles className="text-emerald-400 h-10 w-10" /> 生成された黄金台本</h3><div className="flex gap-6"><div className="bg-emerald-500/10 border-2 border-emerald-500/30 px-10 py-6 rounded-[2rem] text-center shadow-lg"><p className="text-xs font-black text-emerald-500 uppercase tracking-widest italic mb-2">バズ予測スコア</p><p className="text-6xl font-black text-white italic tracking-tighter">{script.viralScore}%</p></div><div className="bg-white/5 border-2 border-white/10 px-10 py-6 rounded-[2rem] text-center flex flex-col justify-center shadow-lg"><p className="text-xs font-black text-white/30 uppercase tracking-widest italic mb-2">再生時間</p><p className="text-4xl font-black text-white italic tracking-tighter">{script.estimatedMinutes}分</p></div></div></div><div className="grid gap-10 text-left">{[{ t: '導入', c: script.opening, col: 'from-emerald-500/20' }, { t: '本編', c: script.body, col: 'from-blue-500/10' }, { t: '結末', c: script.closing, col: 'from-purple-500/10' }].map((s, i) => (<Card key={i} className={`bg-gradient-to-br ${s.col} to-transparent border-2 border-white/10 rounded-[3rem] p-12 shadow-2xl`}><h4 className="text-xl font-black text-emerald-400 uppercase tracking-[0.3em] italic mb-8 border-l-8 border-emerald-500 pl-6">{s.t}</h4><p className="text-2xl text-white font-bold italic leading-[2] whitespace-pre-wrap">{s.c}</p></Card>))}</div><div className="flex justify-center pt-10"><Button onClick={generateExtras} disabled={isProcessing['characters']} className="h-32 px-24 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-4xl rounded-[3rem] shadow-[0_20px_50px_rgba(16,185,129,0.4)] uppercase italic group scale-105 active:scale-95 transition-all"><span className="flex items-center gap-4">ステップ 4: 全データを一括出力 <ChevronRight className="h-10 w-10 group-hover:translate-x-2 transition-transform" /></span></Button></div></div>)}
          </TabsContent>
          ...
          <TabsContent value="visual" className="space-y-12 animate-in fade-in">
            ...
            <div className="grid md:grid-cols-2 gap-16">
              <div className="space-y-10"><div className="flex items-center justify-between border-b-4 border-emerald-500/20 pb-6"><h3 className="text-xl font-black text-white italic uppercase flex items-center gap-3"><Users className="text-emerald-400 h-8 w-8" /> 登場人物プロンプト</h3><Button onClick={() => { const p = "必ず画像を制作して下さい\n名前を出力して下さい\n\n【キャラクター設定】\n"; const txt = characters?.map(c => `${c.name}: ${c.imagePrompt}`).join('\n\n'); navigator.clipboard.writeText(p + (txt || '')); alert("指示文と全プロンプトを一括コピーしました！"); }} className="h-10 px-6 bg-emerald-500 text-slate-950 font-black italic rounded-xl text-xs shadow-lg hover:bg-emerald-400 transition-all active:scale-95">一括コピー 📋</Button></div><div className="space-y-8">{characters?.map((char, i) => (<Card key={i} className="bg-white/5 border-2 border-white/10 rounded-[2.5rem] p-10 space-y-6 shadow-xl"><div className="flex justify-between items-center"><h4 className="font-black text-emerald-400 italic text-2xl">{char.name}</h4><Button onClick={() => { const p = "必ず画像を制作して下さい\n名前を出力して下さい\n\n"; navigator.clipboard.writeText(p + char.imagePrompt); alert("指示文とプロンプトをコピーしました！"); }} className="h-10 bg-white/10 text-white border-2 border-white/10 hover:bg-white/20 font-black italic text-xs rounded-xl">個別コピー</Button></div><p className="text-sm text-emerald-500/60 font-mono break-all leading-relaxed bg-black/40 p-6 rounded-2xl border border-white/5 italic">{char.imagePrompt}</p></Card>))}</div></div>
              <div className="space-y-10"><h3 className="text-xl font-black text-white italic uppercase flex items-center gap-3 border-b-4 border-emerald-500/20 pb-6"><ImageIcon className="text-emerald-400 h-8 w-8" /> サムネイル構成案</h3><div className="space-y-8">{thumbnails?.map((thumb, i) => (<Card key={i} className="bg-white/5 border-2 border-white/10 rounded-[2.5rem] p-10 space-y-6 shadow-xl relative overflow-hidden"><div className="flex items-center justify-between gap-4"><Badge className="bg-red-600 text-white border-0 font-black italic text-sm px-4 py-1 rounded-lg">案 {i+1}</Badge><Button onClick={() => { const p = withLogo ? "YOUTUBEのサムネイル用なので映える構成にして下さい。 もちろん映えるロゴ付き\n\n" : "YOUTUBEのサムネイル素材用なので、ロゴや文字は一切入れずに最高品質の背景・人物画像のみを作成して下さい。\n\n"; navigator.clipboard.writeText(p + thumb.imagePrompt); alert("指示文とサムネイルプロンプトをコピーしました！"); }} className="h-10 bg-white/10 text-white border-2 border-white/10 hover:bg-white/20 font-black italic text-xs rounded-xl shrink-0">コピー</Button></div><h4 className="text-lg font-black text-white italic border-l-4 border-red-500 pl-4">{thumb.title}</h4><p className="text-sm text-slate-400 font-mono italic leading-relaxed bg-black/40 p-6 rounded-2xl border border-white/5">{thumb.imagePrompt}</p></Card>))}</div></div>
            </div>
