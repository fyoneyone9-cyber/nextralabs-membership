            ...
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-emerald-500/20 pb-3">
                  <h3 className="text-sm font-semibold text-white flex items-center gap-2"><Users className="text-emerald-400 h-4 w-4" /> 登場人物プロンプト</h3>
                  <Button onClick={() => { const p = "必ず画像を制作して下さい\n名前を出力して下さい\n\n【キャラクター設定】\n"; const txt = characters?.map(c => `${c.name}: ${c.imagePrompt}`).join('\n\n'); navigator.clipboard.writeText(p + (txt || '')); alert("一括コピーしました！"); }} className="h-8 px-3 bg-emerald-500 text-white font-medium rounded-lg text-xs hover:bg-emerald-400">一括コピー 📋</Button>
                </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-emerald-500/20 pb-3">
                  <h3 className="text-sm font-semibold text-white flex items-center gap-2"><ImageIcon className="text-emerald-400 h-4 w-4" /> サムネイル構成案</h3>
                  <Button onClick={() => { const txt = thumbnails?.map((t, i) => `【案${i+1}】${t.title}\n${t.imagePrompt}`).join('\n\n'); navigator.clipboard.writeText(txt || ''); alert("一括コピーしました！"); }} className="h-8 px-3 bg-emerald-500 text-white font-medium rounded-lg text-xs hover:bg-emerald-400">一括コピー 📋</Button>
                </div>
                <div className="space-y-3">{thumbnails?.map((thumb, i) => (<Card key={i} className="bg-white/5 border border-white/10 rounded-xl p-4 space-y-3"><div className="flex items-center justify-between gap-3"><Badge className="bg-red-600 text-white border-0 font-medium text-xs px-2 py-0.5 rounded">案 {i+1}</Badge><div className="flex gap-2"><Button onClick={() => { const p = withLogo ? "YOUTUBEのサムネイル用なので映える構成にして下さい。もちろん映えるロゴ付き\n\n" : "YOUTUBEのサムネイル素材用なので、ロゴや文字は一切入れずに最高品質の背景・人物画像のみを作成して下さい。\n\n"; navigator.clipboard.writeText(p + thumb.imagePrompt); alert("コピーしました！"); }} className="h-7 bg-white/10 text-white border border-white/10 hover:bg-white/20 font-medium text-xs rounded-lg px-3 shrink-0">コピー</Button><Button onClick={() => setThumbnails(prev => prev?.filter((_, idx) => idx !== i) ?? null)} className="h-7 bg-red-500/20 text-red-400 border border-red-500/30 hover:bg-red-500/40 font-medium text-xs rounded-lg px-3">削除</Button></div></div><h4 className="text-sm font-semibold text-white border-l-2 border-red-500 pl-3">{thumb.title}</h4><p className="text-xs text-slate-400 font-mono leading-relaxed bg-black/40 p-3 rounded-lg border border-white/5">{thumb.imagePrompt}</p></Card>))}</div>
              </div>
            </div>
