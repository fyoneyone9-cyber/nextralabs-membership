          ...
          <TabsContent value="genre" className="space-y-6 animate-in fade-in">
            <div className="space-y-6">
              ...
              <div className="border-t border-white/5 pt-5 space-y-3">
                ...
                <div className="grid md:grid-cols-5 gap-2">{IMAGE_STYLES.map((s) => (<button key={s.id} onClick={() => setImageStyle(s.id)} className={`p-3 rounded-lg font-medium text-center border transition-all ${imageStyle === s.id ? 'bg-emerald-500 border-emerald-400 text-slate-950' : 'bg-white/5 border-white/10 text-slate-400 hover:border-white/20'}`}><span className="text-xs">{s.label}</span></button>))}</div>
              </div>
              <div className="border-t border-white/5 pt-5 space-y-3">
                ...
                <div className="grid md:grid-cols-2 gap-3">
                  {[{ id: true, label: '映えるロゴ入り', desc: '目を引くデザインフォントやロゴを自動構成します。' }, { id: false, label: 'ロゴなし（素材のみ）', desc: '文字を入れず、最高品質の背景・人物画像のみを生成します。' }].map((l) => (
                    <button key={l.id ? 'y' : 'n'} onClick={() => setWithLogo(l.id)} className={`p-4 rounded-xl font-medium text-left border transition-all ${withLogo === l.id ? 'bg-emerald-500 border-emerald-400 text-slate-950' : 'bg-white/5 border-white/10 text-slate-400 hover:border-white/20'}`}><div className="flex items-center justify-between mb-1"><span className="text-sm font-semibold">{l.label}</span>{withLogo === l.id && <CheckCircle2 size={16} />}</div><p className="text-xs opacity-70">{l.desc}</p></button>
                  ))}
                </div>
              </div>
              <div className="border-t border-white/5 pt-5 space-y-3">
          ...
          <TabsContent value="visual" className="space-y-6 animate-in fade-in">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                <div className="space-y-3">{characters?.map((char, i) => (<Card key={i} className="bg-white/5 border border-white/10 rounded-xl p-4 space-y-3"><div className="flex justify-between items-center"><h4 className="font-semibold text-emerald-400 text-sm">{char.name}</h4><Button onClick={() => { const p = "必ず画像を制作して下さい\n名前を出力して下さい\n\n"; navigator.clipboard.writeText(p + char.imagePrompt); alert("コピーしました！"); }} className="h-7 bg-white/10 text-white border border-white/10 hover:bg-white/20 font-medium text-xs rounded-lg px-3">個別コピー</Button></div><p className="text-xs text-emerald-500/60 font-mono break-all leading-relaxed bg-black/40 p-3 rounded-lg border border-white/5">{char.imagePrompt}</p></Card>))}</div>
              </div>
              <div className="space-y-4">
