      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <div className="text-center mb-10 space-y-3">
          <div className="inline-flex items-center gap-2 bg-blue-500/10 border border-blue-500/20 rounded-full px-4 py-1.5">
            ...
            <span className="text-blue-400 text-xs font-bold tracking-tight">法人営業AIアシスタント</span>
          </div>
          <h1 className="text-3xl md:text-4xl font-bold text-white tracking-tighter">AIテレアポくん</h1>
          <p className="text-slate-400 text-sm">架電台本・見積もり・記録分析をAIがサポート</p>
        </div>
        ...
        <div className="flex gap-2 mb-8 bg-slate-800/40 p-1.5 rounded-2xl border border-slate-700/40">
          {([
            ...
            { id: 'estimate', label: '法人見積もり生成', icon: FileText },
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                  ...
                  <select className={inputClass} value={scriptForm.industry} onChange={e => setScriptForm(p => ({ ...p, industry: e.target.value }))}>
                    {['IT', '製造業', '不動産', '小売', '医療', 'その他'].map(v => <option key={v}>{v}</option>)}
                  </select>
                </div>
                <div>
                  <label className={labelClass}>商材・サービス名</label>
                  <input className={inputClass} placeholder="例: AI営業支援システム" value={scriptForm.product} onChange={e => setScriptForm(p => ({ ...p, product: e.target.value }))} />
                </div>
                <div>
                  ...
                  <select className={inputClass} value={scriptForm.scene} onChange={e => setScriptForm(p => ({ ...p, scene: e.target.value }))}>
                    {['初回アポ', 'フォローアップ', 'クロージング', '断られた後'].map(v => <option key={v}>{v}</option>)}
                  </select>
                </div>
                <div>
                  <label className={labelClass}>予算感</label>
                  <select className={inputClass} value={scriptForm.budget} onChange={e => setScriptForm(p => ({ ...p, budget: e.target.value }))}>
                    {['〜10万', '〜50万', '〜100万', '100万以上'].map(v => <option key={v}>{v}</option>)}
                  </select>
                </div>
              </div>
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-2">
                    <button className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-700/50 hover:bg-slate-700 border border-slate-600/50 rounded-lg text-xs font-medium text-slate-300 transition-all" onClick={copyScript}>
                      {scriptCopied ? <><Check className="h-3.5 w-3.5 text-emerald-400" /> コピー済み</> : <><Copy className="h-3.5 w-3.5" /> コピー</>}
                    </button>
                    <button className="flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-blue-600/80 to-cyan-600/80 hover:from-blue-500 hover:to-cyan-500 rounded-lg text-xs font-medium text-white transition-all" onClick={useScriptForEstimate}>
                      <FileText className="h-3.5 w-3.5" /> この台本で見積もりを作る
                    </button>
                  </div>
                </div>
        ...
        {activeTab === 'estimate' && (
          <div className="space-y-6">
            <div className={cardClass}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                ...
                <div>
                  ...
                  <input className={inputClass} placeholder="例: AI営業支援システム 導入支援" value={estimateForm.serviceName} onChange={e => setEstimateForm(p => ({ ...p, serviceName: e.target.value }))} />
                </div>
              </div>
              ...
              <div className="mb-5">
                <div className="flex items-center justify-between mb-2">
                  <label className={labelClass + ' mb-0'}>費目</label>
                ...
                <div className="space-y-2">
                  {estimateItems.map((item, i) => (
                    <div key={i} className="flex gap-2 items-center">
                      <input className={inputClass + ' flex-1'} placeholder="項目名（例: 初期導入費）" value={item.name} onChange={e => updateItem(i, 'name', e.target.value)} />
              ...
              <div className="mb-5">
                <label className={labelClass}>備考・条件</label>
                <textarea className={inputClass + ' h-20 resize-none'} placeholder="例: 3ヶ月契約・毎月請求" value={estimateForm.notes} onChange={e => setEstimateForm(p => ({ ...p, notes: e.target.value }))} />
              </div>
              ...
              <button className={`${btnPrimary} w-full`} onClick={generateEstimate} disabled={estimateLoading || !estimateForm.companyName || !estimateForm.serviceName}>
                {estimateLoading ? <><Spinner /> 見積もりを生成中...</> : <><FileText className="h-4 w-4" /> 見積もり提案文を生成する</>}
              </button>
            </div>
            ...
            {estimateResult && (
              <div className={cardClass}>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-base font-bold text-white">見積もり内容</h3>
                  <button className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-700/50 hover:bg-slate-700 border border-slate-600/50 rounded-lg text-xs font-medium text-slate-300 transition-all" onClick={copyEstimate}>
                    {estimateCopied ? <><Check className="h-3.5 w-3.5 text-emerald-400" /> コピー済み</> : <><Copy className="h-3.5 w-3.5" /> コピー</>}
                  </button>
                </div>
                {/* Table */}
        ...
        {activeTab === 'record' && (
          <div className="space-y-6">
            <div className={cardClass}>
              <h2 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
                <ClipboardList className="h-5 w-5 text-emerald-400" /> 架電を記録する
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <label className={labelClass}>会社名</label>
                ...
                <div>
                  ...
                  <select className={inputClass} value={recordForm.result} onChange={e => setRecordForm(p => ({ ...p, result: e.target.value }))}>
                    {['アポ取れた🎉', '折り返し📞', '検討中🤔', 'NG❌', '不在📵'].map(v => <option key={v}>{v}</option>)}
                  </select>
                </div>
                <div>
                  <label className={labelClass}>次回架電希望日時</label>
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea className={inputClass + ' h-20 resize-none'} placeholder="会話内容・気づきなどを記録" value={recordForm.memo} onChange={e => setRecordForm(p => ({ ...p, memo: e.target.value }))} />
                </div>
              </div>
            ...
            {records.length === 0 && (
              <div className="text-center py-16 text-slate-500">
                ...
                <p className="text-xs mt-1">架電したら記録を追加しましょう</p>
              </div>
            )}
          </div>
        )}
