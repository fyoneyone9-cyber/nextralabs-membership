  ...
  return (
    <div className="min-h-screen bg-[#050507] text-slate-200 font-sans pb-32">
      ...
      <section className="max-w-6xl mx-auto px-4 pt-24 pb-20 text-center space-y-10">
        <Badge className="bg-teal-500/10 text-teal-400 border-teal-500/20 px-6 py-1 rounded-full font-black uppercase text-xs tracking-widest">Spatial AI Coordinator</Badge>
      ...
      <section className="bg-[#13141f] py-24 border-y border-white/5 text-left text-left">
        <div className="max-w-5xl mx-auto px-4 grid md:grid-cols-2 gap-16 items-center">
          <div className="space-y-8 text-left">
            <h3 className="text-3xl font-black text-white italic uppercase tracking-tight">家具選び、疲れていませんか？</h3>
          ...
          <div className="bg-black/50 border-4 border-teal-500/20 rounded-[3rem] p-10 shadow-inner text-center">
             <p className="text-teal-400 text-lg italic font-black leading-loose">
               AIが「空間のバイブス」を数値化。<br/>
      ...
      <section className="max-w-6xl mx-auto px-4 py-32 space-y-20 text-left">
        <div className="text-center space-y-4 text-center">
          <h3 className="text-4xl md:text-5xl font-black text-white italic uppercase tracking-tighter text-center">Spatial Intelligence</h3>
          <p className="text-slate-500 font-bold uppercase italic text-center">理想の部屋を叶える4つのマスタ技術</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-10">
          <div className="bg-[#13141f] border-2 border-white/5 p-10 rounded-[3rem] space-y-6 hover:border-teal-500/30 transition-all group">
            <div className="w-16 h-16 bg-teal-600/10 rounded-2xl flex items-center justify-center text-teal-500 group-hover:scale-110 transition-transform"><Smartphone size={32} /></div>
            <h4 className="text-2xl font-black text-white italic uppercase">リアルタイムARスキャン</h4>
            <p className="text-slate-400 font-bold leading-relaxed text-sm">スマホをかざすだけ。AIが部屋の広さ、既存家具の色調、照明の状態を視覚的にプロファイリングします。</p>
          </div>
          <div className="bg-[#13141f] border-2 border-white/5 p-10 rounded-[3rem] space-y-6 hover:border-teal-500/30 transition-all group">
            <div className="w-16 h-16 bg-teal-600/10 rounded-2xl flex items-center justify-center text-teal-500 group-hover:scale-110 transition-transform"><Palette size={32} /></div>
            <h4 className="text-2xl font-black text-white italic uppercase">スタイル適合エンジニアリング</h4>
            <p className="text-slate-400 font-bold leading-relaxed text-sm">「北欧モダン」「サイバーパンク」など、空間の方向性をAIが決定。適合率90%以上のアイテムを厳選します。</p>
          </div>
          <div className="bg-[#13141f] border-2 border-white/5 p-10 rounded-[3rem] space-y-6 hover:border-teal-500/30 transition-all group">
            <div className="w-16 h-16 bg-teal-600/10 rounded-2xl flex items-center justify-center text-teal-500 group-hover:scale-110 transition-transform"><ShoppingCart size={32} /></div>
            <h4 className="text-2xl font-black text-white italic uppercase">楽天市場・一括同期</h4>
            <p className="text-slate-400 font-bold leading-relaxed text-sm">楽天のリアルな在庫データからアイテムを抽出。そのまま丸ごとセットで購入可能なシームレス動線を構築。</p>
          </div>
          <div className="bg-[#13141f] border-2 border-white/5 p-10 rounded-[3rem] space-y-6 hover:border-teal-500/30 transition-all group">
            <div className="w-16 h-16 bg-teal-600/10 rounded-2xl flex items-center justify-center text-teal-500 group-hover:scale-110 transition-transform"><Globe size={32} /></div>
            <h4 className="text-xl font-black text-white italic uppercase">マルチデバイス対応</h4>
      ...
      <section className="max-w-5xl mx-auto px-4 pt-20 text-center">
        <Card className="bg-gradient-to-br from-teal-600 to-emerald-900 border-0 rounded-[4rem] p-12 md:p-20 shadow-2xl relative overflow-hidden text-center space-y-10 text-center">
          ...
          <div className="relative z-10 space-y-6 text-center text-center">
            <h3 className="text-4xl md:text-6xl font-black text-white italic uppercase tracking-tighter text-center leading-none">Sync Your Living.</h3>
