...
import { 
  ...
  Sofa, Layout, Pen, Sparkles, Box
} from 'lucide-react'

export default function InteriorCoordinatorApp() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult("お部屋の写真を解析しました。現在のナチュラルな雰囲気を活かしつつ、アクセントカラーとして『ミッドナイトブルー』のクッションを配置するプランを策定。推奨家具5点をリストアップしました。");
      ...
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-emerald-500/20 pb-10">
          <div className="flex items-center gap-4">
            <div className="p-4 bg-emerald-500/10 rounded-2xl border border-emerald-500/20"><Sofa className="h-10 w-10 text-emerald-400" /></div>
            <div>
              <h1 className="text-3xl md:text-5xl font-black italic uppercase tracking-tighter text-white">Interior AI Sync</h1>
              <p className="text-emerald-400 font-bold uppercase tracking-[0.2em] text-[10px] italic">Strategic Space & Aesthetic Engine</p>
            </div>
          </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">
            模様替えしたい部屋の写真をアップロードするか、希望のインテリアスタイルを入力してください。AIが空間の広さと色のバランスを解析し、あなたの「理想の空間」を実現するための最適な家具配置と購入リストを提案します。
          </p>
        </div>
        ...
        <Card className="bg-[#13141f] border border-white/5 rounded-2xl overflow-hidden shadow-xl p-8 space-y-6 text-center">
          <div className="w-full aspect-video bg-black/40 rounded-2xl border-2 border-dashed border-white/10 flex items-center justify-center group hover:border-emerald-500/50 transition-all cursor-pointer">
            <div className="space-y-4">
              <Layout className="h-12 w-12 text-slate-600 mx-auto group-hover:text-emerald-400" />
              <p className="text-xs font-black text-slate-500 uppercase italic">Upload Room Photo</p>
            </div>
          </div>
          <Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full h-24 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-3xl rounded-[2rem] shadow-xl uppercase italic">
            {isAnalyzing ? <Loader2 className="animate-spin h-10 w-10" /> : '空間解析を開始する 🚀'}
          </Button>
        </Card>
        ...
        {result && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <Card className="bg-emerald-500/5 border-2 border-emerald-500/30 rounded-[3.5rem] p-12 shadow-inner">
              <h3 className="text-2xl font-black text-white italic uppercase mb-8 flex items-center gap-3"><Zap className="text-emerald-400" /> AI Interior Insight</h3>
              <div className="text-xl text-white font-bold italic leading-loose whitespace-pre-wrap">{result}</div>
            </Card>

            <div className="space-y-6">
              <h3 className="text-xl font-black text-white italic uppercase tracking-widest border-l-4 border-emerald-500 pl-4">Styling Roadmap</h3>
              <div className="grid md:grid-cols-3 gap-6">
                {[
                  { step: '01', title: '空間スキャン', desc: '家具のサイズ感と現在の色相環を分析し、デッドスペースを特定。', icon: Search },
                  { step: '02', title: 'スタイル策定', desc: 'あなたの好みに合わせて、北欧・モダン・和風等のコンセプトを適用。', icon: Sparkles },
                  { step: '03', title: '一括調達', desc: '予算に合わせて、楽天・Amazon等の最適ショップリンクを生成します。', icon: ShoppingCart },
                ].map((s, i) => (
            ...
            <a href="https://www.amazon.co.jp/s?k=おしゃれ家具+インテリア雑貨&tag=nextralabs-22" target="_blank" className="block group">
              <div className="bg-gradient-to-r from-emerald-600 to-teal-800 p-10 rounded-[3rem] flex items-center justify-between shadow-2xl transition-all hover:scale-[1.01]">
                <div className="space-y-2">
                  <p className="text-[10px] font-black text-white/50 uppercase tracking-widest italic">Aesthetic Living</p>
                  <h3 className="text-2xl font-black text-white italic leading-tight">暮らしを、作品に変える。AI厳選のインテリア。</h3>
                </div>
