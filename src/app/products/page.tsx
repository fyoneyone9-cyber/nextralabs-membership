...
import { 
  ...
  Sprout, Zap, Droplets, Utensils, Building2, Hotel, Key, type LucideIcon 
} from 'lucide-react'
    ...
    <Card className="h-full bg-[#1a1b23] border-slate-800 hover:border-emerald-500/30 transition-all duration-300 rounded-[2rem] overflow-hidden group shadow-xl">
      <CardContent className="p-8 flex flex-col h-full text-left">
        <div className="flex items-start justify-between mb-6">
          <div className={`p-4 rounded-2xl ${product.bgColor} ${product.iconColor} group-hover:scale-110 transition-transform shadow-inner`}>
            <Icon className="h-8 w-8" />
          </div>
          <Badge className="bg-slate-800 text-slate-400 border-0 px-3 py-1 font-bold text-[10px] uppercase tracking-widest">{product.status}</Badge>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#0f1115] text-slate-200 pb-32 font-sans">
      <div className="max-w-7xl mx-auto px-4 pt-20 text-center mb-20 space-y-4">
        ...
        <p className="text-slate-500 max-w-xl mx-auto text-lg leading-relaxed font-medium">全22種類のNextraLabs AIツール。機能はそのまま、一本道UIで使いやすく。</p>
      </div>
