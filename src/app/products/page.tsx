  ...
  return (
    <div className="min-h-screen bg-[#0f1115] text-slate-200 pb-32 font-sans">
      ...
      <div className="max-w-7xl mx-auto px-4 space-y-32">
        {/* TODAY'S PICKセクション */}
        <section>
          <SectionTitle emoji="??" title="TODAY'S PICK" color="border-orange-500" />
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {todayRecommended.map(p => <ProductCard key={p.id} product={p} />)}
          </div>
        </section>

        <section><SectionTitle emoji="??" title="FREE TOOLS" color="border-emerald-500" /><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{freeTools.map(p => <ProductCard key={p.id} product={p} />)}</div></section>
