...
export const metadata: Metadata = {
  ...
  description: 'NextraLabsの全てのAIツール。22種類以上の強力なAIが使い放題。',
}
...
const freeTools: Product[] = [
  ...
  { id: 'moving-checker', title: '引っ越し安心AI', subtitle: '治安・騒音スコア表示', description: '物件のリスクをAIが数値化。後悔しない引っ越しをサポート。', priceNote: '無料', tags: ['治安', '騒音'], icon: Home, bgColor: 'bg-emerald-500/10', iconColor: 'text-emerald-400', status: '定番' },
...
const defenseTools: Product[] = [
  { id: 'scam-defender', title: 'AI詐欺ディフェンダー', subtitle: '詐備・闇バイト判定', description: '不審な連絡をAIが即判定。最新の詐欺手口から家族を守ります。', priceNote: 'プレミアム', tags: ['防犯', '家族'], icon: ShieldCheck, bgColor: 'bg-red-500/10', iconColor: 'text-red-400', status: '安心' },
  ...
  { id: 'disaster-guard', title: 'AI防災ガイド', subtitle: 'GPS避難所 × 警報', description: '現在地の避難所を検索、家族の避難プランをAIが提案。', priceNote: 'スタンダード', tags: ['防災', 'GPS'], icon: Shield, bgColor: 'bg-sky-500/10', iconColor: 'text-sky-400', status: 'NEW' },
...
const creativeTools: Product[] = [
  { id: 'prompt-master', title: 'AIプロンプトマスター', subtitle: '画像生成用英語変換', description: '日本語のイメージを高品質な英語プロンプトへ変換します。', priceNote: 'ライト', tags: ['画像AI', 'Midjourney'], icon: Wand2, bgColor: 'bg-purple-500/10', iconColor: 'text-purple-400', status: '必須' },
...
const funTools: Product[] = [
  ...
  { id: 'pet-translator', title: 'AIペット翻訳', subtitle: '鳴き声リアルタイム解析', description: '留守中のペットの声を解析し、感情を日本語で翻訳して通知。', priceNote: 'プレミアム', tags: ['エンタメ', '癒し'], icon: PawPrint, bgColor: 'bg-pink-500/10', iconColor: 'text-pink-400', status: 'NEW' },
      ...
      <CardContent className="p-8 flex flex-col h-full text-left">
        ...
        <div className="pt-6 border-t border-slate-800/50 flex flex-col gap-4 mt-auto">
          ...
          <div className="flex justify-between items-center px-1">
            <span className="text-[10px] font-bold text-slate-600 uppercase tracking-widest">Access: {product.priceNote}</span>
  ...
  return (
    <div className="min-h-screen bg-[#0f1115] text-slate-200 pb-32 font-sans">
      <div className="max-w-7xl mx-auto px-4 pt-20 text-center mb-24 space-y-4">
        ...
        <div className="max-w-5xl mx-auto bg-slate-900/50 border border-white/5 p-8 rounded-[2.5rem] mt-12 grid grid-cols-1 md:grid-cols-3 gap-8 text-left shadow-2xl">
          ...
          <div className="space-y-3">
            ...
            <h3 className="text-xl font-bold text-white">究極のハイブリッド</h3>
      ...
      <div className="max-w-7xl mx-auto px-4 space-y-32">
        ...
        <section><SectionTitle emoji="🏨" title="ホテル・民泊オーナー向け" color="border-blue-500" /><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{hotelTools.map(p => <ProductCard key={p.id} product={p} />)}</div></section>
        ...
        <section><SectionTitle emoji="💬" title="コミュニケーション" color="border-pink-500" /><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{commTools.map(p => <ProductCard key={p.id} product={p} />)}</div></section>
        <section><SectionTitle emoji="🏢" title="キャリア・ライフ" color="border-slate-500" /><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{lifeTools.map(p => <ProductCard key={p.id} product={p} />)}</div></section>
        ...
        <section><SectionTitle emoji="🎨" title="クリエイティブ" color="border-purple-500" /><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{creativeTools.map(p => <ProductCard key={p.id} product={p} />)}</div></section>
        <section><SectionTitle emoji="🐾" title="エンタメ・趣味" color="border-rose-500" /><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{funTools.map(p => <ProductCard key={p.id} product={p} />)}</div></section>
      </div>
    </div>
  )
}
