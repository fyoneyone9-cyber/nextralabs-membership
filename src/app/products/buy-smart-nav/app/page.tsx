...
import { 
  ...
  Scale, ArrowRight, ShieldCheck, Repeat
} from 'lucide-react'
...
export default function BuySmartNavApp() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult("対象商品の市場データを解析しました。中古相場が現在高騰しており、新品との価格差がわずか15%です。メーカー保証とリセールバリューを考慮すると、今回は『新品』の購入が賢明な判断です。");
      ...
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-emerald-500/20 pb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl md:text-5xl font-black italic uppercase tracking-tighter text-white">Buy Smart Nav</h1>
              <p className="text-emerald-400 font-bold uppercase tracking-[0.2em] text-[10px] italic">New vs Used AI Value Decision</p>
            </div>
          </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">
            購入を検討している商品の名前を入力してください。AIが「新品の現在価格」と「中古市場の流通価格・リセール予測」を天秤にかけ、どちらが長期的に見て経済的メリットが大きいかを判定。損をしない賢い選択をナビゲートします。
          </p>
        </div>

        <Card className="bg-[#13141f] border border-white/5 rounded-2xl overflow-hidden shadow-xl p-8 space-y-6">
          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic ml-1">Product Name / Specific Item</label>
          <input className="w-full h-14 bg-black border-2 border-white/10 rounded-xl px-6 font-bold text-white outline-none focus:border-emerald-500 transition-all" placeholder="例：MacBook Air M2 13インチ" />
          <Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full h-24 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-3xl rounded-[2rem] shadow-xl uppercase italic">
            {isAnalyzing ? <Loader2 className="animate-spin h-10 w-10" /> : 'スマート判定を実行 🚀'}
          </Button>
        </Card>
        ...
        {result && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <Card className="bg-emerald-500/5 border-2 border-emerald-500/30 rounded-[3.5rem] p-12 shadow-inner">
              <h3 className="text-2xl font-black text-white italic uppercase mb-8 flex items-center gap-3"><Zap className="text-emerald-400" /> AI Value Insight</h3>
              <div className="text-xl text-white font-bold italic leading-loose whitespace-pre-wrap">{result}</div>
            </Card>

            <div className="space-y-6">
              <h3 className="text-xl font-black text-white italic uppercase tracking-widest border-l-4 border-emerald-500 pl-4">Smart Choice Roadmap</h3>
              <div className="grid md:grid-cols-3 gap-6">
                {[
                  { step: '01', title: '相場比較', desc: '主要フリマサイトと家電量販店の価格差をリアルタイム抽出。', icon: Search },
                  { step: '02', title: 'リセール判定', desc: '1年後の売却価格を予測し、実質的な使用コストを算出します。', icon: TrendingUp },
                  { step: '03', title: '最適購入ルート', desc: '最もポイント還元率が高い、または安価な購入先を提示します。', icon: ShieldCheck },
                ].map((s, i) => (
            ...
            <a href="https://www.amazon.co.jp/s?k=中古+新品+買い物術&tag=nextralabs-22" target="_blank" className="block group">
              <div className="bg-gradient-to-r from-blue-600 to-indigo-800 p-10 rounded-[3rem] flex items-center justify-between shadow-2xl transition-all hover:scale-[1.01]">
                <div className="space-y-2">
                  <p className="text-[10px] font-black text-white/50 uppercase tracking-widest italic">Smart Purchase</p>
                  <h3 className="text-2xl font-black text-white italic leading-tight">「安物買いの銭失い」を回避する、究極の選択眼。</h3>
                </div>
