      ...
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[{ id: 'video', label: '動画', icon: Video }, { id: 'image', label: '画像', icon: ImageIcon }, { id: 'pdf', label: 'PDF', icon: FileText }].map(m => (
            <Button key={m.id} onClick={() => handleModeChange(m.id as any)} className={`h-20 rounded-2xl font-black text-lg border-2 ${mode === m.id ? 'bg-emerald-600 border-emerald-400 text-white shadow-lg' : 'bg-black/40 border-white/5 text-slate-500'}`}>
        ...
        <Card className="bg-[#13141f] border border-white/10 rounded-[2.5rem] p-12 text-center space-y-8 shadow-2xl">
          {!resultUrl ? (
            <>
              ...
              <Button onClick={handleProcess} disabled={isProcessing || !file} className="w-full h-24 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-3xl rounded-[2rem] shadow-xl uppercase italic">{isProcessing ? <Loader2 className="animate-spin h-10 w-10 mx-auto" /> : 'AI変換を開始する 🚀'}</Button>
            </>
          ) : (
            <div className="space-y-8">
              <CheckCircle2 className="h-20 w-20 text-emerald-500 mx-auto" />
              <h2 className="text-4xl font-black text-white italic uppercase">完了しました</h2>
