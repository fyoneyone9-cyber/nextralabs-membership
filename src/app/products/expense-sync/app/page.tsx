...
import { Zap, Loader2, CheckCircle2, TrendingUp, Search, Info, ShoppingCart, Repeat, Download, Video, ImageIcon, FileText, Lock, Copy, Scale, LineChart, Scissors, Sparkles, Trash2, Pen, MessageSquare, LogOut, ShieldCheck, Ticket, Bell, UserCircle, Briefcase, Table, FileSpreadsheet } from 'lucide-react'
...
export default function ExpenseSyncApp() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult("領収書からデータを抽出し、会計ソフト（Freee/マネーフォワード等）へインポート可能な記帳データを生成しました。");
    ...
    <div className="min-h-screen bg-[#050507] text-slate-100 p-4 md:p-12 font-sans selection:bg-emerald-500/30 text-left">
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">領収書の写真、またはスキャンデータをアップロードしてください。AIが日付・金額・科目を自動判定し、CSVデータを生成します。</p>
        </div>
        ...
        <Card className="bg-[#13141f] border border-white/10 rounded-2xl p-8 text-center space-y-6 shadow-xl relative">
          <div className="w-full h-48 bg-black/40 border-2 border-dashed border-white/10 rounded-2xl flex items-center justify-center cursor-pointer hover:border-emerald-500/50 transition-all">
             ...
             <div className="space-y-2">
                ...
                <p className="text-slate-500 font-bold uppercase italic">Upload Receipts</p>
             </div>
          </div>
          <Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full h-20 bg-emerald-600 hover:bg-emerald-50 text-slate-950 font-black text-2xl rounded-[2rem] shadow-xl uppercase italic active:scale-95">全自動記帳を開始 🚀</Button>
        </Card>

        {result && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <Card className="bg-emerald-500/5 border-2 border-emerald-500/30 rounded-[3.5rem] p-12 shadow-inner text-left">
              <h3 className="text-2xl font-black text-white italic uppercase mb-8 flex items-center gap-3"><Zap className="text-emerald-400" /> AI記帳結果レポート</h3>
              <div className="text-xl text-white font-bold italic leading-loose whitespace-pre-wrap mb-10">{result}</div>
              <Button onClick={() => alert('保存完了')} className="h-16 px-10 bg-white text-slate-950 font-black text-lg rounded-xl shadow-xl italic"><Download className="mr-2" /> CSVを保存</Button>
            </Card>
            ...
            <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {[{ title: '自動抽出', desc: '手書き文字も正確に読込。', icon: Search }, { title: '勘定判定', desc: '科目を自動推論して割当。', icon: ListChecks }, { title: 'ソフト連携', desc: '確定申告準備を一瞬で完了。', icon: TrendingUp }].map((s, i) => (
            ...
            <a href="https://www.amazon.co.jp/s?k=確定申告+節税&tag=nextralabs-22" target="_blank" className="block group">
              <div className="bg-gradient-to-r from-blue-600 to-indigo-800 p-10 rounded-[3rem] flex items-center justify-between shadow-2xl transition-all">
                <h3 className="text-2xl font-black text-white italic">不敗の経理：本来の「稼ぐ仕事」へ集中 ➔</h3>
