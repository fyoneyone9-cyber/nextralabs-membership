...
export const metadata = {
  title: 'AI防災パーソナルガイド | リアルタイムGPS×気象連動の生存戦略 | NextraLabs',
  description: 'GPSで現在地と天気を自動特定。公的機関の基準に基づき、あなたの居住環境に最適な72時間生存戦略をAIが立案。3・3・3の法則に基づき、命を守る最優先アクションを提示します。',
  keywords: ['防災 アプリ', '避難計画 AI', 'ハザードマップ', '生存戦略', '災害対策'],
}

export default function DisasterLp() {
  return (
    <div className="min-h-screen bg-[#050507] text-slate-200 font-sans pb-32">
      ...
      <section className="max-w-6xl mx-auto px-4 pt-24 pb-20 text-center space-y-10">
        <Badge className="bg-sky-500/10 text-sky-400 border-sky-500/20 px-6 py-1 rounded-full font-black uppercase text-xs">Survival Intelligence Hub</Badge>
        ...
        <div className="flex flex-wrap justify-center gap-6 pt-6">
          <Link href="/products/disaster-guard/app">
            <button className="h-20 px-12 bg-sky-600 hover:bg-sky-500 text-white font-black text-xl rounded-2xl shadow-[0_20px_50px_rgba(14,165,233,0.3)] transition-all active:scale-95 uppercase italic">
              生存戦略を立案する ➔
            </button>
          </Link>
        </div>
        ...
        <div className="max-w-5xl mx-auto px-4 grid md:grid-cols-2 gap-16 items-center">
          <div className="space-y-8 text-left">
            ...
            <ul className="space-y-4 text-slate-400 font-bold">
              ...
              <li className="flex items-center gap-4"><AlertTriangle className="text-red-500 shrink-0" /> 今、この瞬間の天気（気温・風速）が避難に与える影響</li>
              <li className="flex items-center gap-4"><AlertTriangle className="text-red-500 shrink-0" /> 備蓄品が本当に今の自分に足りているか不明</li>
            </ul>
          </div>
          <div className="bg-black/50 border-4 border-sky-500/20 rounded-[3rem] p-10 shadow-inner">
             <p className="text-sky-400 text-lg italic font-black text-center leading-loose">
      ...
      <section className="max-w-6xl mx-auto px-4 py-32 space-y-20">
        <div className="text-center space-y-4">
          <h3 className="text-4xl md:text-5xl font-black text-white italic uppercase tracking-tighter">Survivor Intelligence</h3>
          <p className="text-slate-500 font-bold uppercase italic">生死を分ける4つのテクノロジー</p>
        </div>

        <div className="grid md:grid-cols-2 gap-10 text-left">
          <div className="bg-[#13141f] border-2 border-white/5 p-10 rounded-[3rem] space-y-4 hover:border-sky-500/30 transition-all">
            <div className="w-12 h-12 bg-sky-500/10 rounded-xl flex items-center justify-center text-sky-500"><MapPin /></div>
          ...
          <div className="bg-[#13141f] border-2 border-white/5 p-10 rounded-[3rem] space-y-4 hover:border-sky-500/30 transition-all">
            <div className="w-12 h-12 bg-sky-500/10 rounded-xl flex items-center justify-center text-sky-500"><Zap /></div>
          ...
          <div className="bg-[#13141f] border-2 border-white/5 p-10 rounded-[3rem] space-y-4 hover:border-sky-500/30 transition-all">
            <div className="w-12 h-12 bg-sky-500/10 rounded-xl flex items-center justify-center text-sky-500"><Navigation /></div>
            <h4 className="text-xl font-black text-white uppercase italic">詳細環境プロファイリング</h4>
            <p className="text-slate-400 text-sm font-bold leading-relaxed">都道府県、市区町村、住居形態（木造・マンション等）を分離入力。あなたの住まいのリスクをピンポイントで分析します。</p>
          </div>
          <div className="bg-[#13141f] border-2 border-white/5 p-10 rounded-[3rem] space-y-4 hover:border-sky-500/30 transition-all">
            <div className="w-12 h-12 bg-sky-500/10 rounded-xl flex items-center justify-center text-sky-500"><Shield /></div>
      ...
      <section className="max-w-5xl mx-auto px-4 pt-20">
        <Card className="bg-gradient-to-br from-sky-600 to-blue-900 border-0 rounded-[4rem] p-12 md:p-20 shadow-2xl relative overflow-hidden text-center space-y-10">
          ...
          <div className="relative z-10 space-y-6">
            <h3 className="text-4xl md:text-6xl font-black text-white italic uppercase tracking-tighter">Stay Safe, Stay Smart.</h3>
            ...
            <div className="flex flex-wrap justify-center gap-6 pt-6">
              <Link href="/signup">
                <button className="h-20 px-16 bg-white text-sky-700 font-black text-2xl rounded-2xl shadow-xl hover:bg-sky-50 transition-all active:scale-95 uppercase italic">
                  立案を開始する ➔
                </button>
              </Link>
            </div>
