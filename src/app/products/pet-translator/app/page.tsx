...
import { 
  ...
  PawPrint, Camera, MessageCircle, Heart, Sparkles, Languages
} from 'lucide-react'

export default function PetTranslatorApp() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult("「今はとってもリラックスしていて、あなたに構ってほしい気分だよ！でも少しお腹が空いているかも。」\n\n解析結果：目の輝きと尾の動きから、高い幸福度と軽微な空腹サインを検知しました。");
      ...
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-emerald-500/20 pb-10">
          <div className="flex items-center gap-4">
            <div className="p-4 bg-emerald-500/10 rounded-2xl border border-emerald-500/20"><PawPrint className="h-10 w-10 text-emerald-400" /></div>
            <div>
              <h1 className="text-3xl md:text-5xl font-black italic uppercase tracking-tighter text-white">Pet Translator AI</h1>
              <p className="text-emerald-400 font-bold uppercase tracking-[0.2em] text-[10px] italic">Strategic Interspecies Communication Engine</p>
            </div>
          </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">
            ペットの写真、鳴き声、または現在の仕草を文章で入力してください。AIが動物行動学に基づき、ペットの「深層心理」と言葉にできない「欲求」を日本語へ翻訳。絆を深めるための最適な接し方をアドバイスします。
          </p>
        </div>
        ...
        <Card className="bg-[#13141f] border border-white/5 rounded-2xl overflow-hidden shadow-xl p-8 space-y-6 text-center">
          <div className="w-full aspect-video bg-black/40 rounded-2xl border-2 border-dashed border-white/10 flex items-center justify-center group hover:border-emerald-500/50 transition-all cursor-pointer">
            <div className="space-y-4">
              <Camera className="h-12 w-12 text-slate-600 mx-auto group-hover:text-emerald-400" />
              <p className="text-xs font-black text-slate-500 uppercase italic">Upload Pet Photo or Voice</p>
            </div>
          </div>
          <Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full h-24 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-3xl rounded-[2rem] shadow-xl uppercase italic">
            {isAnalyzing ? <Loader2 className="animate-spin h-10 w-10" /> : 'ペットの声を翻訳する 🚀'}
          </Button>
        </Card>
        ...
        {result && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <Card className="bg-emerald-500/5 border-2 border-emerald-500/30 rounded-[3.5rem] p-12 shadow-inner">
              <h3 className="text-2xl font-black text-white italic uppercase mb-8 flex items-center gap-3"><Zap className="text-emerald-400" /> AI Translation Output</h3>
              <div className="text-xl text-white font-bold italic leading-loose whitespace-pre-wrap">{result}</div>
            </Card>

            <div className="space-y-6">
              <h3 className="text-xl font-black text-white italic uppercase tracking-widest border-l-4 border-emerald-500 pl-4">Communication Roadmap</h3>
              <div className="grid md:grid-cols-3 gap-6">
                {[
                  { step: '01', title: '感情スキャン', desc: '瞳孔の開き、耳の角度、尻尾の動きから現在のストレスレベルを算出。', icon: Search },
                  { step: '02', title: '欲求特定', desc: '遊び、睡眠、食事、体調不良。今最も求めていることをAIが特定。', icon: Heart },
                  { step: '03', title: '最適ケア', desc: '翻訳結果に基づき、ストレスを与えないための具体的なアクションを提示。', icon: TrendingUp },
                ].map((s, i) => (
            ...
            <a href="https://www.amazon.co.jp/s?k=ペット用品+健康管理+おもちゃ&tag=nextralabs-22" target="_blank" className="block group">
              <div className="bg-gradient-to-r from-emerald-600 to-teal-800 p-10 rounded-[3rem] flex items-center justify-between shadow-2xl transition-all hover:scale-[1.01]">
                <div className="space-y-2">
                  <p className="text-[10px] font-black text-white/50 uppercase tracking-widest italic">Happy Pet Life</p>
                  <h3 className="text-2xl font-black text-white italic leading-tight">言葉を超えた絆を。AIが選んだ最高のご褒美セレクション。</h3>
                </div>
