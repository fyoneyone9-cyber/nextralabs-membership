...
import { 
  ...
  BookOpen, Crown, FileText, Brain, Rocket, FileDown
} from 'lucide-react'
...
export default function KindleFactoryApp() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult("Amazonランキング上位を狙える『売れる目次』と第1章の原稿を生成しました。Googleドキュメントへ出力可能です。");
      ...
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-emerald-500/20 pb-10">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-3xl md:text-5xl font-black italic uppercase tracking-tighter text-white">Kindle AI ファクトリー</h1>
          </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">テーマとターゲットを入力してください。AIが構成案と本文を自動生成。出版までの工数を大幅に短縮します。結果はGoogleドキュメントへ書き出し可能です。</p>
        </div>
        ...
        <Card className="bg-[#13141f] border border-white/5 rounded-2xl p-8 space-y-6 shadow-xl">
          ...
          <Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full h-20 bg-emerald-600 hover:bg-emerald-50 text-slate-950 font-black text-2xl rounded-[2rem] shadow-xl uppercase italic">本の原稿を自動生成 🚀</Button>
        </Card>

        {result && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <Card className="bg-emerald-500/5 border-2 border-emerald-500/30 rounded-[3.5rem] p-12 shadow-inner text-left">
              <h3 className="text-2xl font-black text-white italic uppercase mb-8 flex items-center gap-3"><Zap className="text-emerald-400" /> AI原稿出力</h3>
              <div className="text-xl text-white font-bold italic leading-loose whitespace-pre-wrap mb-10">{result}</div>
              <Button onClick={() => setDocUrl('#')} className="h-16 px-10 bg-white text-slate-950 font-black text-lg rounded-xl shadow-xl hover:bg-slate-100 transition-all italic"><FileText className="mr-2" /> Googleドキュメントへ保存</Button>
            </Card>

            <div className="space-y-6">
              <h3 className="text-xl font-black text-white italic uppercase tracking-widest border-l-4 border-emerald-500 pl-4">出版ロードマップ</h3>
              <div className="grid md:grid-cols-3 gap-6">
                {[{ title: '目次・構成', desc: '最強の章立てを策定。', icon: Search }, { title: '超速執筆', desc: '原稿を爆速で書き上げ。', icon: Brain }, { title: 'KDP入稿', desc: 'フォーマットで出力。', icon: TrendingUp }].map((s, i) => (
            ...
            <a href="https://www.amazon.co.jp/s?k=Kindle出版&tag=nextralabs-22" target="_blank" className="block group">
              <div className="bg-gradient-to-r from-orange-600 to-amber-800 p-10 rounded-[3rem] flex items-center justify-between shadow-2xl transition-all">
                <h3 className="text-2xl font-black text-white italic">不敗の印税生活：出版の全技術 ➔</h3>
