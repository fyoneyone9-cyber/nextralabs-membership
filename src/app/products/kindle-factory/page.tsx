  ...
  return (
    <div className="min-h-screen bg-[#050507] text-slate-200 font-sans pb-32">
      ...
      <section className="max-w-4xl mx-auto px-4 pt-24 pb-20 text-center space-y-8">
        <Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 px-6 py-1 rounded-full font-black uppercase text-xs">KDP出版 完全自動化</Badge>
        ...
        <div className="flex flex-wrap justify-center gap-4 pt-4">
          <Link href="/products/kindle-factory/app">
            <button className="h-16 px-10 bg-emerald-600 hover:bg-emerald-500 text-white font-black text-lg rounded-2xl shadow-[0_20px_50px_rgba(16,185,129,0.3)] transition-all active:scale-95 uppercase italic">
              今すぐ原稿を生成する ➔
            </button>
          </Link>
        </div>
      ...
      <section className="max-w-4xl mx-auto px-4 py-16 grid md:grid-cols-2 gap-8">
        {[
          ...
          <div key={i} className="bg-[#13141f] border-2 border-white/5 p-8 rounded-[2rem] space-y-4 hover:border-emerald-500/30 transition-all group">
            ...
            <h3 className="text-xl font-black text-white italic uppercase">{f.title}</h3>
      ...
      <section className="max-w-3xl mx-auto px-4 py-12">
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-[2rem] p-8 space-y-6">
          <h3 className="font-black text-amber-400 text-lg uppercase tracking-wider flex items-center gap-2">
          ...
          <ol className="space-y-4">
            {[
              ...
              <li key={s.step} className="flex gap-4 items-start">
                <span className="text-emerald-500 font-black text-2xl italic min-w-[40px]">{s.step}</span>
                <div>
      ...
      <section className="max-w-3xl mx-auto px-4 pt-8 text-center space-y-6">
        <h3 className="text-3xl font-black text-white italic uppercase">今日から出版できる。</h3>
        <Link href="/products/kindle-factory/app">
          <button className="h-16 px-12 bg-emerald-600 hover:bg-emerald-500 text-white font-black text-lg rounded-2xl shadow-xl transition-all active:scale-95 uppercase italic">
            原稿生成をはじめる ➔
          </button>
        </Link>
      </section>
