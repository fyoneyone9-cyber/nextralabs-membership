...
const FinalConfirmation: React.FC<FinalConfirmationProps> = ({ reservation, guestInfo, onNext, type = 'checkin' }) => {
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-right-12 duration-1000 pb-32">
      <div className="text-center space-y-4">
        ...
        <p className="text-gray-500 text-lg font-bold tracking-[0.4em] uppercase">
          {isCheckout ? 'チェックアウト内容の確認' : '予約詳細の確認'}
        </p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white/5 backdrop-blur-2xl border border-white/10 p-10 rounded-[50px] space-y-8">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className={`p-6 rounded-3xl border ${!isCheckout ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-white/5 border-white/5 opacity-40'}`}>
              ...
              <div className={`p-6 rounded-3xl border ${isCheckout ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-white/5 border-white/5'}`}>
                <div className="flex items-center gap-3 text-gray-500 mb-2">
                  <Clock size={18} />
             ...
             <div className="space-y-4">
                ...
                {reservation?.extraCharges > 0 && (
                  <div className="flex justify-between items-end pb-4 border-b border-red-500/20 bg-red-500/5 p-4 rounded-xl">
                    <span className="text-red-400 font-black">追加料金（要スタッフ精算）</span>
                    <span className="text-4xl font-black text-red-500">¥{reservation.extraCharges.toLocaleString()}</span>
                  </div>
                )}
                <div className="flex justify-between items-center text-sm">
                   <span className="text-gray-600">うち消費税(10%)</span>
                   <span className="font-bold">¥{Math.floor(((reservation?.amount || 0) + (reservation?.extraCharges || 0)) * 0.1).toLocaleString()}</span>
                </div>
             </div>
          </div>
        </div>
      ...
      <button
        onClick={onNext}
        className={`group w-full py-12 rounded-[50px] text-4xl font-black tracking-[0.2em] hover:scale-[1.02] active:scale-[0.98] transition-all shadow-2xl flex items-center justify-center gap-4 ${
          isCheckout && reservation?.extraCharges > 0 
          ? 'bg-red-600 text-white shadow-red-500/20' 
          : 'bg-white text-black'
        }`}
      >
        {isCheckout 
          ? (reservation?.extraCharges > 0 ? 'CALL STAFF' : 'FINISH CHECK-OUT') 
          : 'FINISH CHECK-IN'}
