...
import { 
  ...
  UserPlus, UserCircle, Database, Table, Contact, Scan, Rocket
} from 'lucide-react'
...
export default function ContactSyncApp() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult("名刺の写真から「氏名・役職・会社名・電話番号・メールアドレス」を正確に抽出しました。CRMやGoogleコンタクトへ即座に一括インポート可能なリストを生成済みです。");
      ...
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-emerald-500/20 pb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-emerald-400 font-bold uppercase tracking-[0.2em] text-[10px] italic">Strategic Network Management</p>
            </div>
          </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">
            手元にある名刺を写真に撮るか、スキャンしてアップロードしてください。AIが記載情報をデジタル化し、連絡先管理ツールや営業CRMに即座に同期できる形式に整えます。「名刺交換したまま」の人脈を、価値ある資産へ変えましょう。
          </p>
        </div>
        ...
        <Card className="bg-[#13141f] border border-white/5 rounded-2xl overflow-hidden shadow-xl p-8 space-y-6 text-center">
          <div className="w-full aspect-video bg-black/40 rounded-2xl border-2 border-dashed border-white/10 flex items-center justify-center group hover:border-emerald-500/50 transition-all cursor-pointer">
            <div className="space-y-4">
              <Contact className="h-12 w-12 text-slate-600 mx-auto group-hover:text-emerald-400" />
              <p className="text-xs font-black text-slate-500 uppercase italic">Upload Business Cards</p>
            </div>
          </div>
          <Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full h-24 bg-emerald-600 hover:bg-emerald-50 text-slate-950 font-black text-3xl rounded-[2rem] shadow-xl uppercase italic">
            {isAnalyzing ? <Loader2 className="animate-spin h-10 w-10" /> : '名刺データのデジタル化を開始 🚀'}
          </Button>
        </Card>
        ...
        {result && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <Card className="bg-emerald-500/5 border-2 border-emerald-500/30 rounded-[3.5rem] p-12 shadow-inner">
              <h3 className="text-2xl font-black text-white italic uppercase mb-8 flex items-center gap-3"><Zap className="text-emerald-400" /> AI Network Insight</h3>
              <div className="text-xl text-white font-bold italic leading-loose whitespace-pre-wrap">{result}</div>
            </Card>

            <div className="space-y-6">
              <h3 className="text-xl font-black text-white italic uppercase tracking-widest border-l-4 border-emerald-500 pl-4">Network Roadmap</h3>
              <div className="grid md:grid-cols-3 gap-6">
                {[
                  { step: '01', title: '全項目抽出', desc: '社名、役職、SNSアカウントなど、記載された全情報を逃さずAIが整理。', icon: Search },
                  { step: '02', title: '名簿自動同期', desc: 'Google・Apple等の各デバイス連絡先へ、一括で神経を繋ぎ込みます。', icon: Table },
                  { step: '03', title: 'フォローアップ', desc: '交換後の「お礼メール」や「商談設定」のタイミングをAIがプランニング。', icon: TrendingUp },
                ].map((s, i) => (
            ...
            <a href="https://www.amazon.co.jp/s?k=人脈術+営業+コミュニケーション&tag=nextralabs-22" target="_blank" className="block group">
              <div className="bg-gradient-to-r from-blue-600 to-indigo-800 p-10 rounded-[3rem] flex items-center justify-between shadow-2xl transition-all hover:scale-[1.01]">
                <div className="space-y-2">
                  <p className="text-[10px] font-black text-white/50 uppercase tracking-widest italic">Connection Master</p>
                  <h3 className="text-2xl font-black text-white italic leading-tight">「人脈」を武器に変え、ビジネスを一段階上へ。</h3>
                </div>
