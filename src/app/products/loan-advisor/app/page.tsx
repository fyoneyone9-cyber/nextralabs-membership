...
import { 
  ...
  Table, ListChecks, FileText, Database, ShieldCheck
} from 'lucide-react'
      ...
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-emerald-500/20 pb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl md:text-5xl font-black italic uppercase tracking-tighter text-white">AI Loan Advisor</h1>
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">
            現在の借入残高、金利、月々の支払額を入力してください。AIが法的知識と最新のローン商品を照合。利息を最小化するための「おまとめプラン」や、完済までの最短ルートをシミュレーションします。
          </p>
        </div>

        <Card className="bg-[#13141f] border border-white/5 rounded-2xl overflow-hidden shadow-xl p-8 space-y-6">
          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic ml-1">Current Debt Info / Monthly Payment</label>
          <textarea className="w-full h-40 bg-black border-2 border-white/10 rounded-xl p-6 font-bold text-white outline-none focus:border-emerald-500 transition-all" placeholder="例：3社から計200万。金利18%。月々8万円返済中。もっと楽にしたい。" />
          <Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full h-24 bg-emerald-600 hover:bg-emerald-50 text-slate-950 font-black text-3xl rounded-[2rem] shadow-xl uppercase italic">
            {isAnalyzing ? <Loader2 className="animate-spin h-10 w-10" /> : '完済シミュレーションを実行 🚀'}
          </Button>
        </Card>
        ...
        {result && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="space-y-6">
              <h3 className="text-xl font-black text-white italic uppercase tracking-widest border-l-4 border-emerald-500 pl-4">Repayment Roadmap</h3>
              <div className="grid md:grid-cols-3 gap-6">
                {[
                  { step: '01', title: '高利子撃破', desc: '利息制限法に基づき、金利15%超の借入を低金利商品へ集約します。', icon: Search },
                  { step: '02', title: 'キャッシュフロー改善', desc: '月々の返済額を調整し、生活を破綻させない「持続可能な返済」を確立。', icon: TrendingUp },
                  { step: '03', title: '完全自由へ', desc: '返済スピードを加速させるための、家計見直しと副業による追加返済プラン。', icon: ShieldCheck },
                ].map((s, i) => (
            ...
            <a href="https://www.amazon.co.jp/s?k=借金完済+資産形成+お金の教養&tag=nextralabs-22" target="_blank" className="block group">
              <div className="bg-gradient-to-r from-blue-600 to-indigo-800 p-10 rounded-[3rem] flex items-center justify-between shadow-2xl transition-all hover:scale-[1.01]">
                <div className="space-y-2">
                  ...
                  <h3 className="text-2xl font-black text-white italic leading-tight">「お金の呪い」を解き、資産を築く。不敗のマネーリテラシー本。</h3>
                </div>
