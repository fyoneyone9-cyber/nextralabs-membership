...
export default function AiRecipeApp() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult("冷蔵庫の在庫から、30分以内で作れる『免疫力アップ御膳』を提案しました。");
    setRecipeData({
    ...
    <div className="min-h-screen bg-[#050507] text-slate-100 p-4 md:p-12 font-sans selection:bg-emerald-500/30">
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed">冷蔵庫の食材を入力してください。AIが栄養バランスを解析し、最高の一皿を提案します。</p>
        </div>
        ...
        {result && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <Card className="bg-emerald-500/5 border-2 border-emerald-500/30 rounded-[3.5rem] p-12 shadow-inner text-left">
              <h3 className="text-2xl font-black text-white italic uppercase mb-8 flex items-center gap-3"><Zap className="text-emerald-400" /> AI献立診断レポート</h3>
              ...
              {recipeData && (
                <div className="grid md:grid-cols-2 gap-6 border-t border-emerald-500/20 pt-8">
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">{recipeData.nutrition.map((n: string) => <Badge key={n} variant="outline" className="text-white">{n}</Badge>)}</div>
                  </div>
                  <div className="space-y-2">
                    <p className="text-[10px] font-black text-rose-400 uppercase italic">不足食材</p>
                    <div className="flex flex-wrap gap-2">{recipeData.missing.map((m: string) => <Badge key={m} className="bg-rose-600 text-white border-0">{m}</Badge>)}</div>
                  </div>
                </div>
              )}
            </Card>
            ...
            <div className="space-y-6">
              <h3 className="text-xl font-black text-white italic uppercase tracking-widest border-l-4 border-emerald-500 pl-4">クッキングロードマップ</h3>
              <div className="grid md:grid-cols-3 gap-6">
                {[{ title: '栄養診断', desc: '摂取できる栄養価をAIが算出。', icon: Search }, { title: '爆速調理', desc: '並行調理の手順を最適化。', icon: Zap }, { title: '在庫最適化', desc: '残り食材の保存法を提示。', icon: TrendingUp }].map((s, i) => (
            ...
            <a href="https://www.amazon.co.jp/s?k=時短調理家電&tag=nextralabs-22" target="_blank" className="block group">
