...
import { Utensils, Camera, Zap, Video, Rocket } from 'lucide-react';
...
export default function AiRecipeLandingPage() {
  const features = [
    ...
    { icon: <Video className="w-6 h-6 text-red-600" />, title: '動画連動', desc: '文字だけでなく、プロの調理動画を同じ画面で確認可能' }
  ];
  ...
  return (
    <div className="container mx-auto px-4 py-16 max-w-5xl text-left font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-20">
        {features.map((f, i) => (
          <Card key={i} className="bg-slate-900 border-slate-800 border-2 rounded-[2.5rem]"><CardContent className="p-8 text-left space-y-4"><div className="w-12 h-12 flex items-center justify-center bg-slate-800 rounded-xl mb-4">{f.icon}</div><h3 className="text-xl font-black text-white italic tracking-tighter uppercase">{f.title}</h3><p className="text-slate-400 font-bold text-sm leading-relaxed">{f.desc}</p></CardContent></Card>
        ))}
      </div>
    </div>
  );
