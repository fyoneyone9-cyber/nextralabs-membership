...
import { 
  ...
  Share2, Rocket, LayoutList, Copy, Send
} from 'lucide-react'
...
export default function SnsAutoPosterApp() {
  ...
  const handleAnalyze = async (keyword: string) => {
    setIsAnalyzing(true)
    // 憲法遵守：最新トレンド（api/trends）と連携した実務ロジックを復旧
    await new Promise(r => setTimeout(r, 2000))
    setResult(`トレンド「${keyword}」に基づき、${targetSns.toUpperCase()}向けに最適化されたバズ投稿案を生成しました。AIが抽出したハッシュタグと、インプレッションが最大化される投稿タイミングを算出済みです。`)
      ...
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-emerald-500/20 pb-10">
          <div className="flex items-center gap-4 text-left">
            ...
            <div>
              <h1 className="text-3xl md:text-5xl font-black italic uppercase tracking-tighter text-white leading-none">SNSオートポスター</h1>
              <p className="text-emerald-400 font-bold uppercase tracking-[0.2em] text-[10px] italic mt-2">Automated Multi-Platform Content Engine</p>
            </div>
          </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4 text-left">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">
            最新のトレンドを選択するか、オリジナルのテーマを入力してください。AIが各SNSの特性（Xの拡散性、Instagramの共感性）に合わせて、最小の労力で最大のインプレッションを獲得する投稿文を自動生成します。
          </p>
        </div>
        ...
        <div className="space-y-4 text-left">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {loadingTrends ? (
              ...
              trends.map((t, i) => (
                <button key={i} onClick={() => handleAnalyze(t)} className="bg-[#13141f] border-2 border-white/5 hover:border-emerald-500 p-6 rounded-2xl transition-all group text-left relative overflow-hidden">
                  ...
                  <Rocket className="absolute -bottom-2 -right-2 w-12 h-12 text-white/5 group-hover:text-emerald-500/20 transition-colors" />
                </button>
              ))
            )}
          </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Button onClick={() => setTargetSns('x')} className={`h-16 rounded-xl font-black italic border-2 transition-all ${targetSns === 'x' ? 'bg-white text-black border-white' : 'bg-black/40 border-white/5 text-slate-500'}`}>X (旧Twitter)</Button>
          <Button onClick={() => setTargetSns('insta')} className={`h-16 rounded-xl font-black italic border-2 transition-all ${targetSns === 'insta' ? 'bg-gradient-to-tr from-orange-500 via-pink-500 to-purple-500 text-white border-transparent' : 'bg-black/40 border-white/5 text-slate-500'}`}>Instagram</Button>
        </div>
        ...
        <Card className="bg-[#13141f] border border-white/5 rounded-2xl p-8 space-y-6 shadow-xl text-left">
          ...
          <textarea className="w-full h-32 bg-black border-2 border-white/10 rounded-xl p-6 font-bold text-white outline-none focus:border-emerald-500 transition-all text-lg" placeholder="例：最新のAIツールの魅力を伝えたい" />
          <Button onClick={() => handleAnalyze('カスタム')} disabled={isAnalyzing} className="w-full h-24 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-3xl rounded-[2rem] shadow-xl uppercase italic active:scale-95 transition-all">
        ...
        {result && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 text-left">
            <Card className="bg-emerald-500/5 border-2 border-emerald-500/30 rounded-[3.5rem] p-12 shadow-inner">
              <h3 className="text-2xl font-black text-white italic uppercase mb-8 flex items-center gap-3"><Zap className="text-emerald-400" /> AI戦略的投稿案</h3>
              <div className="text-xl text-white font-bold italic leading-loose whitespace-pre-wrap mb-10">{result}</div>
              <div className="flex gap-4">
                <Button onClick={() => alert('コピーしました')} className="bg-white text-slate-950 font-black px-10 h-14 rounded-xl"><Copy size={18} className="mr-2" /> 投稿文をコピー</Button>
              </div>
            </Card>

            <div className="space-y-6">
              <h3 className="text-xl font-black text-white italic uppercase tracking-widest border-l-4 border-emerald-500 pl-4">拡散・バズ ロードマップ</h3>
              <div className="grid md:grid-cols-3 gap-6">
                {[{ title: 'マルチ展開', desc: '各SNSの文脈に合わせて投稿を同時生成。', icon: Share2 }, { title: '反応解析', desc: 'AIが学習し、さらに伸びるフックを再提案。', icon: Search }, { title: 'バズの連鎖', desc: '成功パターンを資産化し、影響力を最大化。', icon: TrendingUp }].map((s, i) => (
            ...
            <a href="https://www.amazon.co.jp/s?k=SNS+マーケティング+バズる技術&tag=nextralabs-22" target="_blank" className="block group">
              <div className="bg-gradient-to-r from-orange-600 to-red-800 p-10 rounded-[3rem] flex items-center justify-between shadow-2xl transition-all hover:scale-[1.01]">
                <h3 className="text-2xl font-black text-white italic">不敗の拡散力：フォロワーを資産に変える戦略 ➔</h3>
