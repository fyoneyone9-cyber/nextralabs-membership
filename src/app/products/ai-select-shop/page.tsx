...
import { CheckCircle2, Store, Shirt, Zap, Rocket, ShoppingCart, ArrowRight, TrendingUp, Palette, Box, Package } from 'lucide-react'
import Link from 'next/link'

export const metadata = {
  title: 'AIセレクトショップ | トレンド分析×在庫ゼロのオンデマンド販売OS | NextraLabs',
  description: '最新トレンドをAIが分析し、売れるデザインを自動生成。Printful & Shopifyと完全連携し、在庫リスクゼロでブランドを立ち上げるための最強エンジン。',
  keywords: ['AI Tシャツ作成', '在庫なし 物販', 'Shopify 連携', 'オンデマンド印刷', '副業 物販 AI'],
}
  ...
  return (
    <div className="min-h-screen bg-[#050507] text-slate-200 font-sans pb-32">
      ...
      <section className="max-w-6xl mx-auto px-4 pt-24 pb-20 text-center space-y-10">
        <Badge className="bg-teal-500/10 text-teal-400 border-teal-500/20 px-6 py-1 rounded-full font-black uppercase text-xs">E-Commerce Intelligence</Badge>
        ...
        <div className="max-w-5xl mx-auto px-4 grid md:grid-cols-2 gap-16 items-center">
          <div className="space-y-8">
            ...
            <ul className="space-y-4 text-slate-400 font-bold">
              <li className="flex items-center gap-4"><Box className="text-red-500 shrink-0" /> 大量の在庫を抱えるのが怖い（赤字リスク）</li>
              <li className="flex items-center gap-4"><Box className="text-red-500 shrink-0" /> デザインのスキルがなく、外注費も高い</li>
              <li className="flex items-center gap-4"><Box className="text-red-500 shrink-0" /> 梱包や発送作業が面倒で続けられない</li>
            </ul>
          </div>
      ...
      <section className="max-w-6xl mx-auto px-4 py-32 space-y-20">
        <div className="text-center space-y-4">
          <h3 className="text-4xl md:text-5xl font-black text-white italic uppercase tracking-tighter">Business Infrastructure</h3>
          <p className="text-slate-500 font-bold uppercase italic">在庫ゼロ物販を実現する4つの機能</p>
        </div>

        <div className="grid md:grid-cols-2 gap-10 text-left">
          <div className="bg-[#13141f] border-2 border-white/5 p-10 rounded-[3rem] space-y-4">
            <div className="w-12 h-12 bg-teal-500/10 rounded-xl flex items-center justify-center text-teal-500"><TrendingUp /></div>
          ...
          <div className="bg-[#13141f] border-2 border-white/5 p-10 rounded-[3rem] space-y-4">
            <div className="w-12 h-12 bg-teal-500/10 rounded-xl flex items-center justify-center text-teal-500"><Palette /></div>
          ...
          <div className="bg-[#13141f] border-2 border-white/5 p-10 rounded-[3rem] space-y-4">
            <div className="w-12 h-12 bg-teal-500/10 rounded-xl flex items-center justify-center text-teal-500"><Rocket /></div>
          ...
          <div className="bg-[#13141f] border-2 border-white/5 p-10 rounded-[3rem] space-y-4">
            <div className="w-12 h-12 bg-teal-500/10 rounded-xl flex items-center justify-center text-teal-500"><Package /></div>
            <h4 className="text-xl font-black text-white uppercase italic">Printful 完全自動連携</h4>
            <p className="text-slate-400 text-sm font-bold leading-relaxed">注文が入ったら、Printfulが自動で印刷・梱包・発送。あなたは在庫を持つ必要も、伝票を書く必要もありません。</p>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-5xl mx-auto px-4 pt-20">
        <Card className="bg-gradient-to-br from-teal-600 to-emerald-900 border-0 rounded-[4rem] p-12 md:p-20 shadow-2xl relative overflow-hidden text-center space-y-10">
          ...
          <div className="relative z-10 space-y-6">
            <h3 className="text-4xl md:text-6xl font-black text-white italic uppercase tracking-tighter leading-none">Start Your Legacy.</h3>
            <p className="text-teal-100 text-lg font-bold leading-relaxed max-w-2xl mx-auto">
              あなたのアイデアが、価値ある商品に変わる瞬間。AIセレクトショップで、新しい物販の形を始めましょう。
            </p>
            <div className="flex flex-wrap justify-center gap-6 pt-6">
              <Link href="/signup">
                <button className="h-20 px-16 bg-white text-teal-700 font-black text-2xl rounded-2xl shadow-xl hover:bg-teal-50 transition-all active:scale-95 uppercase italic">
                  Launch Your Shop
                </button>
              </Link>
            </div>
