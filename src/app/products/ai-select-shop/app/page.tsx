  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-32 text-left border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-4 md:p-12 bg-[#050507]">
      <div className="text-center space-y-3">
        ...
        <p className="text-emerald-500 font-bold uppercase tracking-[0.2em] text-[10px] italic">Automated Shopify & Printful Engine</p>
      </div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4 max-w-5xl mx-auto">
        ...
        <p className="text-sm text-slate-300 font-bold leading-relaxed italic">
          最新トレンドからキーワードを選び、AIデザインを生成。ボタン一つでShopifyへ「在庫ゼロ」で自動出品。受注・生産・配送は全てシステムが完結させます。あなたはトレンドを選ぶだけです。
        </p>
      </div>

      <div className="flex gap-2 justify-center bg-white/5 p-2 rounded-2xl border border-white/5 max-w-md mx-auto">
        {[1, 2, 3].map(s => (
          <button key={s} onClick={() => setCurrentStep(s)} className={'flex-1 py-4 rounded-xl font-black italic text-sm md:text-base uppercase tracking-widest transition-all ' + (currentStep === s ? 'bg-emerald-500 text-slate-950 shadow-lg' : 'text-slate-500')}>Step {s}</button>
        ))}
      </div>
      ...
      {currentStep === 2 && (
        <div className="grid lg:grid-cols-2 gap-8 animate-in zoom-in-95">
          <div className="space-y-6">
            <Card className="bg-[#13141f] p-8 border-2 border-white/10 rounded-[2.5rem] space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic ml-1">Design Keyword</label>
          ...
          <div className="bg-[#13141f] rounded-[2.5rem] border-2 border-white/5 p-8 flex flex-col justify-center items-center gap-4 text-center">
             ...
             <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic">AI Design Preview</p>
          </div>
        </div>
      )}
      ...
      {currentStep === 3 && (
        <div className="text-center py-20 space-y-10 animate-in zoom-in">
          ...
          <Button onClick={() => window.open(publishResult?.url)} className="h-20 px-12 bg-white text-emerald-950 font-black text-2xl rounded-2xl shadow-xl hover:scale-105 transition-all uppercase italic">View on Shopify</Button>
          ...
          <div className="space-y-6 pt-10 text-left max-w-5xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {[
                { step: '01', title: '自動出品', desc: 'ShopifyとPrintfulが神経接続。注文が入るまであなたのリスクは¥0。', icon: ShoppingCart },
                { step: '02', title: 'SNS拡散', desc: '生成されたAIモックアップを各SNSへ投稿し、世界の需要を惹きつけます。', icon: TrendingUp },
                { step: '03', title: '完全自動生産', desc: '受注後、AIが工場へ直接発注。あなたは売上の着金を確認するだけ。', icon: Zap },
              ].map((s, i) => (
