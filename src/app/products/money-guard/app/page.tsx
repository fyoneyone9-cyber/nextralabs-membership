...
import { 
  ...
  Wallet, DollarSign, ShieldAlert, BarChart3
} from 'lucide-react'
...
export default function MoneyGuardApp() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult("入力された支出パターンから、心理的な『承認欲求』に紐づく浪費の兆候を検知しました。これを資産運用に回した場合、10年後には約540万円の利益差が生まれる計算です。");
      ...
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-emerald-500/20 pb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl md:text-5xl font-black italic uppercase tracking-tighter text-white">Money Guard AI</h1>
              <p className="text-emerald-400 font-bold uppercase tracking-[0.2em] text-[10px] italic">Strategic Household Defense System</p>
            </div>
          </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">
            最近の大きな買い物や、月々の固定費、気になっている出費の内容を入力してください。AIが「衝動買いの心理的背景」と「将来的な損失額」をシミュレーション。あなたの家計を鉄壁に守る防衛プランを即座に提示します。
          </p>
        </div>

        <Card className="bg-[#13141f] border border-white/5 rounded-2xl overflow-hidden shadow-xl p-8 space-y-6">
          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic ml-1">Expense Detail / Purchase Item</label>
          <textarea className="w-full h-40 bg-black border-2 border-white/10 rounded-xl p-6 font-bold text-white outline-none focus:border-emerald-500 transition-all" placeholder="例：最新のブランドバッグ（25万円）を買おうか迷っている。今の貯金は100万。" />
          <Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full h-24 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-2xl rounded-2xl shadow-xl uppercase italic">
            {isAnalyzing ? <Loader2 className="animate-spin h-10 w-10" /> : '家計防衛シミュレーションを実行 🚀'}
          </Button>
        </Card>
        ...
        {result && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <Card className="bg-emerald-500/5 border-2 border-emerald-500/30 rounded-[3.5rem] p-12 shadow-inner">
              <h3 className="text-2xl font-black text-white italic uppercase mb-8 flex items-center gap-3"><Zap className="text-emerald-400" /> AI Defense Insight</h3>
            ...
            <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {[
                  { step: '01', title: '浪費の根源', desc: 'なぜその出費が必要だと感じたのか、心理的トリガーを暴きます。', icon: Search },
                  { step: '02', title: '損失シミュ', desc: 'その出費を投資に回した場合の「未来の資産額」を可視化。', icon: BarChart2 },
                  { step: '03', title: '鉄壁の家計', desc: '無駄をゼロにし、資産を最大化するための再投資戦略を提示。', icon: ShieldCheck },
                ].map((s, i) => (
            ...
            <a href="https://www.amazon.co.jp/s?k=家計管理+資産運用+心理学&tag=nextralabs-22" target="_blank" className="block group">
              <div className="bg-gradient-to-r from-emerald-600 to-teal-800 p-10 rounded-[3rem] flex items-center justify-between shadow-2xl transition-all hover:scale-[1.01]">
                <div className="space-y-2">
                  <p className="text-[10px] font-black text-white/50 uppercase tracking-widest italic">Asset Building</p>
                  <h3 className="text-2xl font-black text-white italic leading-tight">一生お金に困らないための、最強の「守り」と「攻め」。</h3>
                </div>
