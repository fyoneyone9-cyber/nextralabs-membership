...
export default function MoneyGuardApp() {
  ...
  const handleAnalyze = async () => {
    ...
    const savings = Math.floor(parseInt(amount) * 2.5);
    setResult(`解析完了：その購入を停止し投資へ回した場合、10年後には約¥${savings.toLocaleString()}増加します。`);
    ...
    <div className="min-h-screen bg-[#050507] text-slate-100 p-4 md:p-12 font-sans selection:bg-emerald-500/30 text-left">
      <div className="max-w-5xl mx-auto space-y-10 border-4 border-emerald-500 shadow-[0_0_100px_rgba(16,185,129,0.2)] rounded-[3rem] p-6 md:p-12">
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-4">
          ...
          <p className="text-sm text-slate-300 font-bold leading-relaxed italic">迷っている出費額を入力してください。AIが将来価値を計算し、浪費を抑制するジャッジを行います。</p>
        </div>

        <Card className="bg-[#13141f] border border-white/5 rounded-2xl p-8 space-y-6 shadow-xl">
          <div className="space-y-4">
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic ml-1">検討中の金額 (円)</label>
            <input type="number" value={amount} onChange={e => setAmount(e.target.value)} className="w-full h-16 bg-black border-2 border-white/10 rounded-xl px-6 text-2xl font-black text-white outline-none focus:border-emerald-500 transition-all" />
          </div>
          <Button onClick={handleAnalyze} disabled={isAnalyzing || !amount} className="w-full h-20 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-2xl rounded-[2rem] shadow-xl uppercase italic">
        ...
        {result && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <Card className="bg-red-500/10 border-2 border-red-500/30 rounded-[3.5rem] p-12 shadow-inner">
              <h3 className="text-2xl font-black text-white italic uppercase mb-8 flex items-center gap-3"><Zap className="text-red-500" /> AI浪費抑制レポート</h3>
            ...
            <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {[{ title: '欲望特定', desc: '心理的トリガーを特定。', icon: Search }, { title: '損失可視化', desc: '将来の逸失利益を算出。', icon: BarChart2 }, { title: '鉄壁の防衛', desc: '再投資戦略を提示。', icon: CheckCircle2 }].map((s, i) => (
            ...
            <a href="https://www.amazon.co.jp/s?k=資産運用&tag=nextralabs-22" target="_blank" className="block group">
