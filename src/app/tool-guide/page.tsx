...
import { 
  ...
  Sprout, Zap, Droplets, Utensils, Building2, Hotel, Key, Lock, CreditCard, Coins, Sparkles, Archive, UserPlus, Table, Sofa, Play, TrendingUp, LineChart, Scale, Crown, Gift, HeartHandshake, Smartphone
} from 'lucide-react'

const TOOL_DIRECTORY = [
  { id: 'staysee-ai-finder', name: 'AI×ホテルDXシステム【Nextra】', desc: '宿泊予約・鍵発行を完全同期。人の手を介さない次世代フロント体験。', icon: Building2, color: 'text-emerald-500' },
  ...
  { id: 'ai-konkatsu', name: 'AI婚活コーチ', desc: '上級心理カウンセラーの知見を統合。データと心理で成婚を支援。', icon: Heart, color: 'text-pink-400' },
  { id: 'inbox-organizer', name: 'Gmail AI Accelerator', desc: '未読ゼロを最速で実現。AIがメール要約と返信案作成を代行。', icon: Mail, color: 'text-blue-500' },
  ...
  { id: 'interior-coordinator', name: 'Interior Sync', desc: '空間分析×楽天一括購入。部屋に調和する家具をAIが提案。', icon: Sofa, color: 'text-amber-500' },
  ...
  { id: 'office-politics-graph', name: '社内政治 AI相関図', desc: '組織内のパワーバランスと人間関係の暗部をAIが可視化。', icon: Network, color: 'text-indigo-400' },
  { id: 'moving-checker', name: 'AI引越し安心チェッカー', desc: '住所だけで周辺治安・物件リスクをAIが精密スコアリング。', icon: Home, color: 'text-emerald-400' },
  { id: 'buy-smart-nav', name: '中古・新品AI比較ナビ', desc: '市場価格をAIが比較。今、新品と中古どちらを買うべきか判定。', icon: Scale, color: 'text-indigo-400' },
  { id: 'kdp-guide', name: 'Kindle出版AI完全ナビ', desc: '執筆から申請まで。最短距離で作家デビューをAIがサポート。', icon: BookOpen, color: 'text-orange-400' },
  ...
  { id: 'money-guard', name: 'AI家計防衛シミュレーター', desc: '衝動買いの心理を抑止。支出の致命傷を未然に防ぐ。', icon: Wallet, color: 'text-amber-400' },
  ...
  { id: 'ai-sidejob', name: 'AI副業スタートダッシュ', desc: '適性診断から収益化まで、AI副業の最短ロードマップを提示。', icon: Briefcase, color: 'text-indigo-400' },
  { id: 'prompt-master', name: 'AI画像プロンプトマスター', desc: '1000以上のパーツから究極の画像生成プロンプトを錬成。', icon: Wand2, color: 'text-purple-400' }
];
...
const ToolGuideContent = () => {
  ...
  return (
    <div className="min-h-screen bg-[#050507] text-slate-200 font-sans pb-20">
      <div className="container mx-auto px-4 py-12 max-w-6xl space-y-12">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-500 max-w-2xl mx-auto text-sm md:text-lg font-bold italic leading-relaxed">
            NextraLabsが提供する全24の特化型AIエンジン。その機能と戦略的価値を網羅した公式ディレクトリ。
          </p>
        </div>
            ...
            <Link key={tool.id} href={"/products/" + tool.id}>
              <Card className="bg-[#13141f] transition-all duration-300 rounded-[1.5rem] md:rounded-[2rem] overflow-hidden group shadow-xl relative border-2 border-emerald-500/50 hover:border-emerald-400">
                <CardContent className="p-6 flex items-center gap-6 h-full">
                  <div className={"w-16 h-16 bg-white/5 " + tool.color + " rounded-2xl flex items-center justify-center border border-white/10 group-hover:scale-110 transition-transform shadow-inner flex-shrink-0"}>
                    <tool.icon size={32} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-lg md:text-xl font-black text-white italic uppercase mb-1 truncate group-hover:text-emerald-400 transition-colors">{tool.name}</h3>
                    <p className="text-slate-400 text-[11px] md:text-xs font-bold leading-relaxed line-clamp-2 italic">{tool.desc}</p>
                    <div className="flex items-center gap-2 mt-3 text-emerald-500 text-[9px] font-black uppercase tracking-widest italic opacity-60 group-hover:opacity-100 transition-opacity">
                      解説ページを開く ➔
                    </div>
                  </div>
                </CardContent>
      ...
      <div className="text-center opacity-10 mt-12 font-black uppercase tracking-[0.3em] italic text-[8px]">NextraLabs MASTERMODEL • 2026</div>
    </div>
  )
}
