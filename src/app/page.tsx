...
import {
  BookOpen, Calculator, ShoppingBag, ArrowRight, Star, X, Bell
} from 'lucide-react'
  ...
  return (
    <div className="bg-[#050507] text-slate-200 min-h-screen flex flex-col">
      ...
      <section className="py-12 border-t border-white/5">
        <div className="container mx-auto px-6">
          <Link href="/products/inbound-disaster-alert">
            <div className="group flex flex-col md:flex-row items-center gap-5 bg-[#0d1117] border border-sky-500/25 hover:border-sky-500/50 rounded-2xl px-8 py-7 transition-all duration-200 max-w-3xl mx-auto cursor-pointer hover:bg-sky-500/5">
              <div className="flex-shrink-0 w-14 h-14 rounded-2xl bg-sky-500/10 border border-sky-500/25 flex items-center justify-center">
                <Bell className="w-6 h-6 text-sky-400" />
              </div>
              <div className="flex-1 text-center md:text-left">
                <p className="text-[10px] text-sky-400 font-bold mb-1 uppercase tracking-widest">🆕 New — ホテル・宿泊施設向け</p>
                <p className="text-white font-semibold text-base leading-snug">インバウンド向け 災害時AI多言語避難誘導システム</p>
                <p className="text-slate-400 text-xs mt-1">気象庁アラート検知 → 10言語自動翻訳 → Googleマップルート付きで全ゲストへ一斉送信</p>
              </div>
              <span className="text-sky-400 text-sm font-medium group-hover:translate-x-1 transition-transform shrink-0">詳細を見る →</span>
            </div>
          </Link>
        </div>
      </section>
