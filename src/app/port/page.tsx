...
import { 
  Zap, Mail, Briefcase, Wallet, Shield, Building2, Youtube, Table, UserPlus, Sofa, Play, ArrowRight, Sparkles 
} from 'lucide-react'
    ...
    <div className="min-h-screen bg-[#050507] text-slate-200 font-sans pb-32">
      <div className="max-w-6xl mx-auto px-4 py-20 space-y-12">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-500 font-bold italic uppercase tracking-widest text-sm text-center">Unified Master Model Showcase</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-left">
          {MASTER_MODELS.map((model) => (
            <Link key={model.id} href={`/products/${model.id}/app`}>
              <Card className="bg-[#13141f] border-2 border-emerald-500/50 rounded-[2.5rem] p-8 hover:scale-[1.03] hover:border-emerald-500 transition-all cursor-pointer group shadow-2xl relative overflow-hidden h-48 flex flex-col justify-center text-left">
        ...
        <div className="bg-emerald-600/5 border-2 border-emerald-500/20 rounded-[3rem] p-10 md:p-16 text-center space-y-6 shadow-inner relative overflow-hidden">
           ...
           <p className="text-slate-400 max-w-xl mx-auto font-bold leading-relaxed">
             このポートはNextraLabsの全ての知能（マスタモデル）が集結する中枢です。
           </p>
...
const NoSSRWrapper = dynamic(() => Promise.resolve(PortPageContent), {
  ssr: false,
