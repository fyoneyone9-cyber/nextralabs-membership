  ...
  return (
    <div className="max-w-4xl mx-auto p-2 md:p-10 space-y-4 md:space-y-10 min-h-screen text-slate-200 font-sans pb-32 bg-[#050507] text-left border-4 md:border-8 border-emerald-500/50 rounded-[2rem] md:rounded-[4rem] my-1 md:my-4 shadow-[0_0_100px_rgba(16,185,129,0.2)]">
      <div className="text-center space-y-1 md:space-y-3">
        <Badge className="bg-indigo-600 text-white font-black italic px-4 py-0.5 text-[8px] md:text-[10px] uppercase rounded-full">Purchase Smart Engine</Badge>
        <h1 className="text-2xl md:text-6xl font-black text-white uppercase italic tracking-tighter leading-none drop-shadow-2xl text-center">中古・新品比較ナビ</h1>
        <div className="inline-block bg-emerald-600 text-white font-black px-6 py-1 rounded-full uppercase italic text-[8px] md:text-[10px] tracking-widest shadow-lg">v1.0-MASTER</div>
      </div>
        ...
        <div className="space-y-4">
          <div className="relative group">
            ...
            <input 
              ...
              placeholder="例：iPhone 15 Pro 128GB, MacBook Air M3..."
        ...
        {compareData && (
          <div className="space-y-6 animate-in slide-in-from-bottom-8">
            <div className="grid md:grid-cols-2 gap-6 text-left">
               ...
               <div className={`p-8 rounded-[3rem] space-y-4 border-2 ${compareData.judgment === 'USED_WIN' ? 'bg-emerald-600/10 border-emerald-500/50' : 'bg-black/40 border-white/5'}`}>
                  <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest italic pl-2">Smart Used Price</p>
                  <div className="flex items-baseline gap-4 pl-2">
                    <p className="text-4xl font-black text-emerald-400 italic">¥{compareData.usedPrice.toLocaleString()}</p>
                    <div className="flex flex-col">
                       <span className="text-[8px] font-black text-emerald-500 uppercase">Saving</span>
                       <span className="text-lg font-black text-white italic">¥{(compareData.newPrice - compareData.usedPrice).toLocaleString()}</span>
                    </div>
                  </div>
                  <p className="text-[10px] text-slate-400 font-bold pl-2">{compareData.condition}</p>
               </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
               <button className="h-16 bg-white/5 border-2 border-white/10 rounded-2xl font-black text-white uppercase italic hover:bg-white/10 transition-all flex items-center justify-center gap-2">
                  <ShoppingCart size={20} /> 楽天で新品を見る
               </button>
               <button className="h-16 bg-[#c62828]/10 border-2 border-[#c62828]/30 rounded-2xl font-black text-[#f44336] uppercase italic hover:bg-[#c62828] hover:text-white transition-all flex items-center justify-center gap-2">
