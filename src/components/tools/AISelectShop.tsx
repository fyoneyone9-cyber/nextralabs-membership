...
export default function AISelectShop() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-10 space-y-10 min-h-screen text-slate-200 font-sans pb-32 bg-slate-950 text-left">
                ...
                <div className="grid md:grid-cols-2 gap-12">
                  <div className="space-y-8">
                    ...
                    <div className="relative aspect-square rounded-[2rem] overflow-hidden border-4 border-white/5 bg-[#0a0b14] shadow-inner group">
                      {mockupImage ? <img src={mockupImage} className="w-full h-full object-cover animate-in fade-in" alt="Preview" /> : <div className="absolute inset-0 flex flex-col items-center justify-center space-y-4 opacity-20"><Eye size={64} /><p className="text-sm font-black uppercase tracking-widest">Awaiting Design...</p></div>}
                    ...
                    <Button onClick={generateConcept} disabled={!selectedTrend || isGenerating} className="w-full h-24 bg-white text-black hover:bg-indigo-600 hover:text-white font-black text-2xl rounded-3xl shadow-2xl transition-all flex items-center justify-center gap-4">
      ...
      {activeTab === 'settings' && (
        <Card className="bg-[#13141f] border-2 border-white/10 rounded-[3rem] p-12 md:p-24 animate-in zoom-in-95 relative overflow-hidden text-center">
          <div className="max-w-2xl mx-auto space-y-12">
             ...
             <Button onClick={saveSettings} className="w-full h-20 bg-[#5845e0] text-white font-black rounded-3xl text-xl shadow-2xl flex items-center justify-center gap-4 transition-all"><ShieldCheck size={28}/> <span>構成情報をアップデート</span></Button>
          </div>
        </Card>
      )}
