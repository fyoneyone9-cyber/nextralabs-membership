...
const MasterEngine = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // 🚀 【本物の仕組み】画像を保存（ダウンロード）させることでAIへの添付準備を促す
      const link = document.createElement('a');
      link.href = URL.createObjectURL(file);
      link.download = `evidence-${file.name}`;
      link.click();
      alert("証拠画像を保存しました。AI（ChatGPT/Gemini等）にこの画像を添付して送信してください。");
    }
  };

  const FINAL_PROMPT = `あなたはプロの金融・防犯コンサルタントです。
これから送る【証拠画像（レシート/明細）】と以下の【状況メモ】を分析してください。

【状況メモ】: ${inputText || "（画像を参照）"}
