...
export default function OfficePoliticsGraph() {
  ...
  // 🔴 DYNAMIC MERMAID LOADING (NO NPM NEEDED)
  useEffect(() => {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-10 space-y-8 min-h-screen text-slate-200 font-sans pb-20">
      ...
      <div className="mt-4">
        {activeTab === 'input' && (
          <Card className="bg-slate-900 border border-slate-800 rounded-2xl p-8 md:p-12 shadow-2xl animate-in fade-in slide-in-from-bottom-4">
            <h3 className="text-2xl md:text-4xl font-black text-white italic uppercase tracking-tighter mb-8 flex items-center gap-3"><FileSpreadsheet className="text-indigo-500" /> ① ログ解析</h3>
              ...
              <div className="space-y-6">
                {!file ? (
                  ...
                  <div className="bg-slate-950 border-2 border-indigo-600/30 rounded-3xl p-6 space-y-6 text-center shadow-xl">
                    ...
                    <Button onClick={() => handleCopy(PROMPTS.analyze)} className={`w-full h-16 font-black rounded-xl transition-all ${copied ? 'bg-emerald-500 text-slate-950' : 'bg-indigo-600 text-white'}`}>解析指示をコピー</Button>
                    <Button variant="outline" onClick={() => window.open('https://claude.ai', '_blank')} className="w-full h-12 border-slate-800 text-xs font-black">CLAUDEを開く ↗</Button>
                  </div>
                )}
              </div>
        ...
        {activeTab === 'analyze' && (
          <Card className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-12 animate-in fade-in zoom-in text-center shadow-2xl">
             ...
             <Button onClick={() => handleCopy(PROMPTS.graph)} className="w-full h-20 bg-indigo-600 text-white font-black rounded-2xl shadow-xl mb-10">図解指示をコピー</Button>
             ...
             {mermaidCode && <Button onClick={() => setActiveTab('graph')} className="w-full h-16 mt-10 bg-emerald-600 hover:bg-emerald-500 text-white font-black rounded-2xl shadow-xl flex items-center justify-center gap-3 uppercase italic group">③ 相関図を描画する <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" /></Button>}
          </Card>
        )}
        ...
        {activeTab === 'graph' && (
          <Card className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-8 md:p-12 shadow-2xl animate-in fade-in zoom-in text-center">
             ...
             <Button onClick={() => setActiveTab('input')} variant="outline" className="w-full h-16 mt-10 border-slate-800 text-slate-500 hover:bg-slate-800 font-black rounded-2xl uppercase italic"><RotateCcw className="mr-2 h-5 w-5" /> 最初からやり直す</Button>
          </Card>
        )}
      </div>
