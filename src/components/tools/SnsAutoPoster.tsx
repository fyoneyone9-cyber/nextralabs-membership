...
import { 
  ArrowRight, ClipboardPaste, Zap, ChevronRight, Copy, ExternalLink, RotateCcw, Lightbulb, Share2, Send, Hash, Calendar, Clock, CheckCircle2, Download
} from 'lucide-react'
...
2. 【最強タグ】: 関連度の高い15個のハッシュタグ。
3. 【投稿予約アドバイス】: 最も反応が取れる曜日・時間帯。
  ...
  const renderGuide = (steps: string[]) => (
    <div className="bg-slate-900 border-2 border-red-600/50 rounded-2xl p-6 mb-8 flex items-start gap-4 shadow-xl">
      ...
      <div className="space-y-1">
        <p className="text-sm font-black text-white uppercase italic tracking-widest opacity-70">Auto Posting Protocol</p>
        <div className="space-y-1">
          {steps.map((s, i) => (
      ...
      <div className="mt-4">
        {activeTab === 'input' && (
          <Card className="bg-slate-900 border-2 border-slate-800 rounded-[2.5rem] p-8 md:p-16 shadow-2xl animate-in fade-in slide-in-from-bottom-4 text-center">
            <h3 className="text-2xl md:text-5xl font-black text-white italic uppercase mb-10 flex items-center justify-center gap-4 text-red-500"><Share2 /> ① 投稿内容案</h3>
            {renderGuide(['投稿したい内容のメモを入力する', '最適化指示をコピーしてAI三台体制へ投げる', 'AIが返した最強の投稿セットを右に戻す'])}
            <div className="grid lg:grid-cols-2 gap-12 text-left">
              <div className="space-y-6">
                 <textarea value={contentDraft} onChange={(e) => setContentDraft(e.target.value)} placeholder="投稿ネタをペースト..." className="w-full h-64 bg-slate-950 border-2 border-slate-800 rounded-2xl p-6 text-base text-slate-200 focus:border-red-600 outline-none font-medium shadow-inner leading-relaxed" />
                 {contentDraft && (
              ...
              <div className="bg-slate-950 rounded-[3rem] p-10 border border-slate-800 space-y-6 shadow-2xl flex flex-col justify-center">
                 <div className="flex items-center gap-4"><ClipboardPaste className="h-8 w-8 text-red-500" /><h3 className="text-xl font-black text-white italic uppercase tracking-tighter">最適化結果を戻す</h3></div>
            ...
            {optimizedResult && (
               <Button onClick={() => setActiveTab('optimize')} className="w-full h-20 mt-10 bg-emerald-600 hover:bg-emerald-500 text-white font-black rounded-2xl shadow-xl flex items-center justify-center gap-4 uppercase italic text-xl group">
                  ② 最終投稿セットを確認 <ArrowRight className="w-8 h-8 group-hover:translate-x-2 transition-transform" />
               </Button>
            )}
          </Card>
        )}
        ...
        {activeTab === 'optimize' && (
          <div className="animate-in fade-in zoom-in space-y-8 text-center pb-20 text-left">
            ...
            <Button onClick={() => { setContentDraft(''); setOptimizedResult(''); setActiveTab('input'); }} variant="outline" className="w-full h-16 border-2 border-slate-800 text-slate-500 hover:bg-slate-800 font-black rounded-2xl uppercase italic"><RotateCcw className="mr-2 h-5 w-5" /> 最初からやり直す</Button>
          </div>
        )}
      </div>
    </div>
