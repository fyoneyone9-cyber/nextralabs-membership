  ...
  const renderGuide = (steps: string[]) => (
    <div className="bg-slate-900 border-2 border-indigo-600/50 rounded-2xl p-6 mb-8 flex items-start gap-4 shadow-xl">
      ...
      <div className="space-y-1">
        ...
        <div className="space-y-1">
          {steps.map((s, i) => (
            <p key={i} className="text-xs md:text-lg text-slate-200 font-bold flex items-center gap-2 md:gap-4 leading-tight"><span className="text-indigo-500 italic">#{i+1}</span> {s}</p>
          ))}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-10 space-y-8 min-h-screen text-slate-200 font-sans pb-20 bg-slate-950">
      <div className="text-center space-y-2">
        <Badge className="bg-indigo-600 text-white font-black italic tracking-widest px-4 py-1 text-[10px] uppercase rounded-full shadow-lg">SIDEJOB STARTUP ENGINE</Badge>
      ...
      <div className="mt-4">
        {activeTab === 'diagnosis' && (
          <Card className="bg-slate-900 border-2 border-slate-800 rounded-[2.5rem] p-8 md:p-16 shadow-2xl animate-in fade-in slide-in-from-bottom-4">
            <h3 className="text-2xl md:text-5xl font-black text-white italic uppercase mb-10 flex items-center justify-center gap-4 text-indigo-500"><Target /> ① 副業適性診断</h3>
            {renderGuide(['質問に答えて強みを可視化する', '診断指示をコピーしてAI三台体制へ投げる', 'AIの診断結果をロードマップ画面へ戻す'])}
            ...
            <Button onClick={() => setActiveTab('roadmap')} className="w-full h-16 md:h-20 mt-10 bg-indigo-600 hover:bg-indigo-500 text-white font-black rounded-2xl shadow-xl flex items-center justify-center gap-3 uppercase italic text-sm md:text-lg group">② ロードマップへ進む <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" /></Button>
          </Card>
        )}
        ...
        {activeTab === 'roadmap' && (
          <Card className="bg-slate-900 border-2 border-slate-800 rounded-[3rem] p-8 md:p-16 shadow-2xl animate-in fade-in zoom-in">
            ...
            <div className="grid lg:grid-cols-2 gap-12 text-left">
              <div className="space-y-6">
                 <textarea value={skillInfo} onChange={(e) => setSkillInfo(e.target.value)} placeholder="AIから返ってきたジャンル等を入力..." className="w-full h-64 bg-slate-950 border-2 border-slate-800 rounded-2xl p-6 text-base text-slate-200 focus:border-indigo-500 outline-none font-medium shadow-inner leading-relaxed" />
                 <Button onClick={() => handleCopy(`ジャンル「${skillInfo}」で収益化するための具体的ステップをAIで作成してください。`)} className={`w-full h-16 font-black rounded-xl transition-all ${copied ? 'bg-emerald-500 text-slate-950' : 'bg-red-600 text-white'}`}>計画指示をコピー</Button>
              </div>
              <div className="bg-slate-950 rounded-[3rem] p-10 border border-slate-800 space-y-6 shadow-2xl flex flex-col justify-center">
                 ...
                 <textarea value={sidejobPlan} onChange={(e) => setSidejobPlan(e.target.value)} placeholder="AIが作ったロードマップをペースト..." className="w-full h-80 bg-slate-900 border-2 border-slate-800 rounded-3xl p-6 text-sm text-slate-200 focus:border-indigo-500 outline-none font-mono" />
              </div>
            </div>
            <Button onClick={() => setActiveTab('simulator')} className="w-full h-16 md:h-20 mt-10 bg-indigo-600 hover:bg-indigo-500 text-white font-black rounded-2xl shadow-xl flex items-center justify-center gap-4 uppercase italic group">③ 収益計算へ進む <ArrowRight className="w-8 h-8 group-hover:translate-x-2 transition-transform" /></Button>
          </Card>
        )}
          ...
          <Card className="bg-slate-900 border-2 border-slate-800 rounded-[2.5rem] p-8 md:p-16 shadow-2xl animate-in fade-in zoom-in">
             ...
             <div className="grid lg:grid-cols-2 gap-12 text-center">
                <div className="space-y-8">
                   ...
                   <div className="space-y-4 text-left"><p className="text-sm font-black text-slate-400 uppercase italic">件数: {quantity} 件</p><input type="range" min="1" max="50" step="1" value={quantity} onChange={(e) => setQuantity(Number(e.target.value))} className="w-full accent-indigo-600" /></div>
                ...
                <div className="bg-slate-950 rounded-[3rem] p-10 border border-slate-800 flex flex-col justify-center space-y-6 shadow-2xl">
                   ...
                   <p className="text-slate-400 font-bold italic text-left leading-relaxed">まずは小さく始め、AIを使いこなして時給単価を上げていきましょう。</p>
