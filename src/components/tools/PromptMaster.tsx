...
export default function PromptMaster() {
  ...
  const [logoConcept, setLogoConcept] = useState(''); // ②ロゴ作成項目（自由入力）
  const [selectedTags, setSelectedTags] = useState<string[]>([]); // パーツ
  const [copied, setCopied] = useState(false);

  const STEPS = ["主題を入力", "ロゴ設定", "パーツ選択", "AI実行"];
  const activeStepIndex = subject ? (logoConcept ? (selectedTags.length > 0 ? 3 : 2) : 1) : 0;
...
今すぐ、以下の【主題】と【ロゴ設定】、【指定パーツ】を完璧に組み合わせた「高品質な画像」を1枚生成してください。
...
【指定パーツ】: ${tagsPart}`;
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-10 space-y-8 min-h-screen text-slate-200 font-sans pb-32 bg-slate-950 text-left">
      <div className="text-center space-y-3">
        <Badge className="bg-orange-600 text-white font-black italic tracking-widest px-6 py-1 text-[10px] uppercase rounded-full shadow-lg">ULTIMATE PROMPT STUDIO v6.0</Badge>
        ...
        <Card className="bg-slate-900 border-2 border-slate-800 rounded-[3.5rem] p-8 md:p-16 shadow-[0_40px_100px_rgba(0,0,0,0.6)] relative overflow-hidden">
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            <div className="space-y-10">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center px-2">
                  <h3 className="text-2xl font-black text-white italic uppercase flex items-center gap-3 text-left"><Sparkles className="text-orange-500" /> ① 主題（メインテーマ）</h3>
                ...
                <textarea 
                  ...
                  className="w-full h-40 bg-slate-950 border-2 border-slate-800 rounded-[2rem] p-8 text-2xl text-white font-bold focus:border-orange-500 outline-none shadow-inner transition-all leading-tight" 
                />
              </div>

              {/* ② ロゴ作成・詳細設定（自由入力） */}
              <div className="space-y-4">
                <h3 className="text-2xl font-black text-white italic uppercase flex items-center gap-3 px-2 text-left"><Wand2 className="text-indigo-500" /> ② ロゴ作成項目（自由入力）</h3>
                <textarea 
                  value={logoConcept} 
                  onChange={(e) => setLogoConcept(e.target.value)} 
                  placeholder="例：ロゴにしたい場合はここに「白背景、文字なし、ミニマルなベクター形式」など詳細を自由に入力..." 
                  className="w-full h-40 bg-slate-950 border-2 border-slate-800 rounded-[2rem] p-8 text-xl text-indigo-200 font-bold focus:border-indigo-500 outline-none shadow-inner transition-all" 
                />
              </div>

              {/* プロのパーツを選択 */}
              <div className="space-y-8 max-h-[600px] overflow-y-auto pr-4 scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      {tags.map((tag) => (
                        <Button 
                          ...
                          className={`h-12 px-2 rounded-xl font-black text-[13px] leading-tight transition-all ${selectedTags.includes(tag.content) ? 'bg-orange-600 border-orange-400 text-white scale-105 shadow-xl' : 'bg-slate-950 border-slate-800 text-slate-400 hover:bg-slate-900 hover:text-white'}`}
                        >
                          {tag.label}
                        </Button>
                      ))}
            ...
            <div className="space-y-8">
              <div className="bg-slate-950 p-10 rounded-[3.5rem] border border-slate-800 space-y-8 shadow-2xl text-center flex flex-col justify-center min-h-[700px]">
                ...
                <div className="space-y-6">
                  <Button 
                    ...
                    disabled={!subject}
                    className={`w-full h-48 text-4xl font-black rounded-[3rem] transition-all shadow-xl ${copied ? 'bg-emerald-500 text-slate-950 scale-95' : 'bg-orange-600 text-white hover:bg-orange-500'}`}
                  >
                    {copied ? '✅ 指令をコピーしました' : '錬成指令をコピー'}
                  </Button>
                  <p className="text-base text-slate-400 font-black italic">※コピーして以下のAIへ貼り付けると、自動的に画像生成が始まります</p>
                </div>
