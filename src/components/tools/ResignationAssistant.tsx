  ...
  const renderGuide = (steps: string[]) => (
    <div className="bg-slate-900 border-2 border-emerald-600/50 rounded-2xl p-6 mb-8 flex items-start gap-4 shadow-xl">
      ...
      <div className="space-y-1">
        <p className="text-sm font-bold text-emerald-400 uppercase tracking-tight opacity-70">Liberation Protocol</p>
        <div className="space-y-1">
          {steps.map((s, i) => (
      ...
      <div className="mt-4">
        {activeTab === 'input' && (
          <Card className="bg-slate-900 border-2 border-slate-800 rounded-[2.5rem] p-8 md:p-16 shadow-2xl animate-in fade-in slide-in-from-bottom-4">
            <h3 className="text-2xl md:text-5xl font-bold text-white uppercase mb-10 flex items-center gap-4 text-emerald-500"><FileText /> ① 状況相談</h3>
                 ...
                 {resignationInfo && (
                    <div className="space-y-4">
                       ...
                       <div className="grid grid-cols-2 gap-4">
                          <Button variant="outline" className="h-12 border-slate-800 text-xs font-bold uppercase " onClick={() => window.open('https://claude.ai', '_blank')}>CLAUDE ↗</Button>
                          <Button variant="outline" className="h-12 border-slate-800 text-xs font-bold uppercase " onClick={() => window.open('https://chatgpt.com', '_blank')}>CHATGPT ↗</Button>
                       </div>
                    </div>
                 )}
              </div>
            ...
            {finalAdvice && (
               <Button onClick={() => setActiveTab('plan')} className="w-full h-20 mt-10 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-2xl shadow-xl flex items-center justify-center gap-4 uppercase text-xl group">
        ...
        {activeTab === 'plan' && (
          <div className="animate-in fade-in zoom-in space-y-8 text-center pb-20">
            ...
            <Button onClick={() => { setResignationInfo(''); setFinalAdvice(''); setActiveTab('input'); }} variant="outline" className="w-full h-12 border-2 border-slate-800 text-slate-500 hover:bg-slate-800 font-bold rounded-2xl uppercase "><RotateCcw className="mr-2 h-5 w-5" /> 最初からやり直す</Button>
          </div>
        )}
      </div>
    </div>
