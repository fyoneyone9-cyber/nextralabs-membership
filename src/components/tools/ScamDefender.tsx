...
import { ArrowRight, Upload, CheckCircle2, ShieldAlert, ShieldCheck, Zap, ChevronRight, Copy, ExternalLink, RotateCcw, Lightbulb, ClipboardPaste, Search, AlertOctagon, EyeOff, AlertTriangle, Download, Camera, Loader2 } from 'lucide-react'
...
3. 【鉄壁の防御指示】: 今すぐ実行すべきアクション、および警察への通報手順。`;
              ...
              <div className="space-y-6 text-center">
                {!image ? (
                  <div className="border-4 border-dashed border-slate-800 rounded-[2rem] p-16 hover:bg-slate-950 transition-all cursor-pointer bg-slate-900/50 shadow-inner" onClick={() => fileInputRef.current?.click()}>
                    <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept="image/*" /><Upload className="h-14 w-14 text-slate-700 group-hover:text-red-500 mx-auto mb-6" /><p className="text-xl text-slate-500 font-black italic uppercase tracking-widest">Scan Suspect</p>
                  </div>
                ) : (
                  <div className="space-y-6">
                    <div className="relative aspect-video max-w-[500px] mx-auto rounded-3xl overflow-hidden border-4 border-red-600/30 shadow-2xl bg-black flex items-center justify-center">
                       {isProcessing ? <Loader2 className="animate-spin text-red-500 h-10 w-10" /> : <img src={image} alt="Suspect" className="object-contain w-full h-full" />}
