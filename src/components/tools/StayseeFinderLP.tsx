  ...
  return (
    <div className="min-h-screen bg-[#050507] text-slate-100 font-sans pb-32 selection:bg-emerald-500/30 text-left">
      ...
      <section className="max-w-6xl mx-auto px-4 pt-24 pb-20 text-center space-y-10 border-b border-emerald-500/20">
        <Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 px-6 py-1 rounded-full font-bold uppercase text-[10px] tracking-tight ">💎 MASTERMODEL</Badge>
        ...
        <p className="text-xl md:text-2xl text-emerald-400 font-bold ">宿泊予約・鍵発行を完全同期</p>
        <h2 className="text-lg md:text-xl font-bold text-slate-400 max-w-4xl mx-auto leading-relaxed px-4 ">
          フロント業務を「ゼロ」にする、最強のホテルDXエンジン。<br className="hidden md:block" />
          PMS(Staysee)とスマートロックをAIで神経接続し、無人化・省人化を極限まで加速させます。
        </h2>
        <div className="flex flex-wrap justify-center gap-6 pt-6">
          <ToolLaunchButton productId="staysee-ai-finder" className="h-24 px-16 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-bold text-3xl rounded-[2rem] shadow-[0_0_60px_rgba(16,185,129,0.4)] transition-all active:scale-95 uppercase " />
        </div>
      </section>
      ...
      <section className="bg-[#13141f] py-24 border-y border-white/5">
        <div className="max-w-5xl mx-auto px-4 grid md:grid-cols-2 gap-16 items-center">
          <div className="space-y-8">
            <h3 className="text-3xl font-bold text-white uppercase tracking-tight border-l-4 border-emerald-500 pl-6">現場の「不都合な真実」</h3>
      ...
      <section className="max-w-6xl mx-auto px-4 py-32 space-y-20">
        <div className="text-center space-y-4">
          <h3 className="text-4xl md:text-5xl font-bold text-white uppercase tracking-tighter">真のホテルDXを司る4機能</h3>
        </div>
      ...
      <section className="py-24 bg-black/40 border-y border-white/5">
        <div className="max-w-6xl mx-auto px-4">
          <h3 className="text-3xl font-bold text-white uppercase tracking-tight text-center mb-16">Implementation Roadmap</h3>
      ...
      <section className="max-w-4xl mx-auto px-4 py-20 space-y-8">
        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.4em] text-center ">Strategic External Partners</p>
        <div className="grid grid-cols-3 gap-4">
          {['ChatGPT', 'Gemini', 'Claude'].map(ai => (
            <Button key={ai} variant="outline" className="h-12 border-2 border-white/5 bg-[#13141f] text-slate-400 font-bold rounded-2xl hover:text-white hover:border-emerald-500 transition-all uppercase" onClick={() => window.open(`https://${ai.toLowerCase()}.com`)}>Analyze with {ai}</Button>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-5xl mx-auto px-4">
        <div className="bg-gradient-to-br from-emerald-600 to-teal-900 rounded-[4rem] p-12 md:p-24 shadow-2xl relative overflow-hidden text-center space-y-10">
          <div className="relative z-10 space-y-8">
            <h3 className="text-4xl md:text-7xl font-bold text-white uppercase leading-none tracking-tighter">現場をAIに任せ、<br/>あなたは「おもてなし」へ。</h3>
            <ToolLaunchButton productId="staysee-ai-finder" className="h-24 px-16 bg-white text-emerald-900 font-bold text-3xl rounded-[2rem] shadow-xl hover:scale-105 transition-all active:scale-95" />
          </div>
        </div>
      </section>
    </div>
