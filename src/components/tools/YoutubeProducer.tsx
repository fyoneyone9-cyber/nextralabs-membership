...
const STEPS = [
  ...
  { id: 2, label: '台本作成', what: '最高に面白いストーリー構築', how: '生成されたプロンプトをコピーしてAIに渡してください。', result: 'プロ級の神台本' },
...
const MAJOR_AI = [
  { id: 'gemini', name: 'GEMINI', url: 'https://gemini.google.com', icon: '💎', desc: '【検索】Google連動' },
  { id: 'chatgpt', name: 'CHATGPT', url: 'https://chatgpt.com', icon: '🟢', desc: '【万能】画像生成' },
  { id: 'claude', name: 'CLAUDE', url: 'https://claude.ai', icon: '🟠', desc: '【台本】日本語最強' }
]
                ...
                {!extractedAudioUrl ? (
                  ...
                  <div className="bg-slate-950 p-12 rounded-[3.5rem] border-2 border-blue-500/30 space-y-10 animate-in zoom-in duration-500 text-center">
                    ...
                    <div className="grid grid-cols-3 gap-6 pt-12 border-t border-white/5">
                      {MAJOR_AI.map(ai => (
                        <a key={ai.id} href={ai.url} target="_blank" className="h-24 bg-slate-900 border-2 border-white/5 rounded-3xl flex flex-col items-center justify-center gap-2 hover:scale-105 transition-all shadow-xl group">
                          <span className="text-4xl">{ai.icon}</span><span className="font-black text-white text-xs uppercase">{ai.name}</span>
                        </a>
                      ))}
                    </div>
                  </div>
          ...
          {currentStep > 1 && (
             <div className="space-y-12">
                ...
                <div className="bg-slate-950 p-12 rounded-[3rem] border-2 border-white/5 font-mono text-2xl text-slate-300 leading-relaxed mb-10 max-h-[600px] overflow-y-auto shadow-inner text-left">
                   以下の文字起こしを徹底分析し、NextraLabs品質の結果を出力して。:\n\n{inputText}
                </div>
                ...
                {currentStep < 6 && <Button onClick={() => setCurrentStep(currentStep + 1)} className="w-full h-24 bg-slate-800 hover:bg-slate-700 text-white font-black text-2xl rounded-3xl mt-12">NEXT STAGE</Button>}
             </div>
          )}
        </Card>
      </div>
