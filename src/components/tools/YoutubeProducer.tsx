  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-10 space-y-12 min-h-screen text-slate-200 font-sans pb-20">
      ...
      <div className="max-w-4xl mx-auto bg-indigo-600 p-10 rounded-[3.5rem] text-white shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-4 text-left">
          <div className="flex items-center gap-4">
            <Badge className="bg-white text-indigo-600 font-black px-6 py-2 text-xl rounded-full">STEP 0{currentStep}</Badge>
      ...
      <div className="animate-in fade-in slide-in-from-bottom-8 duration-700">
        <Card className="bg-slate-900 border-2 border-slate-800 rounded-[4rem] p-12 shadow-2xl max-w-5xl mx-auto">
          {currentStep === 1 ? (
                ...
                {!extractedAudioUrl ? (
                  ...
                  <div className="bg-slate-950 p-12 rounded-[3.5rem] border-2 border-blue-500/30 space-y-8 animate-in zoom-in duration-500">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        ...
                        <Button className="h-24 bg-white text-black font-black text-2xl rounded-[2rem] shadow-xl" onClick={() => { navigator.clipboard.writeText("文字起こしして。"); setCopiedId('tr') }}>コピー</Button>
                    </div>
                  </div>
                )}
             ...
             <div className="text-center py-20">
                ...
                <div className="bg-slate-950 p-12 rounded-[3rem] border-2 border-white/5 mt-10 text-slate-300 text-2xl font-mono">
                   指示をコピーしてAIに渡してください。
                </div>
                <div className="grid grid-cols-3 gap-6 mt-12">
                   {MAJOR_AI.map(ai => (
                     <a key={ai.id} href={ai.url} target="_blank" className="p-8 rounded-3xl border-2 border-white/5 bg-slate-950 flex flex-col items-center justify-center gap-4 hover:scale-105 transition-all shadow-xl">
                        <span className="text-5xl">{ai.icon}</span><span className="font-black text-white text-3xl uppercase tracking-tighter">{ai.name}</span>
                     </a>
                   ))}
                </div>
             </div>
