...
import { ArrowRight, Video, FileText, Download, Copy, ExternalLink, FileAudio, Clapperboard, CheckCircle2, Sparkles, MessageSquareText } from 'lucide-react'
...
export default function YoutubeProducer() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-10 space-y-16 min-h-screen text-slate-200">
      ...
      <Card className="bg-slate-900 border-2 border-slate-800 rounded-[4rem] p-16 shadow-2xl overflow-hidden max-w-5xl mx-auto">
        {currentStep === 1 ? (
          <div className="space-y-12">
            ...
            {!extractedAudioUrl ? (
              <div onClick={() => fileRef.current.click()} className="border-4 border-dashed border-slate-800 rounded-[3rem] p-24 text-center hover:bg-slate-800/30 cursor-pointer">
              ...
              <div className="bg-slate-950 p-10 rounded-[3rem] border-2 border-blue-500/30 space-y-8 text-center">
                <p className="text-xl font-bold text-blue-400">✅ 音声抽出成功</p>
                <audio src={extractedAudioUrl} controls className="w-full h-16" />
                <div className="grid grid-cols-2 gap-6">
                  <a href={extractedAudioUrl} download="audio.mp3" className="h-24 bg-blue-600 text-white font-black text-2xl rounded-3xl flex items-center justify-center gap-4">保存</a>
                  <Button className="h-24 bg-white text-black font-black text-2xl rounded-3xl" onClick={() => navigator.clipboard.writeText("文字起こしして。")}>コピー</Button>
                </div>
              </div>
            )}
            <textarea value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="結果を貼り付け..." className="w-full h-80 bg-slate-950 border-2 border-slate-800 rounded-[3rem] p-10 text-2xl font-medium text-white shadow-inner" />
          ...
          <div className="text-center py-20">
            <h2 className="text-4xl font-black text-white italic">Step 02: Scripting Mode</h2>
            <div className="bg-slate-950 p-10 rounded-[3rem] border-2 border-white/5 mt-10 text-slate-300 text-2xl">
              プロンプトが生成されました。コピーして AI に渡してください。
            </div>
          </div>
        )}
      </Card>
