...
import { 
  ArrowRight, Video, FileText, Users, ImageIcon, Music, Type, 
  Download, Copy, ExternalLink, FileAudio, Clapperboard, 
  CheckCircle2, Sparkles, MessageSquareText, HelpCircle, Globe, Zap 
} from 'lucide-react'
...
const MAJOR_AI = [
  { id: 'gemini', name: 'GEMINI', url: 'https://gemini.google.com', icon: '💎', desc: '【検索・大容量】Google連携。' },
  { id: 'chatgpt', name: 'CHATGPT', url: 'https://chatgpt.com', icon: '🟢', desc: '【万能】画像生成に最強。' },
  { id: 'claude', name: 'CLAUDE', url: 'https://claude.ai', icon: '🟠', desc: '【台本】日本語最強。', isBestForScript: true }
]
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-10 space-y-16 min-h-screen text-slate-200 font-sans pb-20">
      <div className="text-center space-y-4">
        <div className="w-24 h-24 rounded-[2rem] bg-red-600 flex items-center justify-center mx-auto shadow-2xl shadow-red-600/20">
                ...
                {!extractedAudioUrl ? (
                  ...
                  <div className="bg-slate-950 p-12 rounded-[3.5rem] border-2 border-blue-500/30 space-y-10 animate-in zoom-in duration-500 text-center">
                    ...
                    <div className="grid grid-cols-2 gap-8">
                        <a href={extractedAudioUrl} download="audio.mp3" className="w-full h-32 bg-blue-600 hover:bg-blue-500 text-white font-black text-3xl rounded-[2.5rem] shadow-2xl flex items-center justify-center gap-6">
                           <Download className="h-12 w-12" /> 音声を保存
                        </a>
                        <Button 
                          onClick={() => handleCopy("詳細に漏れのないように文字起こしして。", "tr")} 
