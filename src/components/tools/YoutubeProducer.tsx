...
const STEPS = [
  { id: 1, label: '素材取り込み', icon: Video, what: '動画・音声の解析準備', how: '動画または音声ファイルをアップロード。AIが扱いやすい形式に自動抽出します。', result: '文字起こし用の音声データ' },
  { id: 2, label: '台本作成', icon: FileText, what: '最高に面白いストーリー構築', how: '生成されたプロンプトをコピーしてAIに渡してください。', result: 'プロ級の神台本' },
  { id: 3, label: 'キャラ設定', icon: Users, what: '登場人物の定義', how: '台本から魅力的なキャラ像をAIに提案させます。', result: '詳細なキャラ設定案' },
  { id: 4, label: 'サムネイル', icon: ImageIcon, what: 'クリック率最大化デザイン', how: 'デザインの天才DALL-E 3等にサムネイル案を依頼します。', result: 'インパクトのある構図案' },
  { id: 5, label: 'タイトル/タグ', icon: Type, what: 'YouTube SEO最適化', how: '検索アルゴリズムに最適化したタイトルを作成します。', result: '検索に強いタイトルとタグ' },
  { id: 6, label: 'BGM作成', icon: Music, what: '感情を揺さぶる音の魔法', how: '動画の雰囲気にマッチするBGMプロンプトを作成します。', result: 'オリジナルBGM' }
]
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-10 space-y-16 min-h-screen text-slate-200 font-sans pb-20">
      ...
      <div className="animate-in fade-in slide-in-from-bottom-8 duration-700">
        <Card className="bg-slate-900 border-2 border-slate-800 rounded-[4rem] p-16 shadow-2xl max-w-5xl mx-auto overflow-hidden">
          {currentStep === 1 && (
             <div className="space-y-12">
                ...
                {!extractedAudioUrl ? (
                  ...
                  <div className="bg-slate-950 p-12 rounded-[3.5rem] border-2 border-blue-500/30 space-y-10 animate-in zoom-in duration-500 shadow-2xl text-center">
                    ...
                    <div className="grid grid-cols-2 gap-8">
                        ...
                        <Button 
                          onClick={() => { navigator.clipboard.writeText("詳細に漏れのないように文字起こしして。"); setCopiedId('tr'); setTimeout(() => setCopiedId(null), 3000) }} 
                          className={w-full h-28 font-black text-3xl rounded-[2rem] shadow-2xl transition-all \}
                        >
                    ...
                    <div className="grid grid-cols-3 gap-6 pt-12 border-t border-white/5">
                      {MAJOR_AI.map(ai => (
                        <a key={ai.id} href={ai.url} target="_blank" className="h-24 bg-slate-900 border-2 border-white/5 rounded-3xl flex flex-col items-center justify-center gap-4 hover:scale-105 transition-all shadow-xl group">
                          ...
                          <span className="text-[10px] text-slate-600 group-hover:text-slate-400">{ai.billing}</span>
                        </a>
                      ))}
                    </div>
                  </div>
          ...
          {currentStep > 1 && (
             <div className="space-y-12">
                <div className="flex justify-between items-center mb-4"><h2 className="text-4xl font-black text-white uppercase italic">Step 0{currentStep}: {STEPS[currentStep-1].label}</h2></div>
                ...
                <Button onClick={() => { navigator.clipboard.writeText("YouTube台本を書いて：" + inputText); setCopiedId('pr'); setTimeout(() => setCopiedId(null), 3000) }} className={w-full h-32 font-black text-4xl rounded-[2.5rem] shadow-2xl transition-all \}>
                   {copiedId === 'pr' ? <><CheckCircle2 className="h-10 w-10" /> コピー完了！</> : "プロンプトをコピー"}
                </Button>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-12 border-t border-white/5">
                  {MAJOR_AI.map(ai => (
                    <a key={ai.id} href={ai.url} target="_blank" className={p-10 rounded-[3rem] border-2 bg-slate-950 flex flex-col items-center justify-center gap-4 hover:scale-105 transition-all shadow-xl \}>
                      {currentStep === 2 && ai.id === 'claude' && <Badge className="absolute -top-4 bg-orange-600 text-white font-black px-6 py-2 text-sm uppercase mb-2 shadow-lg">Recommended</Badge>}
                      ...
                      <p className="text-slate-500 text-xs font-bold uppercase">{ai.desc}</p>
                    </a>
                  ))}
                </div>
                {currentStep < 6 && <Button onClick={() => setCurrentStep((currentStep + 1) as any)} className="w-full h-24 bg-slate-800 hover:bg-slate-700 text-white font-black text-2xl rounded-3xl mt-12 border-b-8 border-slate-950">NEXT STAGE</Button>}
             </div>
          )}
        </Card>
      </div>
