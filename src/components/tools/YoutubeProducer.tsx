...
export default function YoutubeProducer() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-10 space-y-16 min-h-screen text-slate-200 font-sans pb-20">
      ...
      {/* 🟢 STEP PROGRESS BAR - FULLY RESTORED */}
          ...
          {STEPS.map((s, i) => (
            <div key={s.id} className="flex items-center flex-1 last:flex-none">
              <div className={lex flex-col items-center gap-3 transition-all \}>
                ...
                <span className="text-[10px] font-black uppercase tracking-widest mt-2 font-mono">\</span>
              </div>
        ...
        <Card className="bg-slate-900 border-2 border-slate-800 rounded-[4.5rem] p-16 shadow-2xl max-w-5xl mx-auto overflow-hidden">
          {currentStep === 1 ? (
             <div className="space-y-12">
                ...
                {!extractedAudioUrl ? (
                  <div onClick={() => fileRef.current.click()} className="border-4 border-dashed border-slate-800 rounded-[3.5rem] p-24 text-center hover:bg-slate-800/30 cursor-pointer group transition-all">
                  ...
                  <div className="bg-slate-950 p-12 rounded-[3.5rem] border-2 border-blue-500/30 space-y-10 animate-in zoom-in duration-500 text-center">
                    ...
                    <div className="grid grid-cols-2 gap-8">
                        <a href={extractedAudioUrl} download="nextra-audio.mp3" className="w-full h-32 bg-blue-600 hover:bg-blue-500 text-white font-black text-4xl rounded-[2.5rem] shadow-2xl flex items-center justify-center gap-6">
                           <Download className="h-12 w-12" /> 音声を保存
                        </a>
                        <Button 
                          onClick={() => handleCopy("スプレッドシート形式不要、詳細に漏れのないように抜き出して下さい。この音声を一字一句正確に文字起こしして下さい。", 'tr')} 
                          className={w-full h-32 font-black text-4xl rounded-[2.5rem] shadow-2xl transition-all \}
                        >
                ...
                <div className="space-y-8 pt-12 border-t border-slate-800">
                  <Label className="text-4xl font-black text-white flex items-center gap-6 italic uppercase"><MessageSquareText className="h-14 w-14 text-red-500" /> Result Paste Area</Label>
             ...
             <div className="space-y-16">
                <div className="flex justify-between items-center"><h2 className="text-4xl font-black text-white uppercase italic tracking-tighter">Step 0\: \</h2><Button variant="ghost" onClick={() => setCurrentStep(1)} className="text-slate-500 font-bold text-2xl uppercase">← Back</Button></div>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                     {MAJOR_AI.map(ai => (
                       <a key={ai.id} href={ai.url} target="_blank" className={p-10 rounded-[3rem] border-2 bg-slate-950 flex flex-col items-center justify-center gap-6 hover:scale-105 transition-all shadow-xl \}>
                         ...
                         <p className="text-slate-500 text-sm font-bold text-center px-4 uppercase">{ai.desc}</p>
                       </a>
                     ))}
                   </div>
                </div>
