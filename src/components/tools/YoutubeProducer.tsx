  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-10 space-y-16 min-h-screen text-slate-200 font-sans pb-20">
      <div className="text-center space-y-4">
        <div className="w-24 h-24 rounded-[2.5rem] bg-red-600 flex items-center justify-center mx-auto shadow-2xl shadow-red-600/20"><Clapperboard className="h-12 w-12 text-white" /></div>
          ...
          {STEPS.map((s, i) => (
            <div key={s.id} className="flex items-center flex-1 last:flex-none">
              <div className={lex flex-col items-center gap-3 transition-all \}>
                ...
                <span className="text-[11px] font-black uppercase tracking-widest mt-2">\</span>
              </div>
        ...
        <Card className="bg-slate-900 border-2 border-slate-800 rounded-[4rem] p-16 shadow-2xl max-w-5xl mx-auto overflow-hidden">
          {currentStep === 1 && (
             <div className="space-y-12">
                ...
                {!extractedAudioUrl ? (
                  ...
                  <div className="bg-slate-950 p-12 rounded-[3.5rem] border-2 border-blue-500/30 space-y-10 animate-in zoom-in duration-500 text-center">
                    ...
                    <div className="grid grid-cols-2 gap-8">
                        <a href={extractedAudioUrl} download="audio.mp3" className="h-28 bg-blue-600 hover:bg-blue-500 text-white font-black text-3xl rounded-[2rem] shadow-2xl flex items-center justify-center gap-4 transition-transform active:scale-95">
                    ...
                    {/* 🛠️ 復活：3大AIボタン (GEMINI ➔ GPT ➔ CLAUDE) */}
                ...
                <div className="space-y-8 pt-12 border-t border-slate-800">
                  ...
                  <textarea value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="AIから返ってきたテキストを貼り付け..." className="w-full h-80 bg-slate-950 border-2 border-slate-800 rounded-[3rem] p-10 text-2xl font-medium focus:border-red-600 text-white" />
          ...
          {currentStep > 1 && (
             <div className="space-y-12">
                ...
                <div className="bg-slate-950 p-12 rounded-[3rem] border-2 border-white/5 font-mono text-2xl text-slate-300 leading-relaxed mb-10 max-h-[600px] overflow-y-auto shadow-inner text-left">以下を元にYouTube台本を書いて：\n\n{inputText}</div>
                <Button onClick={() => { navigator.clipboard.writeText("YouTube台本："+inputText); setCopiedId('pr'); setTimeout(() => setCopiedId(null), 3000) }} className={w-full h-32 font-black text-4xl rounded-[2.5rem] shadow-2xl transition-all \}>プロンプトをコピー</Button>
