          ...
          <Card className="bg-[#13141f] border-2 border-white/5 rounded-[2.5rem] p-6 md:p-12 shadow-2xl animate-in fade-in">
            ...
            <div className="bg-black p-8 rounded-[3rem] border border-white/5 space-y-8 shadow-inner mb-12 text-center relative overflow-hidden">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <button onClick={() => handleCopy(PROMPTS.script)} className={`h-24 font-black rounded-3xl transition-all shadow-2xl text-xl border-b-8 ${copied ? 'bg-emerald-500 border-emerald-800 text-slate-950' : 'bg-red-600 border-red-800 text-white hover:bg-red-500'}`}>
                  {copied ? '✅ コピー完了' : '① 構成指示をコピー'}
                </button>
                <button onClick={() => window.open('https://claude.ai', '_blank')} className="h-24 bg-white/5 border-2 border-white/10 text-slate-400 font-black italic rounded-3xl uppercase hover:text-white hover:bg-white/10 transition-all text-xl flex items-center justify-center gap-4">
                  ② CLAUDE で実行 🚀
                </button>
              </div>
            </div>
