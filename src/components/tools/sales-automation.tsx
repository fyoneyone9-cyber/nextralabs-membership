...
import { Zap, Mail, Building2, Send, Loader2, Search, X, ShieldCheck, Copy, Sparkles, Bot, Heart, AlertCircle, HelpCircle } from "lucide-react";
...
`;
    ...
    setIsCopied(true);
    toast.success("最強セールスプロンプトをコピーしました！");
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 min-h-screen font-sans bg-slate-50/50 text-left">
      <Card className="border-none bg-white shadow-2xl rounded-[3rem] overflow-hidden antialiased text-slate-900">
        <div className="flex flex-col lg:flex-row min-h-[750px]">
          ...
          <div className="lg:w-1/2 p-12 bg-slate-900 text-white flex flex-col justify-between relative overflow-hidden">
              ...
              <div className="space-y-6">
                <div className="p-6 bg-white/5 rounded-3xl border border-white/5 space-y-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">Target Company</label>
                    <Input placeholder="example.com" className="bg-black/40 border-slate-800 text-white h-16 rounded-2xl text-xl font-black focus:border-blue-500" value={domain} onChange={(e) => setDomain(e.target.value)} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">Contact Name</label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">Context / Needs</label>
            ...
            <div className="p-6 bg-white/5 rounded-3xl border border-white/5 flex items-start gap-4">
               ...
               <p className="text-slate-400 text-xs font-bold leading-relaxed">分析データを最強のプロンプトへ変換。世界最高峰のAI知能（Gemini Pro等）があなたの代わりに刺さる提案を完遂します。</p>
            </div>
          </div>
          ...
          <div className="lg:w-1/2 p-12 flex flex-col bg-white overflow-hidden border-l border-slate-100">
            <div className="flex-1 space-y-10">
              <div className="text-center space-y-4">
                 <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.4em]">Step 2: AIを選んで鑑定開始</p>
                 <div className="grid grid-cols-1 gap-5">
                    <Button onClick={() => handleCopyAndGo('https://gemini.google.com/')} className="h-28 bg-blue-600 hover:bg-blue-500 text-white rounded-[2.5rem] shadow-2xl flex flex-col items-center justify-center group active:scale-95 transition-all border-none relative overflow-hidden">
                      <div className="flex items-center gap-3 text-2xl font-black italic tracking-tighter uppercase relative z-10"><Sparkles className="w-8 h-8 text-amber-300" /> 1. Geminiで鑑定</div>
                      <span className="text-[10px] text-blue-100 font-black uppercase tracking-widest relative z-10">最新情報の検索・分析に最適</span>
                    </Button>
                    <div className="grid grid-cols-2 gap-5">
                      <Button onClick={() => handleAutoCopyAndGo('https://chatgpt.com/')} className="h-16 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl shadow-xl flex items-center justify-center gap-3 font-black active:scale-95 transition-all border-none text-lg italic tracking-tighter uppercase"><Bot className="w-6 h-6" /> ChatGPT</Button>
                      <Button onClick={() => handleAutoCopyAndGo('https://claude.ai/')} className="h-16 bg-orange-600 hover:bg-orange-500 text-white rounded-3xl shadow-xl flex items-center justify-center gap-3 font-black active:scale-95 transition-all border-none text-lg italic tracking-tighter uppercase"><Heart className="w-6 h-6 fill-white" /> Claude</Button>
                    </div>
                 </div>
              </div>
              ...
              {isCopied && (
                <div className="p-10 bg-red-50 rounded-[3rem] border-4 border-red-100 animate-in fade-in slide-in-from-top-6 shadow-2xl relative overflow-hidden text-left">
                   ...
                   <div className="flex flex-col gap-1 mb-8">
                     <div className="flex items-center gap-3 text-red-700 font-black italic text-xl uppercase tracking-tighter">貼り付けるだけ！</div>
                   ...
                   <div className="space-y-5 text-[15px] text-red-950 font-black leading-tight">
                     <p className="flex items-start gap-4"><span className="bg-red-600 text-white w-7 h-7 rounded-xl flex items-center justify-center text-xs flex-shrink-0 mt-0.5 shadow-md">1</span><span>AIアプリが起動したら、入力欄を長押し</span></p>
                     <p className="flex items-start gap-4"><span className="bg-red-600 text-white w-7 h-7 rounded-xl flex items-center justify-center text-xs flex-shrink-0 mt-0.5 shadow-md">2</span><span>そのまま <span className="underline decoration-red-500 decoration-[3px] underline-offset-4 font-black text-red-600">「貼り付け（ペースト）」</span></span></p>
                     <p className="flex items-start gap-4"><span className="bg-red-600 text-white w-7 h-7 rounded-xl flex items-center justify-center text-xs flex-shrink-0 mt-0.5 shadow-md">3</span><span>送信ボタンを押して、戦略メールを取得！</span></p>
                   </div>
                </div>
              )}
            </div>
            <div className="mt-16 pt-8 border-t border-slate-100 flex items-center justify-between text-[10px] font-black text-slate-300 uppercase tracking-[0.4em]">
