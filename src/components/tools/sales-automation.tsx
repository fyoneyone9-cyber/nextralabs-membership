...
export default function sales-automation() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-10 space-y-16 min-h-screen text-slate-200 font-sans pb-20">
      <div className="text-center">
        <h1 className="text-5xl md:text-7xl font-bold text-white uppercase tracking-tighter">sales-automation</h1>
      </div>
      ...
      <div className="max-w-4xl mx-auto bg-emerald-600 p-10 rounded-[3.5rem] text-white shadow-2xl relative overflow-hidden text-left">
        <div className="absolute top-0 right-0 p-8 opacity-10"><HelpCircle className="h-40 w-40" /></div>
        <div className="relative z-10 space-y-4">
          <div className="flex items-center gap-4">
            <Badge className="bg-white text-emerald-600 font-bold px-6 py-2 text-xl rounded-2xl shadow-xl">STEP 0{currentStep}</Badge>
            <h3 className="text-4xl font-bold uppercase tracking-tighter">{currentInfo.what}</h3>
          </div>
      ...
      <div className="animate-in fade-in slide-in-from-bottom-8 duration-700">
        <Card className="bg-slate-900 border-2 border-slate-800 rounded-[4rem] p-16 shadow-2xl max-w-5xl mx-auto overflow-hidden">
          <div className="space-y-12">
            ...
            <Button onClick={handleCopy} className={"w-full h-32 font-bold text-4xl rounded-[2rem] shadow-2xl transition-all " + (copied ? 'bg-emerald-500 text-slate-950 scale-105' : 'bg-white text-black hover:bg-slate-100')}>
            ...
            <div className="grid grid-cols-3 gap-6 pt-12 border-t border-white/5">
              {MAJOR_AI.map(ai => (
                <a key={ai.id} href={ai.url} target="_blank" className="h-24 bg-slate-900 border-2 border-white/5 rounded-3xl flex flex-col items-center justify-center gap-2 hover:scale-105 transition-all shadow-xl group text-slate-400 hover:text-white">
                  <span className="text-3xl">{ai.icon}</span><span className="font-bold text-xs uppercase tracking-tighter">{ai.name}</span>
                </a>
              ))}
            </div>
          </div>
      ...
      <DebugPanel data={null} toolId="sales-automation" />
    </div>
  )
}
