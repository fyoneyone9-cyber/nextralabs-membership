...
export default function StayseeFinderEngine() {
  ...
  const analyzeImage = async () => {
    ...
    try {
      ...
      // 🛠️ どんな結果(エラー含む)でも必ずデバッグデータに保存
      setDebugData({ status: "API_RESPONSE_RECEIVED", http_status: response.status, payload: data });
      ...
      setAnalysisResult({
        ...data,
        suggestedGuests: [{ name: "ヨネヤマ フミタカ 様", room: "302", date: "2026/05/03" }]
      });
    } catch (error: any) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              <div className="space-y-6">
                <div className="relative aspect-square rounded-[2.5rem] overflow-hidden border-4 border-slate-800 bg-black flex items-center justify-center">
                  ...
                  <Button variant="destructive" className="absolute top-6 right-6 rounded-2xl font-black px-6 py-4 h-auto shadow-xl" onClick={() => {setImage(null); setAnalysisResult(null); setDebugData(null)}}>削除</Button>
                </div>
                {!analysisResult && (
                  <Button className="w-full h-24 text-3xl bg-blue-600 hover:bg-blue-500 text-white font-black rounded-[2rem] shadow-2xl shadow-blue-600/30 gap-4" onClick={analyzeImage} disabled={isAnalyzing}>
                    {isAnalyzing ? <><Loader2 className="h-10 w-10 animate-spin" /> 解析中...</> : <><Search className="h-10 w-10" /> 持ち主を特定する</>}
                  </Button>
                )}
              </div>
                ...
                {analysisResult && (
                  <div className="space-y-8 animate-in fade-in zoom-in duration-500">
                    <div className="p-8 bg-green-500/10 border-2 border-green-500/20 rounded-[2.5rem]">
                      <h4 className="font-black text-green-400 flex items-center gap-3 text-2xl mb-6">AI解析完了</h4>
                      <div className="space-y-4 text-xl text-white">
                        <div>品目: {analysisResult.item}</div>
                        <div>カラー: {analysisResult.color}</div>
                        <div>ブランド: {analysisResult.brand}</div>
                      </div>
                    </div>
                    <Button className="w-full h-24 bg-white text-slate-950 font-black text-2xl rounded-[2rem]">自動連絡する</Button>
                  </div>
                )}
              </div>
            </div>
