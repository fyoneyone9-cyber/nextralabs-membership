...
export default function CommCoach() {
  ...
  const FINAL_PROMPT = `あなたは対人心理学とアサーティブ・トレーニングの専門家です。
添付された【トーク画面のスクショ】を分析し、円滑な関係を築くための改善案を出力してください。

1. 【意図の解析】: 相手の発言の背後にある心理状態と、今のやり取りの問題点。
2. 【ベスト返信案】: 角を立てず、かつ自分の意思を明確に伝える3つの返信例文。
3. 【コミュニケーションの鍵】: この相手と今後接する際に意識すべき最重要ポイント。`;
  ...
  const renderGuide = (steps: string[]) => (
    <div className="bg-slate-900 border-2 border-indigo-600/50 rounded-2xl p-5 md:p-8 mb-8 flex items-start gap-4 shadow-xl">
      ...
      <div className="space-y-1">
        <p className="text-sm font-black text-indigo-500 uppercase italic tracking-widest">Communication Protocol</p>
        {steps.map((s, i) => (
      ...
      <div className="mt-4">
        {activeTab === 'input' && (
          <Card className="bg-slate-900 border-2 border-slate-800 rounded-[2.5rem] p-8 md:p-16 shadow-2xl animate-in fade-in slide-in-from-bottom-4 text-center">
            <h3 className="text-2xl md:text-5xl font-black text-white italic uppercase mb-10 flex items-center gap-4 text-indigo-500"><MessageSquare /> ① トーク解析</h3>
            {renderGuide(['悩んでいるトーク画面のスクショをアップ', '指示をコピーしてAIへ投げ、画像をドロップ', 'AIからの処方箋を右のエリアに戻す'])}
              ...
              <div className="space-y-6 text-center">
                {!image ? (
                  <div className="border-4 border-dashed border-slate-800 rounded-[2rem] p-16 hover:bg-slate-950 transition-all cursor-pointer bg-slate-900/50 shadow-inner" onClick={() => fileInputRef.current?.click()}>
                    <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept="image/*" /><Upload className="h-16 w-16 text-slate-700 group-hover:text-indigo-500 mx-auto mb-6" /><p className="text-xl text-slate-500 font-black italic uppercase tracking-widest">Drop Screenshot</p>
                  </div>
                ) : (
        ...
        {activeTab === 'coach' && (
          <div className="animate-in fade-in zoom-in space-y-8 text-center pb-20">
            ...
            <Button onClick={() => { setImage(null); setCommAdvice(''); setActiveTab('input'); }} variant="outline" className="w-full h-16 border-2 border-slate-800 text-slate-500 hover:bg-slate-800 font-black rounded-2xl uppercase italic"><RotateCcw className="mr-2 h-5 w-5" /> 最初からやり直す</Button>
          </div>
        )}
      </div>
    </div>
