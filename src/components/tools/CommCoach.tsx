...
3. 【信頼の鍵】: この相手と今後接する際に意識すべき最重要ポイント。`;
  ...
  const renderGuide = (steps: string[]) => (
    <div className="bg-slate-900 border-2 border-emerald-600/50 rounded-2xl p-6 mb-8 flex items-start gap-4 shadow-xl">
      ...
      <div className="space-y-1 text-left">
        <p className="text-sm font-bold text-emerald-400 uppercase tracking-tight opacity-70">Social Protocol</p>
        <div className="space-y-1">
          {steps.map((s, i) => (
            <p key={i} className="text-xs md:text-lg text-slate-200 font-bold flex items-center gap-2 md:gap-4 leading-tight"><span className="text-emerald-600 ">#{i+1}</span> {s}</p>
          ))}
        </div>
      </div>
    </div>
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-10 space-y-8 min-h-screen text-slate-200 font-sans pb-20 bg-[#050507]">
      ...
      <div className="overflow-x-auto pb-4 scrollbar-hide">
        <div className="bg-slate-900 border border-slate-800 p-2 flex min-w-[500px] md:min-w-full rounded-2xl">
          {TABS.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex-1 py-5 px-2 rounded-xl font-bold text-sm uppercase transition-all flex items-center justify-center gap-2 relative ${activeTab === tab.id ? 'bg-emerald-600 text-white shadow-xl scale-[1.03] z-10' : 'text-slate-500 hover:text-white hover:bg-slate-800'}`}>
      ...
      <div className="mt-4">
        {activeTab === 'input' && (
          <Card className="bg-slate-900 border-2 border-slate-800 rounded-[2.5rem] p-8 md:p-16 shadow-2xl animate-in fade-in slide-in-from-bottom-4 text-center">
            <h3 className="text-2xl md:text-5xl font-bold text-white uppercase mb-10 flex items-center justify-center gap-4 text-emerald-500"><MessageSquare /> ① トーク解析</h3>
              ...
              <div className="space-y-6 text-center">
                {!image ? (
                  ...
                  <div className="space-y-6">
                    <div className="relative aspect-auto max-w-[450px] mx-auto rounded-3xl overflow-hidden border-4 border-emerald-600/30 shadow-2xl bg-black flex items-center justify-center">
                       {isProcessing ? <Loader2 className="animate-spin text-emerald-500 h-10 w-10" /> : <img src={image} alt="Talk" className="object-contain w-full h-auto" />}
                    ...
                    <Button onClick={() => { navigator.clipboard.writeText(FINAL_PROMPT); setCopied(true); setTimeout(() => setCopied(false), 2000); }} className={`w-full h-12 font-bold text-2xl rounded-2xl shadow-2xl transition-all ${copied ? 'bg-emerald-500 text-slate-950' : 'bg-emerald-600 text-white'}`}>改善指示をコピー</Button>
                    <div className="grid grid-cols-3 gap-2">
                       <Button variant="outline" className="h-12 border-slate-800 text-[8px] font-bold uppercase " onClick={() => window.open('https://chatgpt.com', '_blank')}>CHATGPT</Button>
                       <Button variant="outline" className="h-12 border-slate-800 text-[8px] font-bold uppercase " onClick={() => window.open('https://gemini.google.com', '_blank')}>GEMINI</Button>
                       <Button variant="outline" className="h-12 border-slate-800 text-[8px] font-bold uppercase " onClick={() => window.open('https://claude.ai', '_blank')}>CLAUDE</Button>
                    </div>
                  </div>
                )}
              </div>
            ...
            {commAdvice && (
               <Button onClick={() => setActiveTab('coach')} className="w-full h-20 mt-10 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-2xl shadow-xl flex items-center justify-center gap-4 uppercase text-xl group">
        ...
        {activeTab === 'coach' && (
          <div className="animate-in fade-in zoom-in space-y-8 text-center pb-20">
            ...
            <Button onClick={() => { setImage(null); setCommAdvice(''); setActiveTab('input'); }} variant="outline" className="w-full h-12 border-2 border-slate-800 text-slate-500 hover:bg-slate-800 font-bold rounded-2xl uppercase "><RotateCcw className="mr-2 h-5 w-5" /> 別のトークを相談する</Button>
          </div>
        )}
      </div>
    </div>
