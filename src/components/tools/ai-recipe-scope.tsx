...
export default function AiRecipeScope() {
  const [plantName, setPlantName] = useState(''); // 料理名
  ...
  const handleAutoCopyAndGo = (url: string) => {
    ...
    const target = plantName || "写真の食材";
    const magicPrompt = `
重要：このテキストと一緒に、私が今撮影した写真を1枚送信しています。まずその画像を詳細に確認してください。
あなたは天才シェフです。
【現場データ】
・対象の食材: ${target}
・環境情報: ${weatherInfo}
            ...
            {isCameraActive ? (
              ...
              <div className="w-full h-full relative animate-in fade-in duration-700">
                ...
                <div className="absolute top-10 right-10 flex gap-3 z-30">
                  <Button onClick={downloadImage} className="h-16 px-8 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-black shadow-xl border-2 border-white/20 animate-bounce text-lg">1. 写真を保存（必須）</Button>
                  <Button onClick={() => {setImage(null); setStatus('idle'); setIsCopied(false);}} className="h-16 w-16 bg-black/60 text-white rounded-full hover:bg-red-500 border-2 border-white/20"><X className="w-8 h-8" /></Button>
                </div>
                {status === 'scanning' && (
                ...
                {status === 'done' && (
                  <div className="absolute top-32 left-10 right-10 p-8 bg-black/70 backdrop-blur-2xl rounded-[2.5rem] border border-white/20 animate-in slide-in-from-top-4 shadow-2xl text-left">
                    <div className="p-6 bg-green-500/20 border-2 border-green-500/40 rounded-3xl animate-pulse">
                      <p className="text-xl font-black leading-tight text-white tracking-tight">鑑定文はコピー済みです。右下のAIボタンをクリックし、写真を添付して貼り付けるだけで最高のレシピが届きます！</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="text-center p-10 space-y-10 animate-in fade-in">
                ...
                <div className="flex flex-col gap-4">
                  <Button onClick={startCamera} className="bg-red-600 hover:bg-red-500 text-white h-20 px-12 rounded-3xl font-black text-2xl shadow-xl active:scale-95 transition-all italic uppercase">Fridge Scan</Button>
          ...
          <div className="lg:w-2/5 p-12 flex flex-col bg-white overflow-y-auto border-l border-slate-100 text-left">
            <div className="flex-1 space-y-12">
              ...
              <div className="p-8 bg-slate-50 border-2 border-slate-100 rounded-[2.5rem] shadow-inner space-y-4 text-left">
                <div className="space-y-1">
                  <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest block">1. 対象の名前</label>
                  <input className="bg-transparent border-none p-0 font-black text-2xl text-slate-900 w-full focus:ring-0 tracking-tighter" placeholder="対象の名前（例：バナナ）" value={plantName} onChange={(e) => setPlantName(e.target.value)} />
                </div>
                <div className="h-px bg-slate-200 w-full" />
                <div className="space-y-1">
                  <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest block">2. 相談内容</label>
                  <Textarea className="bg-transparent border-none p-0 font-black text-xl text-slate-900 w-full min-h-[80px] resize-none focus:ring-0 leading-relaxed placeholder:text-slate-300" placeholder="今の困りごと..." value={prompt} onChange={(e) => setPrompt(e.target.value)} />
                </div>
              </div>
              ...
              <section className="space-y-6 pt-8 border-t border-slate-100 text-center">
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 gap-5">
                    <Button onClick={() => handleAutoCopyAndGo('https://gemini.google.com/')} disabled={!image} className="h-28 bg-blue-600 hover:bg-blue-500 text-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(37,99,235,0.3)] flex flex-col items-center justify-center group active:scale-95 transition-all border-none relative overflow-hidden">
                      ...
                      <span className="text-[11px] text-blue-100 font-black uppercase tracking-[0.2em] relative z-10">画像解析・Google検索に最適</span>
                    </Button>
                    <div className="grid grid-cols-2 gap-5">
                      <Button onClick={() => handleAutoCopyAndGo('https://chatgpt.com/')} disabled={!image} className="h-20 bg-emerald-600 hover:bg-emerald-500 text-white rounded-3xl shadow-xl flex items-center justify-center gap-3 font-black active:scale-95 transition-all border-none text-lg tracking-tighter uppercase italic"><Bot className="w-6 h-6 text-white" /> ChatGPT</Button>
                      <Button onClick={() => handleAutoCopyAndGo('https://claude.ai/')} disabled={!image} className="h-20 bg-orange-600 hover:bg-orange-500 text-white rounded-3xl shadow-xl flex items-center justify-center gap-3 font-black active:scale-95 transition-all border-none text-lg tracking-tighter uppercase italic"><Heart className="w-6 h-6 fill-white" /> Claude</Button>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="pt-8 border-t border-slate-100 space-y-4 text-left animate-in fade-in">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-lg font-black text-slate-900 italic tracking-tighter uppercase tracking-tight">Cook Videos</h3>
                      </div>
                      <Button variant="ghost" className="text-blue-600 font-black text-xs h-auto p-0" onClick={() => window.open(`https://www.youtube.com/results?search_query=${encodeURIComponent(plantName)}+作り方`, '_blank')}>YouTubeで開く</Button>
                    </div>
                    <Card className="bg-slate-950 border-none rounded-3xl overflow-hidden shadow-xl cursor-pointer group" onClick={() => window.open(`https://www.youtube.com/results?search_query=${encodeURIComponent(plantName)}+作り方`, '_blank')}>
                      <div className="aspect-video relative bg-slate-900 flex items-center justify-center">
                        ...
                        <div className="absolute bottom-6 left-6 right-6"><p className="text-white font-black text-lg tracking-tight line-clamp-1">{plantName} のプロレシピ動画</p></div>
                      </div>
                    </Card>
                  </div>
                )}
              ...
              {isCopied && (
                <div className="p-10 bg-red-50 rounded-[3rem] border-4 border-red-100 animate-in fade-in slide-in-from-top-6 shadow-2xl relative overflow-hidden text-left">
                   ...
                   <div className="flex flex-col gap-1 mb-8 text-left">
                     <div className="flex items-center gap-3 text-red-700 font-black italic text-xl uppercase tracking-tight text-left">鑑定アプリでの操作</div>
