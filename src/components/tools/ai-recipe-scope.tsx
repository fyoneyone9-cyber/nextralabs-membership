...
export default function AiRecipeScope() {
  ...
  const startCamera = async () => {
    try {
      ...
      fileInputRef.current?.click(); // 失敗時に自動でファイル選択を開く
    }
  };
            ...
            {isCameraActive ? (
              ...
              <div className="w-full h-full relative animate-in fade-in">
                ...
                <div className="absolute top-10 right-10 flex gap-3 z-30">
                  ...
                  <Button onClick={() => {setImage(null); setDishName(''); setStatus('idle'); setIsCopied(false);}} className="h-16 w-16 bg-black/60 text-white rounded-full hover:bg-red-500 border-2 border-white/20"><X className="w-8 h-8" /></Button>
                </div>
                {status === 'scanning' && (
                  <div className="absolute inset-0 bg-red-600/30 backdrop-blur-md flex flex-col items-center justify-center animate-pulse text-white font-black text-2xl uppercase tracking-tighter">AI Scanning...</div>
                )}
              ...
              <div className="text-center p-10 space-y-8">
                ...
                <div className="flex flex-col gap-4 max-w-xs mx-auto">
                  <Button onClick={startCamera} className="bg-red-600 hover:bg-red-500 text-white h-24 px-12 rounded-[2rem] font-black text-3xl shadow-2xl transition-all active:scale-95 uppercase italic tracking-tighter">スコープを起動</Button>
                  <Button onClick={() => fileInputRef.current?.click()} variant="outline" className="border-white/10 text-slate-400 hover:bg-white/5 hover:text-white h-16 px-10 rounded-2xl font-black text-lg transition-all uppercase tracking-widest">保存した写真を選択</Button>
                </div>
          ...
          <div className="lg:w-2/5 p-12 flex flex-col bg-white overflow-y-auto border-l border-slate-100">
            <div className="flex-1 space-y-10">
              <div className="flex items-start gap-4 p-6 bg-gradient-to-br from-red-50 to-orange-50 rounded-[2rem] border-2 border-red-100 shadow-sm">
                ...
                <div className="space-y-1"><p className="text-sm text-red-900 font-black">写真を保存してAIボタンを押すだけ！</p><p className="text-[12px] text-red-700 font-bold opacity-70 italic leading-tight">鑑定文は自動でコピーされます。あとはAIに「写真」を渡して「貼り付け」るだけ。</p></div>
              </div>

              <section className="space-y-6">
                <div className="p-6 bg-blue-50/50 border-2 border-blue-100 rounded-[2rem] flex items-center justify-between shadow-sm transition-all hover:shadow-md">
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 gap-5">
                    <Button onClick={() => handleAutoCopyAndGo('https://gemini.google.com/')} disabled={!image || status === 'scanning'} className="h-28 bg-blue-600 hover:bg-blue-500 text-white rounded-[2.5rem] shadow-xl flex flex-col items-center justify-center group active:scale-95 transition-all border-none relative overflow-hidden">
                      ...
                      <span className="text-[10px] text-blue-100 font-black uppercase relative z-10">画像解析に最も強い推奨AI</span>
                    </Button>
                  ...
                  <div className="pt-8 border-t border-slate-100 space-y-4 text-left animate-in fade-in">
                    ...
                    <Card className="bg-slate-950 border-none rounded-3xl overflow-hidden shadow-xl cursor-pointer group shadow-red-600/10" onClick={openYouTube}>
                      <div className="aspect-video relative bg-slate-900 flex items-center justify-center">
                        ...
                        <div className="absolute bottom-6 left-6 right-6 font-black text-white text-lg tracking-tight line-clamp-1">{dishName} のプロレシピ</div>
                      </div>
                    </Card>
                  </div>
                )}
              ...
              {isCopied && (
                <div className="p-10 bg-red-50 rounded-[3rem] border-4 border-red-100 animate-in fade-in slide-in-from-top-6 shadow-2xl relative overflow-hidden text-left">
                   ...
                   <div className="space-y-5 text-[15px] text-red-950 font-black leading-tight text-left">
                     <p className="flex items-start gap-4 text-left font-black"><span className="bg-red-600 text-white w-7 h-7 rounded-xl flex items-center justify-center text-xs flex-shrink-0 mt-0.5 shadow-md font-sans font-black">1</span><span>入力欄の <span className="bg-red-200 px-2 py-0.5 rounded-lg text-red-600">「＋」</span> をタップ</span></p>
                     <p className="flex items-start gap-4 text-left font-black"><span className="bg-red-600 text-white w-7 h-7 rounded-xl flex items-center justify-center text-xs flex-shrink-0 mt-0.5 shadow-md font-sans font-black">2</span><span>保存した <span className="underline decoration-red-500 decoration-[3px] underline-offset-4 font-black text-red-600">現場写真</span> を選択</span></p>
                     <p className="flex items-start gap-4 text-left font-black"><span className="bg-red-600 text-white w-7 h-7 rounded-xl flex items-center justify-center text-xs flex-shrink-0 mt-0.5 shadow-md font-sans font-black">3</span><span>そのまま <span className="underline decoration-red-500 decoration-[3px] underline-offset-4 font-black text-red-600">「貼り付け」</span> して送信！</span></p>
                   </div>
                </div>
              )}
            </div>
            <div className="mt-16 pt-8 border-t border-slate-100 flex items-center justify-between text-[10px] font-black text-slate-300 uppercase tracking-[0.5em]">
