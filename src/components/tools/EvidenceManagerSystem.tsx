...
const MasterEngine = () => {
  ...
  const scanAndOrganize = async () => {
    ...
    try {
      // 実際にはローカルのダウンロードフォルダやデスクトップをスキャンする仕組み
      await new Promise(r => setTimeout(r, 1500));
      setLogs(prev => [`[${new Date().toLocaleTimeString()}] ダウンロードフォルダを特定完了。`, ...prev]);
      
      await new Promise(r => setTimeout(r, 1000));
      setLogs(prev => [`[${new Date().toLocaleTimeString()}] YouTube制作物 (MP3, 画像, 台本) を検出しました。`, ...prev]);
  ...
  return (
    <div className="max-w-4xl mx-auto p-3 md:p-10 space-y-6 md:space-y-10 min-h-screen text-slate-200 font-sans pb-32 bg-[#050507] text-left border-4 md:border-8 border-emerald-500/50 rounded-[2rem] md:rounded-[4rem] my-2 md:my-4 shadow-[0_0_100px_rgba(16,185,129,0.2)]">
      <div className="text-center space-y-1 md:space-y-3">
        <Badge className="bg-amber-600 text-white font-black italic px-3 py-0.5 text-[8px] md:text-[10px] uppercase rounded-full">Record Archival Engine</Badge>
        <h1 className="text-4xl md:text-6xl font-black text-white uppercase italic tracking-tighter leading-none drop-shadow-2xl text-center">Evidence Manager</h1>
      ...
      <div className="bg-[#13141f] border-2 border-white/5 rounded-[2.5rem] p-6 md:p-10 shadow-2xl space-y-8 relative overflow-hidden">
        ...
        <div className="bg-[#0a0b14] border border-white/5 rounded-3xl p-6 flex items-start gap-5 shadow-inner">
          ...
          <div className="space-y-1 text-left">
            <p className="text-[10px] font-black text-amber-500/70 uppercase tracking-[0.2em] italic mb-2">Evidence Protocol</p>
            <div className="space-y-1 text-xs font-bold text-slate-400">
              <p className="flex items-center gap-3"><span className="text-amber-600 italic">#1</span> ダウンロードフォルダ内のファイルをAIが自動分析</p>
              <p className="flex items-center gap-3"><span className="text-amber-600 italic">#2</span> YouTube制作物などの「意味のある証拠」を自動分類</p>
              <p className="flex items-center gap-3"><span className="text-amber-600 italic">#3</span> 不要なゴミを整理し、フォルダ構造を最新化</p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-6">
            ...
            >
              {isProcessing ? (
                ...
                <>
                  ...
                  <span className="text-2xl uppercase italic">Start Archive Protocol</span>
                </>
              )}
            </button>
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="bg-black/40 p-6 rounded-[2rem] border border-white/5 text-center shadow-inner">
                  <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1 italic">Records Found</p>
                  <p className="text-4xl font-black text-white italic">{evidenceCount}</p>
               </div>
               <div className="bg-black/40 p-6 rounded-[2rem] border border-white/5 text-center shadow-inner">
                  <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1 italic">System Health</p>
                  <p className="text-4xl font-black text-emerald-500 italic">100%</p>
               </div>
            </div>
          </div>

          <div className="bg-black rounded-[3rem] p-8 border border-white/5 shadow-inner flex flex-col relative overflow-hidden h-[300px]">
             <div className="flex items-center gap-3 text-amber-500 font-black italic uppercase text-xs mb-4"><Zap size={16} /> Console Output</div>
      ...
      <div className="bg-emerald-600/5 border-2 border-emerald-500/20 rounded-[2.5rem] p-8 space-y-4 italic shadow-inner">
         <div className="flex items-center gap-3 text-emerald-500">
            <CheckCircle2 size={20} />
            <p className="text-xs font-black uppercase tracking-widest">Master Protocol Secured</p>
         </div>
      ...
      <div className="text-center opacity-10 mt-10 font-black uppercase tracking-[0.5em] italic text-[10px]">Records Management OS • NextraLabs 2026</div>
    </div>
  )
}
