  ...
  return (
    <div className="max-w-4xl mx-auto p-3 md:p-10 space-y-6 md:space-y-10 min-h-screen text-slate-200 font-sans pb-32 bg-[#050507] text-left border-4 md:border-8 border-emerald-500/50 rounded-[2rem] md:rounded-[4rem] my-2 md:my-4 shadow-[0_0_100px_rgba(16,185,129,0.2)]">
        ...
        <div className="space-y-6">
          <div className="grid md:grid-cols-2 gap-6 text-left">
            <div className="space-y-2">
              ...
              <input value={hotelName} onChange={(e) => setHotelName(e.target.value)} placeholder="例：星野リゾート BEB5土浦" className="w-full h-14 bg-black border-2 border-white/10 rounded-xl px-6 text-sm text-white focus:border-pink-500 outline-none" />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-500 uppercase px-4 italic">楽天アフィリエイトID</label>
        ...
        {result && (
          <div className="grid lg:grid-cols-2 gap-8 animate-in slide-in-from-bottom-8 text-left">
            <div className="bg-black/40 border-2 border-white/5 rounded-[3rem] p-8 space-y-6 shadow-inner relative overflow-hidden">
               ...
               <button onClick={() => handleCopy(`${result.postText}\n\n詳細はこちら👇\n${result.affiliateLink}`)} className={`w-full h-16 rounded-2xl font-black italic uppercase transition-all shadow-xl flex items-center justify-center gap-3 ${copied ? 'bg-emerald-500 text-slate-950' : 'bg-white text-slate-950 hover:bg-pink-500 hover:text-white'}`}>
            ...
            <div className="space-y-6">
              <div className="bg-[#1a1c2e] p-8 rounded-[2.5rem] border-2 border-emerald-500/20 shadow-2xl">
                 ...
                 <div className="bg-emerald-600/5 p-4 rounded-xl border border-emerald-500/20"><p className="text-xs text-slate-300 font-bold italic">「 {result.strategy} 」</p></div>
              </div>
      ...
      <div className="bg-pink-600/5 border-2 border-pink-500/20 rounded-[2.5rem] p-8 italic shadow-inner text-left">
         ...
         <p className="text-slate-400 text-sm font-bold leading-relaxed">Nextra AIが宿泊者の感動を、思わず予約したくなる紹介文へと変換します。楽天トラベルとの連動により、あなたのSNSが最強の予約窓口に変わります。</p>
      </div>
